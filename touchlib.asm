//Deviec:FT62F08X
//-----------------------Variable---------------------------------
		_PB4		EQU		6CH
		_PB5		EQU		6DH
		_TRISA4		EQU		464H
		_TRISA5		EQU		465H
		_TRISA6		EQU		466H
		_TRISA7		EQU		467H
		_TRISB2		EQU		46AH
		_TRISB3		EQU		46BH
		_TRISB4		EQU		46CH
		_TRISB7		EQU		46FH
		_TRISC0		EQU		470H
		_TRISC1		EQU		471H
		_TRISC3		EQU		473H
		_TRISC4		EQU		474H
		_TRISC5		EQU		475H
		_TRISC6		EQU		476H
		_TRISC7		EQU		477H
		_TRISD5		EQU		47DH
		_LATA0		EQU		860H
		_LATA1		EQU		861H
		_LATA4		EQU		864H
		_LATB0		EQU		868H
		_LATB1		EQU		869H
		_LATB3		EQU		86BH
		_LATC0		EQU		870H
		_LATC1		EQU		871H
		_LATD1		EQU		879H
		_LATD2		EQU		87AH
		_LATD3		EQU		87BH
		_T4UIE		EQU		890H
		_T4UIF		EQU		898H
		_T2UIE		EQU		1868H
		_T2UIF		EQU		1870H
		_TKST		EQU		1C6DH
		_TKRCOV		EQU		1C6EH
		_URTE		EQU		2471H
		_TXEF		EQU		2495H
		_TCF		EQU		24E0H
		_RxData		EQU		DAH
		_RFs.RFsp		EQU		A6H
		_RFs.tRFH		EQU		A7H
		_RFs.tRFL		EQU		A8H
		_RFs.BitCount		EQU		A9H
		_RFs.Bptr		EQU		AAH
		_RFs.RecBufFull		EQU		ABH
		_RFs		EQU		A6H
		_buf_rec		EQU		ACH
		_RemOutTime		EQU		D9H
		_tRFtemp		EQU		D8H
		_keybit1		EQU		D7H
		_keybit2		EQU		D6H
		_keybit3		EQU		D5H
		_keybit4		EQU		D4H
		_keybit5		EQU		D3H
		_SysFunFlg0.OneBits		EQU		49H
		_SysFunFlg0.AllBits		EQU		49H
		_SysFunFlg0		EQU		49H
		_SysFunFlg1.OneBits		EQU		48H
		_SysFunFlg1.AllBits		EQU		48H
		_SysFunFlg1		EQU		48H
		_SysFunFlg2.OneBits		EQU		46H
		_SysFunFlg2.AllBits		EQU		46H
		_SysFunFlg2		EQU		46H
		_PwrType		EQU		7BH
		_ModeType		EQU		7AH
		_HeatType		EQU		E4H
		_StrongType		EQU		79H
		_ManualType		EQU		78H
		_TimeType		EQU		E1H
		_KeyType		EQU		45H
		_LastMode		EQU		E3H
		_LastManual		EQU		E2H
		_BuzzerCnt		EQU		C5H
		_TmpCnt		EQU		C1H
		_KeyCnt		EQU		BFH
		_DspIndex		EQU		E0H
		_dsp_100ms		EQU		D2H
		_TouchTimeCount		EQU		C3H
		_readKeyNum		EQU		39H
		_TimeUs		EQU		DFH
		_Time2Ms		EQU		DEH
		_Time10Ms		EQU		DDH
		_Time100Ms		EQU		DCH
		_TimeSec		EQU		DBH
		_TimeMin		EQU		77H
		_NumD		EQU		44H
		_TouchcapBuff1		EQU		120H
		_TouchcapBuff2		EQU		1A0H
		_fmd_updataChannel		EQU		14DH
		_fmd_OriginalData		EQU		21H
		_fmd_ReferenceData		EQU		25H
		_fmd_ResultData		EQU		27H
		_fmd_ResultDataH		EQU		29H
		_fmd_Null0		EQU		20H
		_fmd_Null1		EQU		23H
		_fmd_Null2		EQU		24H
		_fmd_Null3		EQU		2AH
		_fmd_antishaketimecount		EQU		43H
		_fmd_antishaketimecount1		EQU		CDH
		_fmd_Keystatus		EQU		37H
		_fmd_Keystatus1		EQU		35H
		_fmd_Keystatus2		EQU		33H
		_fmd_KeyBuffPress		EQU		42H
		_fmd_KeyBuffUnPress		EQU		41H
		_fmd_channels		EQU		47H
		_fmd_a		EQU		4BH
		_fmd_a1		EQU		40H
		_fmd_a2		EQU		3FH
		_fmd_a3		EQU		CCH
		_fmd_u		EQU		3EH
		_fmd_thedeathcount		EQU		BDH
		_fmd_thedeathcount1		EQU		BBH
		_fmd_thedeathcount2		EQU		B9H
		_fmd_currentCount		EQU		CBH
		_fmd_TouchDataBuff_Key		EQU		2BH
		_fmd_nodownavrdata_Key		EQU		2378H
		_fmd_nodownsum_Key		EQU		2300H
		_fmd_KeyDValue_Key		EQU		B5H
		_fmd_KeyDValue_Key_base		EQU		B1H
		_fmd_KeyDValue_Key_baseBuff		EQU		15CH
		_fmd_theUsedChannel		EQU		13EH
		_fmd_f_keydown_flag		EQU		76H
		_fmd_f_firstdown		EQU		76H
		_fmd_f_firstloop		EQU		76H
		_fmd_f_main_key		EQU		76H
		_fmd_f_forbidden		EQU		76H
		_fmd_updataFlag		EQU		3DH
		_fmd_TestCount		EQU		CAH
		_fmd_PressOff		EQU		C9H
		_fmd_PressOffCount		EQU		C8H
		_fmd_UpTouchCount		EQU		EBH
		_fmd_DownTouchCount		EQU		E9H
		_fmd_HfreqOn		EQU		3CH
		_fmd_AntiWaterBit		EQU		4AH
		_fmd_x		EQU		3BH
		_fmd_UpDataTime		EQU		E7H
		_fmd_DownDataTime		EQU		E5H
		_fmd_Startgx		EQU		C7H
		_send_buf		EQU		A0H
//		CheckRfCmd@rfcmd		EQU		4CH
//		CheckRfCmd@rfcmd		EQU		4CH
//		CheckRfCmd@rfcmd		EQU		4CH
//		UartSetEmptyInt@value		EQU		4CH
//		UartSetEmptyInt@value		EQU		4CH
//		UartSetEmptyInt@value		EQU		4CH
//		___awdiv@quotient		EQU		52H
//		___awdiv@sign		EQU		51H
//		___awdiv@counter		EQU		50H
//		___awdiv@divisor		EQU		4CH
//		___awdiv@dividend		EQU		4EH
//		refresh_display@keybuf		EQU		4CH
//		refresh_display@keybuf		EQU		4CH
//		refresh_display@keybuf		EQU		4CH
//		MxSet013@a		EQU		4CH
//		MxSet013@a		EQU		4CH
//		MxSet013@a		EQU		4CH
//		___wmul@product		EQU		50H
//		___wmul@multiplier		EQU		4CH
//		___wmul@multiplicand		EQU		4EH
//		StartHigeFreq@a		EQU		51H
//		StartHigeFreq@a		EQU		51H
//		StartHigeFreq@a		EQU		51H
//		TouchInit@Time		EQU		4CH
//		TouchInit@Time		EQU		4CH
//		TouchInit@Time		EQU		4CH
//		HighFreqCo@a		EQU		52H
//		HighFreqCo@a		EQU		52H
//		HighFreqCo@a		EQU		52H
//		send_uart_data@ByteNum		EQU		CEH
//		i1UartSetEmptyInt@value		EQU		70H
//		i1UartSetEmptyInt@value		EQU		70H
//		i1UartSetEmptyInt@value		EQU		70H
//		DisplayHandler@LedMenuIndex		EQU		D1H
//		DisplayHandler@DspBuf		EQU		D0H
//		DisplayHandler@DspCnt		EQU		CFH
//		SpecialDisplay2@DspBuf		EQU		75H
//		___lbmod@dividend		EQU		72H
//		___lbmod@rem		EQU		74H
//		___lbmod@counter		EQU		73H
//		___lbmod@dividend		EQU		72H
//		___lbmod@divisor		EQU		70H
//		___lbmod@dividend		EQU		72H
//		SpecialDisplay1@DspBuf		EQU		74H
//		___lbdiv@dividend		EQU		71H
//		___lbdiv@quotient		EQU		73H
//		___lbdiv@counter		EQU		72H
//		___lbdiv@dividend		EQU		71H
//		___lbdiv@divisor		EQU		70H
//		___lbdiv@dividend		EQU		71H
//		ShowLedData@DspBuf		EQU		70H
//		ShowLedData@DspBuf		EQU		70H
//		ShowLedData@DspBuf		EQU		70H
//-----------------------Variable END---------------------------------

		ORG		0000H
		MOVLP 	0H 			//0000 	0180
		LJUMP 	37H 			//0001 	3837
		ORG		0003H
		NOP 					//0003 	1000
		BSR 	7EH, 0H 			//0004 	247E
		MOVLP 	0H 			//0005 	0180

		//;CHIPINIT.C: 54: if(T2UIE && T2UIF)
		MOVLB 	6H 			//0006 	1026
		BTSC 	DH, 0H 			//0007 	280D
		BTSS 	EH, 0H 			//0008 	2C0E
		LJUMP 	11H 			//0009 	3811

		//;CHIPINIT.C: 55: {
		//;CHIPINIT.C: 56: T2UIF = 1;
		BSR 	EH, 0H 			//000A 	240E

		//;CHIPINIT.C: 57: ReadRf433();
		MOVLP 	5H 			//000B 	0185
		LCALL 	5ACH 			//000C 	35AC
		MOVLP 	0H 			//000D 	0180

		//;CHIPINIT.C: 58: exact_event_process();
		MOVLP 	6H 			//000E 	0186
		LCALL 	624H 			//000F 	3624
		MOVLP 	0H 			//0010 	0180

		//;CHIPINIT.C: 60: }
		//;CHIPINIT.C: 63: if(T4UIE && T4UIF)
		MOVLB 	2H 			//0011 	1022
		BTSC 	12H, 0H 		//0012 	2812
		BTSS 	13H, 0H 		//0013 	2C13
		LJUMP 	2CH 			//0014 	382C

		//;CHIPINIT.C: 64: {
		//;CHIPINIT.C: 65: T4UIF = 1;
		BSR 	13H, 0H 			//0015 	2413

		//;CHIPINIT.C: 67: DisplayHandler();
		MOVLP 	15H 			//0016 	0195
		LCALL 	531H 			//0017 	3531
		MOVLP 	0H 			//0018 	0180

		//;CHIPINIT.C: 69: if(BuzzerCnt)
		MOVLB 	1H 			//0019 	1021
		LDR 	45H, 0H 			//001A 	1845
		IORWR 	46H, 0H 		//001B 	1446
		BTSC 	3H, 2H 			//001C 	2903
		LJUMP 	2CH 			//001D 	382C

		//;CHIPINIT.C: 70: {
		//;CHIPINIT.C: 71: PB5=~PB5;
		LDWI 	20H 			//001E 	0020
		MOVLB 	0H 			//001F 	1020
		XORWR 	DH, 1H 		//0020 	168D

		//;CHIPINIT.C: 72: BuzzerCnt--;
		LDWI 	1H 			//0021 	0001
		MOVLB 	1H 			//0022 	1021
		SUBWR 	45H, 1H 		//0023 	12C5
		LDWI 	0H 			//0024 	0000
		SUBWFB 	46H, 1H 		//0025 	0BC6

		//;CHIPINIT.C: 73: if(BuzzerCnt==0)
		LDR 	45H, 0H 			//0026 	1845
		IORWR 	46H, 0H 		//0027 	1446
		BTSS 	3H, 2H 			//0028 	2D03
		LJUMP 	2CH 			//0029 	382C

		//;CHIPINIT.C: 74: {
		//;CHIPINIT.C: 75: PB5=0;
		MOVLB 	0H 			//002A 	1020
		BCR 	DH, 5H 			//002B 	228D

		//;CHIPINIT.C: 76: }
		//;CHIPINIT.C: 77: }
		//;CHIPINIT.C: 78: }
		//;CHIPINIT.C: 79: if(URTE && TXEF)
		MOVLB 	9H 			//002C 	1029
		BTSC 	EH, 1H 			//002D 	288E
		BTSS 	12H, 5H 		//002E 	2E92
		LJUMP 	32H 			//002F 	3832

		//;CHIPINIT.C: 80: {
		//;CHIPINIT.C: 81: send_uart_data();
		MOVLP 	6H 			//0030 	0186
		LCALL 	651H 			//0031 	3651

		//;CHIPINIT.C: 82: }
		//;CHIPINIT.C: 83: if(EPIF0&0x10)
		MOVLB 	0H 			//0032 	1020
		BTSC 	14H, 4H 		//0033 	2A14

		//;CHIPINIT.C: 84: {
		//;CHIPINIT.C: 85: EPIF0|=0x10;
		BSR 	14H, 4H 			//0034 	2614
		BCR 	7EH, 0H 			//0035 	207E
		RETI 					//0036 	1009
		MOVLP 	8H 			//0037 	0188
		LJUMP 	16H 			//0038 	3816
		CLRF 	42H 			//0039 	11C2
		CLRF 	41H 			//003A 	11C1
		CLRF 	3EH 			//003B 	11BE
		LDWI 	2H 			//003C 	0002
		SUBWR 	3EH, 0H 		//003D 	123E
		BTSC 	3H, 0H 			//003E 	2803
		LJUMP 	10CH 			//003F 	390C
		LSLF 	3EH, 0H 		//0040 	053E
		ADDWI 	2BH 			//0041 	0E2B
		STR 	6H 			//0042 	1086
		CLRF 	7H 			//0043 	1187
		MOVIW 	0H[1] 			//0044 	0F40
		STR 	53H 			//0045 	10D3
		MOVIW 	1H[1] 			//0046 	0F41
		STR 	54H 			//0047 	10D4
		LDR 	3EH, 0H 			//0048 	183E
		STR 	55H 			//0049 	10D5
		CLRF 	56H 			//004A 	11D6
		LSLF 	55H, 1H 		//004B 	05D5
		RLR 	56H, 1H 			//004C 	1DD6
		LDR 	4BH, 0H 			//004D 	184B
		STR 	57H 			//004E 	10D7
		LDWI 	78H 			//004F 	0078
		CLRF 	58H 			//0050 	11D8
		LSLF 	57H, 1H 		//0051 	05D7
		RLR 	58H, 1H 			//0052 	1DD8
		LSLF 	57H, 1H 		//0053 	05D7
		RLR 	58H, 1H 			//0054 	1DD8
		LSLF 	57H, 1H 		//0055 	05D7
		RLR 	58H, 1H 			//0056 	1DD8
		STR 	59H 			//0057 	10D9
		LDWI 	23H 			//0058 	0023
		STR 	5AH 			//0059 	10DA
		LDR 	57H, 0H 			//005A 	1857
		ADDWR 	59H, 0H 		//005B 	1759
		STR 	5BH 			//005C 	10DB
		LDR 	58H, 0H 			//005D 	1858
		ADDWFC 	5AH, 0H 		//005E 	0D5A
		STR 	5CH 			//005F 	10DC
		LDR 	55H, 0H 			//0060 	1855
		ADDWR 	5BH, 0H 		//0061 	175B
		STR 	6H 			//0062 	1086
		LDR 	56H, 0H 			//0063 	1856
		ADDWFC 	5CH, 0H 		//0064 	0D5C
		STR 	7H 			//0065 	1087
		MOVIW 	0H[1] 			//0066 	0F40
		STR 	5DH 			//0067 	10DD
		MOVIW 	1H[1] 			//0068 	0F41
		STR 	5EH 			//0069 	10DE
		LDR 	54H, 0H 			//006A 	1854
		SUBWR 	5EH, 0H 		//006B 	125E
		BTSS 	3H, 2H 			//006C 	2D03
		LJUMP 	70H 			//006D 	3870
		LDR 	53H, 0H 			//006E 	1853
		SUBWR 	5DH, 0H 		//006F 	125D
		BTSS 	3H, 0H 			//0070 	2C03
		LJUMP 	D2H 			//0071 	38D2
		LSLF 	3EH, 0H 		//0072 	053E
		ADDWI 	2BH 			//0073 	0E2B
		STR 	6H 			//0074 	1086
		CLRF 	7H 			//0075 	1187
		LDR 	3EH, 0H 			//0076 	183E
		LCALL 	4CCH 			//0077 	34CC
		MOVLP 	0H 			//0078 	0180
		STR 	55H 			//0079 	10D5
		LDWI 	78H 			//007A 	0078
		CLRF 	56H 			//007B 	11D6
		LSLF 	55H, 1H 		//007C 	05D5
		RLR 	56H, 1H 			//007D 	1DD6
		LSLF 	55H, 1H 		//007E 	05D5
		RLR 	56H, 1H 			//007F 	1DD6
		LSLF 	55H, 1H 		//0080 	05D5
		RLR 	56H, 1H 			//0081 	1DD6
		STR 	57H 			//0082 	10D7
		LDWI 	23H 			//0083 	0023
		STR 	58H 			//0084 	10D8
		LDR 	55H, 0H 			//0085 	1855
		ADDWR 	57H, 0H 		//0086 	1757
		STR 	59H 			//0087 	10D9
		LDR 	56H, 0H 			//0088 	1856
		ADDWFC 	58H, 0H 		//0089 	0D58
		STR 	5AH 			//008A 	10DA
		LDR 	53H, 0H 			//008B 	1853
		ADDWR 	59H, 0H 		//008C 	1759
		STR 	4H 			//008D 	1084
		LDR 	54H, 0H 			//008E 	1854
		ADDWFC 	5AH, 0H 		//008F 	0D5A
		STR 	5H 			//0090 	1085
		MOVIW 	0H[1] 			//0091 	0F40
		SUBWR 	0H, 0H 		//0092 	1200
		STR 	5BH 			//0093 	10DB
		ADDFSR 	0H, 1H 		//0094 	0101
		MOVIW 	1H[1] 			//0095 	0F41
		SUBWFB 	0H, 0H 		//0096 	0B00
		STR 	5CH 			//0097 	10DC
		LDWI 	0H 			//0098 	0000
		SUBWR 	5CH, 0H 		//0099 	125C
		LDWI 	FFH 			//009A 	00FF
		BTSC 	3H, 2H 			//009B 	2903
		SUBWR 	5BH, 0H 		//009C 	125B
		LDR 	3EH, 0H 			//009D 	183E
		BTSC 	3H, 0H 			//009E 	2803
		LJUMP 	CCH 			//009F 	38CC
		ADDWI 	B5H 			//00A0 	0EB5
		STR 	6H 			//00A1 	1086
		CLRF 	7H 			//00A2 	1187
		LSLF 	3EH, 0H 		//00A3 	053E
		ADDWI 	2BH 			//00A4 	0E2B
		STR 	4H 			//00A5 	1084
		CLRF 	5H 			//00A6 	1185
		LDR 	0H, 0H 			//00A7 	1800
		STR 	53H 			//00A8 	10D3
		LDR 	3EH, 0H 			//00A9 	183E
		STR 	54H 			//00AA 	10D4
		CLRF 	55H 			//00AB 	11D5
		LSLF 	54H, 1H 		//00AC 	05D4
		RLR 	55H, 1H 			//00AD 	1DD5
		LDR 	4BH, 0H 			//00AE 	184B
		STR 	56H 			//00AF 	10D6
		LDWI 	78H 			//00B0 	0078
		CLRF 	57H 			//00B1 	11D7
		LSLF 	56H, 1H 		//00B2 	05D6
		RLR 	57H, 1H 			//00B3 	1DD7
		LSLF 	56H, 1H 		//00B4 	05D6
		RLR 	57H, 1H 			//00B5 	1DD7
		LSLF 	56H, 1H 		//00B6 	05D6
		RLR 	57H, 1H 			//00B7 	1DD7
		STR 	58H 			//00B8 	10D8
		LDWI 	23H 			//00B9 	0023
		STR 	59H 			//00BA 	10D9
		LDR 	56H, 0H 			//00BB 	1856
		ADDWR 	58H, 0H 		//00BC 	1758
		STR 	5AH 			//00BD 	10DA
		LDR 	57H, 0H 			//00BE 	1857
		ADDWFC 	59H, 0H 		//00BF 	0D59
		STR 	5BH 			//00C0 	10DB
		LDR 	54H, 0H 			//00C1 	1854
		ADDWR 	5AH, 0H 		//00C2 	175A
		STR 	4H 			//00C3 	1084
		LDR 	55H, 0H 			//00C4 	1855
		ADDWFC 	5BH, 0H 		//00C5 	0D5B
		STR 	5H 			//00C6 	1085
		LDR 	0H, 0H 			//00C7 	1800
		STR 	5CH 			//00C8 	10DC
		LDR 	53H, 0H 			//00C9 	1853
		SUBWR 	5CH, 0H 		//00CA 	125C
		LJUMP 	D0H 			//00CB 	38D0
		ADDWI 	B5H 			//00CC 	0EB5
		STR 	6H 			//00CD 	1086
		LDWI 	FFH 			//00CE 	00FF
		CLRF 	7H 			//00CF 	1187
		STR 	1H 			//00D0 	1081
		LJUMP 	D7H 			//00D1 	38D7
		LDR 	3EH, 0H 			//00D2 	183E
		ADDWI 	B5H 			//00D3 	0EB5
		STR 	6H 			//00D4 	1086
		CLRF 	7H 			//00D5 	1187
		CLRF 	1H 			//00D6 	1181
		LCALL 	55EH 			//00D7 	355E
		MOVLP 	0H 			//00D8 	0180
		LCALL 	532H 			//00D9 	3532
		MOVLP 	0H 			//00DA 	0180
		LCALL 	403H 			//00DB 	3403
		MOVLP 	0H 			//00DC 	0180
		LDWI 	0H 			//00DD 	0000
		STR 	53H 			//00DE 	10D3
		LDWI 	98H 			//00DF 	0098
		LCALL 	540H 			//00E0 	3540
		MOVLP 	0H 			//00E1 	0180
		STR 	57H 			//00E2 	10D7
		LDR 	1H, 0H 			//00E3 	1801
		SUBWR 	57H, 0H 		//00E4 	1257
		BTSS 	3H, 0H 			//00E5 	2C03
		LJUMP 	F0H 			//00E6 	38F0
		LDWI 	1H 			//00E7 	0001
		STR 	53H 			//00E8 	10D3
		INCR 	3EH, 0H 		//00E9 	1A3E
		LJUMP 	ECH 			//00EA 	38EC
		LSLF 	53H, 1H 		//00EB 	05D3
		DECRSZ 	9H, 1H 		//00EC 	1B89
		LJUMP 	EBH 			//00ED 	38EB
		LDR 	53H, 0H 			//00EE 	1853
		IORWR 	41H, 1H 		//00EF 	14C1
		LCALL 	55EH 			//00F0 	355E
		MOVLP 	0H 			//00F1 	0180
		LCALL 	532H 			//00F2 	3532
		MOVLP 	0H 			//00F3 	0180
		LCALL 	403H 			//00F4 	3403
		MOVLP 	0H 			//00F5 	0180
		LDWI 	5AH 			//00F6 	005A
		STR 	53H 			//00F7 	10D3
		LDWI 	98H 			//00F8 	0098
		LCALL 	540H 			//00F9 	3540
		MOVLP 	0H 			//00FA 	0180
		SUBWR 	1H, 0H 		//00FB 	1201
		BTSS 	3H, 0H 			//00FC 	2C03
		LJUMP 	107H 			//00FD 	3907
		LDWI 	1H 			//00FE 	0001
		STR 	53H 			//00FF 	10D3
		INCR 	3EH, 0H 		//0100 	1A3E
		LJUMP 	103H 			//0101 	3903
		LSLF 	53H, 1H 		//0102 	05D3
		DECRSZ 	9H, 1H 		//0103 	1B89
		LJUMP 	102H 			//0104 	3902
		LDR 	53H, 0H 			//0105 	1853
		IORWR 	42H, 1H 		//0106 	14C2
		INCR 	3EH, 1H 		//0107 	1ABE
		LDWI 	2H 			//0108 	0002
		SUBWR 	3EH, 0H 		//0109 	123E
		BTSS 	3H, 0H 			//010A 	2C03
		LJUMP 	40H 			//010B 	3840
		LCALL 	57CH 			//010C 	357C
		MOVLP 	0H 			//010D 	0180
		LJUMP 	111H 			//010E 	3911
		LSRF 	54H, 1H 		//010F 	06D4
		RRR 	53H, 1H 			//0110 	1CD3
		DECRSZ 	9H, 1H 		//0111 	1B89
		LJUMP 	10FH 			//0112 	390F
		BTSS 	53H, 0H 		//0113 	2C53
		LJUMP 	18DH 			//0114 	398D
		MOVLB 	1H 			//0115 	1021
		LDWI 	3H 			//0116 	0003
		INCR 	4AH, 1H 		//0117 	1ACA
		MOVLB 	0H 			//0118 	1020
		ANDWR 	41H, 0H 		//0119 	1541
		STR 	53H 			//011A 	10D3
		XORWI 	3H 			//011B 	0A03
		BTSS 	3H, 2H 			//011C 	2D03
		LJUMP 	12DH 			//011D 	392D
		LDWI 	1H 			//011E 	0001
		STR 	53H 			//011F 	10D3
		CLRF 	54H 			//0120 	11D4
		INCR 	4BH, 0H 		//0121 	1A4B
		LJUMP 	125H 			//0122 	3925
		LSLF 	53H, 1H 		//0123 	05D3
		RLR 	54H, 1H 			//0124 	1DD4
		DECRSZ 	9H, 1H 		//0125 	1B89
		LJUMP 	123H 			//0126 	3923
		LCALL 	58EH 			//0127 	358E
		MOVLP 	0H 			//0128 	0180
		LCALL 	5A1H 			//0129 	35A1
		MOVLP 	0H 			//012A 	0180
		DECR 	4AH, 1H 		//012B 	13CA
		LJUMP 	165H 			//012C 	3965
		LDR 	41H, 0H 			//012D 	1841
		ANDWI 	3H 			//012E 	0903
		BTSC 	3H, 2H 			//012F 	2903
		LJUMP 	165H 			//0130 	3965
		CLRF 	3CH 			//0131 	11BC
		INCR 	3CH, 1H 		//0132 	1ABC
		LDR 	4BH, 0H 			//0133 	184B
		MOVLP 	9H 			//0134 	0189
		LCALL 	1E6H 			//0135 	31E6
		MOVLP 	0H 			//0136 	0180
		MOVLB 	0H 			//0137 	1020
		LDR 	4BH, 0H 			//0138 	184B
		LCALL 	418H 			//0139 	3418
		MOVLP 	0H 			//013A 	0180
		MOVLB 	0H 			//013B 	1020
		CLRF 	3CH 			//013C 	11BC
		LCALL 	532H 			//013D 	3532
		MOVLP 	0H 			//013E 	0180
		LCALL 	403H 			//013F 	3403
		MOVLP 	0H 			//0140 	0180
		LDR 	4DH, 0H 			//0141 	184D
		STR 	5H 			//0142 	1085
		LDR 	4CH, 0H 			//0143 	184C
		STR 	4H 			//0144 	1084
		LDWI 	3H 			//0145 	0003
		ADDWR 	4H, 1H 		//0146 	1784
		LDWI 	98H 			//0147 	0098
		ADDWFC 	5H, 1H 		//0148 	0D85
		LDR 	0H, 0H 			//0149 	1800
		STR 	53H 			//014A 	10D3
		MOVLB 	1H 			//014B 	1021
		LDR 	38H, 0H 			//014C 	1838
		MOVLB 	0H 			//014D 	1020
		SUBWR 	53H, 0H 		//014E 	1253
		BTSS 	3H, 0H 			//014F 	2C03
		LJUMP 	165H 			//0150 	3965
		LCALL 	532H 			//0151 	3532
		MOVLP 	0H 			//0152 	0180
		LCALL 	403H 			//0153 	3403
		MOVLP 	0H 			//0154 	0180
		LDR 	4DH, 0H 			//0155 	184D
		STR 	5H 			//0156 	1085
		LDR 	4CH, 0H 			//0157 	184C
		STR 	4H 			//0158 	1084
		LDWI 	2H 			//0159 	0002
		ADDWR 	4H, 1H 		//015A 	1784
		LDWI 	98H 			//015B 	0098
		ADDWFC 	5H, 1H 		//015C 	0D85
		LDR 	0H, 0H 			//015D 	1800
		STR 	53H 			//015E 	10D3
		MOVLB 	1H 			//015F 	1021
		LDR 	37H, 0H 			//0160 	1837
		MOVLB 	0H 			//0161 	1020
		SUBWR 	53H, 0H 		//0162 	1253
		BTSC 	3H, 0H 			//0163 	2803
		LJUMP 	11EH 			//0164 	391E
		MOVLB 	0H 			//0165 	1020
		LDR 	36H, 0H 			//0166 	1836
		STR 	54H 			//0167 	10D4
		LDR 	35H, 0H 			//0168 	1835
		STR 	53H 			//0169 	10D3
		INCR 	4BH, 0H 		//016A 	1A4B
		LJUMP 	16EH 			//016B 	396E
		LSRF 	54H, 1H 		//016C 	06D4
		RRR 	53H, 1H 			//016D 	1CD3
		DECRSZ 	9H, 1H 		//016E 	1B89
		LJUMP 	16CH 			//016F 	396C
		BTSS 	53H, 0H 		//0170 	2C53
		LJUMP 	203H 			//0171 	3A03
		LCALL 	57CH 			//0172 	357C
		MOVLP 	0H 			//0173 	0180
		LJUMP 	177H 			//0174 	3977
		LSRF 	54H, 1H 		//0175 	06D4
		RRR 	53H, 1H 			//0176 	1CD3
		DECRSZ 	9H, 1H 		//0177 	1B89
		LJUMP 	175H 			//0178 	3975
		BTSS 	53H, 0H 		//0179 	2C53
		LJUMP 	17EH 			//017A 	397E
		MOVLB 	1H 			//017B 	1021
		CLRF 	49H 			//017C 	11C9
		LJUMP 	203H 			//017D 	3A03
		MOVLB 	1H 			//017E 	1021
		LDR 	49H, 0H 			//017F 	1849
		BTSS 	3H, 2H 			//0180 	2D03
		LJUMP 	185H 			//0181 	3985
		CLRF 	48H 			//0182 	11C8
		INCR 	48H, 1H 		//0183 	1AC8
		LJUMP 	187H 			//0184 	3987
		LDWI 	2H 			//0185 	0002
		STR 	48H 			//0186 	10C8
		CLRF 	49H 			//0187 	11C9
		INCR 	49H, 1H 		//0188 	1AC9
		CLRF 	3DH 			//0189 	11BD
		CLRF 	3EH 			//018A 	11BE
		BCR 	76H, 3H 			//018B 	21F6
		LJUMP 	203H 			//018C 	3A03
		LDWI 	3H 			//018D 	0003
		ANDWR 	42H, 0H 		//018E 	1542
		STR 	53H 			//018F 	10D3
		XORWI 	3H 			//0190 	0A03
		BTSS 	3H, 2H 			//0191 	2D03
		LJUMP 	1A6H 			//0192 	39A6
		LDWI 	1H 			//0193 	0001
		MOVLB 	0H 			//0194 	1020
		STR 	53H 			//0195 	10D3
		CLRF 	54H 			//0196 	11D4
		INCR 	4BH, 0H 		//0197 	1A4B
		LJUMP 	19BH 			//0198 	399B
		LSLF 	53H, 1H 		//0199 	05D3
		RLR 	54H, 1H 			//019A 	1DD4
		DECRSZ 	9H, 1H 		//019B 	1B89
		LJUMP 	199H 			//019C 	3999
		LDR 	53H, 0H 			//019D 	1853
		IORWR 	37H, 1H 		//019E 	14B7
		LDR 	54H, 0H 			//019F 	1854
		IORWR 	38H, 1H 		//01A0 	14B8
		LCALL 	5A1H 			//01A1 	35A1
		MOVLP 	0H 			//01A2 	0180
		INCR 	4AH, 1H 		//01A3 	1ACA
		BSR 	76H, 4H 			//01A4 	2676
		LJUMP 	203H 			//01A5 	3A03
		LDR 	42H, 0H 			//01A6 	1842
		ANDWI 	3H 			//01A7 	0903
		BTSC 	3H, 2H 			//01A8 	2903
		LJUMP 	203H 			//01A9 	3A03
		CLRF 	3CH 			//01AA 	11BC
		INCR 	3CH, 1H 		//01AB 	1ABC
		LDR 	4BH, 0H 			//01AC 	184B
		MOVLP 	9H 			//01AD 	0189
		LCALL 	1E6H 			//01AE 	31E6
		MOVLP 	0H 			//01AF 	0180
		MOVLB 	0H 			//01B0 	1020
		LDR 	4BH, 0H 			//01B1 	184B
		LCALL 	418H 			//01B2 	3418
		MOVLP 	0H 			//01B3 	0180
		MOVLB 	0H 			//01B4 	1020
		CLRF 	3CH 			//01B5 	11BC
		LCALL 	532H 			//01B6 	3532
		MOVLP 	0H 			//01B7 	0180
		LCALL 	403H 			//01B8 	3403
		MOVLP 	0H 			//01B9 	0180
		LDR 	4DH, 0H 			//01BA 	184D
		STR 	5H 			//01BB 	1085
		LDR 	4CH, 0H 			//01BC 	184C
		STR 	4H 			//01BD 	1084
		LDWI 	5DH 			//01BE 	005D
		ADDWR 	4H, 1H 		//01BF 	1784
		LDWI 	98H 			//01C0 	0098
		ADDWFC 	5H, 1H 		//01C1 	0D85
		LDR 	0H, 0H 			//01C2 	1800
		MOVLB 	1H 			//01C3 	1021
		SUBWR 	38H, 0H 		//01C4 	1238
		BTSS 	3H, 0H 			//01C5 	2C03
		LJUMP 	203H 			//01C6 	3A03
		MOVLB 	0H 			//01C7 	1020
		LCALL 	532H 			//01C8 	3532
		MOVLP 	0H 			//01C9 	0180
		LCALL 	403H 			//01CA 	3403
		MOVLP 	0H 			//01CB 	0180
		LDR 	4DH, 0H 			//01CC 	184D
		STR 	5H 			//01CD 	1085
		LDR 	4CH, 0H 			//01CE 	184C
		STR 	4H 			//01CF 	1084
		LDWI 	5CH 			//01D0 	005C
		ADDWR 	4H, 1H 		//01D1 	1784
		LDWI 	98H 			//01D2 	0098
		ADDWFC 	5H, 1H 		//01D3 	0D85
		LDR 	0H, 0H 			//01D4 	1800
		MOVLB 	1H 			//01D5 	1021
		SUBWR 	37H, 0H 		//01D6 	1237
		BTSS 	3H, 0H 			//01D7 	2C03
		LJUMP 	203H 			//01D8 	3A03
		MOVLB 	0H 			//01D9 	1020
		LCALL 	532H 			//01DA 	3532
		MOVLP 	0H 			//01DB 	0180
		LCALL 	403H 			//01DC 	3403
		MOVLP 	0H 			//01DD 	0180
		LDR 	4DH, 0H 			//01DE 	184D
		STR 	5H 			//01DF 	1085
		LDR 	4CH, 0H 			//01E0 	184C
		STR 	4H 			//01E1 	1084
		LDWI 	5AH 			//01E2 	005A
		ADDWR 	4H, 1H 		//01E3 	1784
		LDWI 	98H 			//01E4 	0098
		LCALL 	563H 			//01E5 	3563
		MOVLP 	0H 			//01E6 	0180
		BTSS 	3H, 2H 			//01E7 	2D03
		LJUMP 	1ECH 			//01E8 	39EC
		LDR 	53H, 0H 			//01E9 	1853
		MOVLB 	1H 			//01EA 	1021
		SUBWR 	35H, 0H 		//01EB 	1235
		BTSC 	3H, 0H 			//01EC 	2803
		LJUMP 	193H 			//01ED 	3993
		MOVLB 	0H 			//01EE 	1020
		LCALL 	532H 			//01EF 	3532
		MOVLP 	0H 			//01F0 	0180
		LCALL 	403H 			//01F1 	3403
		MOVLP 	0H 			//01F2 	0180
		LDR 	4DH, 0H 			//01F3 	184D
		STR 	5H 			//01F4 	1085
		LDR 	4CH, 0H 			//01F5 	184C
		STR 	4H 			//01F6 	1084
		LDWI 	5BH 			//01F7 	005B
		ADDWR 	4H, 1H 		//01F8 	1784
		LDWI 	98H 			//01F9 	0098
		LCALL 	563H 			//01FA 	3563
		MOVLP 	0H 			//01FB 	0180
		BTSS 	3H, 2H 			//01FC 	2D03
		LJUMP 	201H 			//01FD 	3A01
		LDR 	53H, 0H 			//01FE 	1853
		MOVLB 	1H 			//01FF 	1021
		SUBWR 	36H, 0H 		//0200 	1236
		BTSC 	3H, 0H 			//0201 	2803
		LJUMP 	193H 			//0202 	3993
		BTSS 	76H, 4H 		//0203 	2E76
		LJUMP 	292H 			//0204 	3A92
		BCR 	76H, 4H 			//0205 	2276
		BTSC 	76H, 3H 		//0206 	29F6
		LJUMP 	22CH 			//0207 	3A2C
		BSR 	76H, 3H 			//0208 	25F6
		BSR 	76H, 2H 			//0209 	2576
		MOVLB 	0H 			//020A 	1020
		LDR 	3FH, 0H 			//020B 	183F
		LCALL 	588H 			//020C 	3588
		MOVLP 	0H 			//020D 	0180
		MOVLB 	1H 			//020E 	1021
		STR 	4CH 			//020F 	10CC
		MOVLB 	0H 			//0210 	1020
		CLRF 	3EH 			//0211 	11BE
		LDWI 	2H 			//0212 	0002
		SUBWR 	3EH, 0H 		//0213 	123E
		BTSC 	3H, 0H 			//0214 	2803
		LJUMP 	292H 			//0215 	3A92
		LCALL 	505H 			//0216 	3505
		MOVLP 	0H 			//0217 	0180
		LCALL 	4CCH 			//0218 	34CC
		MOVLP 	0H 			//0219 	0180
		LCALL 	479H 			//021A 	3479
		STR 	25H 			//021B 	10A5
		MOVIW 	1H[1] 			//021C 	0F41
		STR 	26H 			//021D 	10A6
		MOVLP 	BH 			//021E 	018B
		LCALL 	325H 			//021F 	3325
		MOVLP 	0H 			//0220 	0180
		MOVLB 	0H 			//0221 	1020
		LSLF 	3EH, 0H 		//0222 	053E
		ADDWI 	B1H 			//0223 	0EB1
		STR 	6H 			//0224 	1086
		CLRF 	7H 			//0225 	1187
		LDR 	27H, 0H 			//0226 	1827
		MOVWI 	0H[1] 			//0227 	0FC0
		LDR 	28H, 0H 			//0228 	1828
		MOVWI 	1H[1] 			//0229 	0FC1
		INCR 	3EH, 1H 		//022A 	1ABE
		LJUMP 	212H 			//022B 	3A12
		MOVLB 	1H 			//022C 	1021
		CLRF 	4BH 			//022D 	11CB
		LCALL 	5A7H 			//022E 	35A7
		MOVLP 	0H 			//022F 	0180
		BTSC 	3H, 0H 			//0230 	2803
		LJUMP 	263H 			//0231 	3A63
		LCALL 	505H 			//0232 	3505
		MOVLP 	0H 			//0233 	0180
		LCALL 	4CCH 			//0234 	34CC
		MOVLP 	0H 			//0235 	0180
		LCALL 	479H 			//0236 	3479
		STR 	25H 			//0237 	10A5
		MOVIW 	1H[1] 			//0238 	0F41
		STR 	26H 			//0239 	10A6
		MOVLP 	BH 			//023A 	018B
		LCALL 	325H 			//023B 	3325
		MOVLP 	0H 			//023C 	0180
		MOVLB 	0H 			//023D 	1020
		LCALL 	595H 			//023E 	3595
		MOVLP 	0H 			//023F 	0180
		LDR 	27H, 0H 			//0240 	1827
		MOVWI 	0H[1] 			//0241 	0FC0
		LDR 	28H, 0H 			//0242 	1828
		MOVWI 	1H[1] 			//0243 	0FC1
		LSLF 	3EH, 0H 		//0244 	053E
		ADDWI 	5CH 			//0245 	0E5C
		STR 	6H 			//0246 	1086
		MOVIW 	0H[1] 			//0247 	0F40
		STR 	53H 			//0248 	10D3
		MOVIW 	1H[1] 			//0249 	0F41
		STR 	54H 			//024A 	10D4
		LSLF 	3EH, 0H 		//024B 	053E
		ADDWI 	B1H 			//024C 	0EB1
		STR 	6H 			//024D 	1086
		CLRF 	7H 			//024E 	1187
		MOVIW 	0H[1] 			//024F 	0F40
		STR 	55H 			//0250 	10D5
		MOVIW 	1H[1] 			//0251 	0F41
		STR 	56H 			//0252 	10D6
		LDR 	54H, 0H 			//0253 	1854
		SUBWR 	56H, 0H 		//0254 	1256
		BTSS 	3H, 2H 			//0255 	2D03
		LJUMP 	259H 			//0256 	3A59
		LDR 	53H, 0H 			//0257 	1853
		SUBWR 	55H, 0H 		//0258 	1255
		BTSC 	3H, 0H 			//0259 	2803
		LJUMP 	25DH 			//025A 	3A5D
		MOVLB 	1H 			//025B 	1021
		INCR 	4BH, 1H 		//025C 	1ACB
		MOVLB 	0H 			//025D 	1020
		INCR 	3EH, 1H 		//025E 	1ABE
		LDWI 	2H 			//025F 	0002
		SUBWR 	3EH, 0H 		//0260 	123E
		BTSS 	3H, 0H 			//0261 	2C03
		LJUMP 	232H 			//0262 	3A32
		LDWI 	2H 			//0263 	0002
		MOVLB 	1H 			//0264 	1021
		XORWR 	4BH, 0H 		//0265 	164B
		LDWI 	1H 			//0266 	0001
		BTSS 	3H, 2H 			//0267 	2D03
		LJUMP 	287H 			//0268 	3A87
		MOVLB 	0H 			//0269 	1020
		STR 	53H 			//026A 	10D3
		CLRF 	54H 			//026B 	11D4
		INCR 	4BH, 0H 		//026C 	1A4B
		LJUMP 	270H 			//026D 	3A70
		LSLF 	53H, 1H 		//026E 	05D3
		RLR 	54H, 1H 			//026F 	1DD4
		DECRSZ 	9H, 1H 		//0270 	1B89
		LJUMP 	26EH 			//0271 	3A6E
		LDR 	53H, 0H 			//0272 	1853
		STR 	37H 			//0273 	10B7
		LDR 	54H, 0H 			//0274 	1854
		STR 	38H 			//0275 	10B8
		CLRF 	3EH 			//0276 	11BE
		LDWI 	2H 			//0277 	0002
		SUBWR 	3EH, 0H 		//0278 	123E
		BTSC 	3H, 0H 			//0279 	2803
		LJUMP 	292H 			//027A 	3A92
		LCALL 	595H 			//027B 	3595
		MOVLP 	0H 			//027C 	0180
		LSLF 	3EH, 0H 		//027D 	053E
		ADDWI 	B1H 			//027E 	0EB1
		STR 	4H 			//027F 	1084
		CLRF 	5H 			//0280 	1185
		MOVIW 	0H[1] 			//0281 	0F40
		MOVWI 	0H[0] 			//0282 	0F80
		MOVIW 	1H[1] 			//0283 	0F41
		MOVWI 	1H[0] 			//0284 	0F81
		INCR 	3EH, 1H 		//0285 	1ABE
		LJUMP 	277H 			//0286 	3A77
		MOVLB 	0H 			//0287 	1020
		STR 	53H 			//0288 	10D3
		CLRF 	54H 			//0289 	11D4
		INCR 	4BH, 0H 		//028A 	1A4B
		LJUMP 	28EH 			//028B 	3A8E
		LSLF 	53H, 1H 		//028C 	05D3
		RLR 	54H, 1H 			//028D 	1DD4
		DECRSZ 	9H, 1H 		//028E 	1B89
		LJUMP 	28CH 			//028F 	3A8C
		LCALL 	58EH 			//0290 	358E
		MOVLP 	0H 			//0291 	0180
		MOVLB 	0H 			//0292 	1020
		LDR 	37H, 0H 			//0293 	1837
		IORWR 	38H, 0H 		//0294 	1438
		BTSS 	3H, 2H 			//0295 	2D03
		LJUMP 	2AAH 			//0296 	3AAA
		MOVLB 	1H 			//0297 	1021
		CLRF 	49H 			//0298 	11C9
		INCR 	49H, 1H 		//0299 	1AC9
		LCALL 	5A7H 			//029A 	35A7
		MOVLP 	0H 			//029B 	0180
		BTSC 	3H, 0H 			//029C 	2803
		LJUMP 	2AAH 			//029D 	3AAA
		LSLF 	3EH, 0H 		//029E 	053E
		ADDWI 	B1H 			//029F 	0EB1
		STR 	6H 			//02A0 	1086
		LDWI 	0H 			//02A1 	0000
		CLRF 	7H 			//02A2 	1187
		MOVWI 	0H[1] 			//02A3 	0FC0
		MOVWI 	1H[1] 			//02A4 	0FC1
		INCR 	3EH, 1H 		//02A5 	1ABE
		LDWI 	2H 			//02A6 	0002
		SUBWR 	3EH, 0H 		//02A7 	123E
		BTSS 	3H, 0H 			//02A8 	2C03
		LJUMP 	29EH 			//02A9 	3A9E
		LCALL 	57CH 			//02AA 	357C
		MOVLP 	0H 			//02AB 	0180
		LJUMP 	2AFH 			//02AC 	3AAF
		LSRF 	54H, 1H 		//02AD 	06D4
		RRR 	53H, 1H 			//02AE 	1CD3
		DECRSZ 	9H, 1H 		//02AF 	1B89
		LJUMP 	2ADH 			//02B0 	3AAD
		BTSC 	53H, 0H 		//02B1 	2853
		LJUMP 	32CH 			//02B2 	3B2C
		LCALL 	576H 			//02B3 	3576
		MOVLP 	0H 			//02B4 	0180
		MOVIW 	0H[1] 			//02B5 	0F40
		STR 	53H 			//02B6 	10D3
		MOVIW 	1H[1] 			//02B7 	0F41
		STR 	54H 			//02B8 	10D4
		SUBWR 	2CH, 0H 		//02B9 	122C
		BTSS 	3H, 2H 			//02BA 	2D03
		LJUMP 	2BEH 			//02BB 	3ABE
		LDR 	53H, 0H 			//02BC 	1853
		SUBWR 	2BH, 0H 		//02BD 	122B
		BTSS 	3H, 0H 			//02BE 	2C03
		LJUMP 	2C5H 			//02BF 	3AC5
		LCALL 	576H 			//02C0 	3576
		MOVLP 	0H 			//02C1 	0180
		LDR 	1H, 0H 			//02C2 	1801
		SUBWR 	2BH, 0H 		//02C3 	122B
		LJUMP 	2C9H 			//02C4 	3AC9
		LCALL 	576H 			//02C5 	3576
		MOVLP 	0H 			//02C6 	0180
		LDR 	2BH, 0H 			//02C7 	182B
		SUBWR 	1H, 0H 		//02C8 	1201
		STR 	44H 			//02C9 	10C4
		LDWI 	AH 			//02CA 	000A
		SUBWR 	44H, 0H 		//02CB 	1244
		BTSC 	3H, 0H 			//02CC 	2803
		LJUMP 	31AH 			//02CD 	3B1A
		LCALL 	582H 			//02CE 	3582
		MOVLP 	0H 			//02CF 	0180
		MOVIW 	0H[1] 			//02D0 	0F40
		STR 	53H 			//02D1 	10D3
		MOVIW 	1H[1] 			//02D2 	0F41
		STR 	54H 			//02D3 	10D4
		SUBWR 	2EH, 0H 		//02D4 	122E
		BTSS 	3H, 2H 			//02D5 	2D03
		LJUMP 	2D9H 			//02D6 	3AD9
		LDR 	53H, 0H 			//02D7 	1853
		SUBWR 	2DH, 0H 		//02D8 	122D
		BTSS 	3H, 0H 			//02D9 	2C03
		LJUMP 	2E0H 			//02DA 	3AE0
		LCALL 	582H 			//02DB 	3582
		MOVLP 	0H 			//02DC 	0180
		LDR 	1H, 0H 			//02DD 	1801
		SUBWR 	2DH, 0H 		//02DE 	122D
		LJUMP 	2E4H 			//02DF 	3AE4
		LCALL 	582H 			//02E0 	3582
		MOVLP 	0H 			//02E1 	0180
		LDR 	2DH, 0H 			//02E2 	182D
		SUBWR 	1H, 0H 		//02E3 	1201
		STR 	44H 			//02E4 	10C4
		LDWI 	AH 			//02E5 	000A
		SUBWR 	44H, 0H 		//02E6 	1244
		LDR 	4BH, 0H 			//02E7 	184B
		BTSC 	3H, 0H 			//02E8 	2803
		LJUMP 	31BH 			//02E9 	3B1B
		ADDWI 	4DH 			//02EA 	0E4D
		STR 	6H 			//02EB 	1086
		LDWI 	1H 			//02EC 	0001
		STR 	7H 			//02ED 	1087
		INCR 	1H, 1H 			//02EE 	1A81
		LDR 	4BH, 0H 			//02EF 	184B
		ADDWI 	4DH 			//02F0 	0E4D
		STR 	6H 			//02F1 	1086
		LDR 	1H, 0H 			//02F2 	1801
		STR 	53H 			//02F3 	10D3
		CLRF 	54H 			//02F4 	11D4
		LDR 	54H, 0H 			//02F5 	1854
		MOVLB 	1H 			//02F6 	1021
		SUBWR 	68H, 0H 		//02F7 	1268
		BTSS 	3H, 2H 			//02F8 	2D03
		LJUMP 	2FEH 			//02F9 	3AFE
		MOVLB 	0H 			//02FA 	1020
		LDR 	53H, 0H 			//02FB 	1853
		MOVLB 	1H 			//02FC 	1021
		SUBWR 	67H, 0H 		//02FD 	1267
		BTSC 	3H, 0H 			//02FE 	2803
		LJUMP 	31DH 			//02FF 	3B1D
		NOP 					//0300 	1000
		NOP 					//0301 	1000
		LCALL 	5A7H 			//0302 	35A7
		MOVLP 	0H 			//0303 	0180
		BTSC 	3H, 0H 			//0304 	2803
		LJUMP 	31AH 			//0305 	3B1A
		LDR 	3EH, 0H 			//0306 	183E
		LCALL 	4CCH 			//0307 	34CC
		MOVLP 	0H 			//0308 	0180
		STR 	55H 			//0309 	10D5
		LDWI 	23H 			//030A 	0023
		LCALL 	493H 			//030B 	3493
		MOVLP 	0H 			//030C 	0180
		LDR 	3EH, 0H 			//030D 	183E
		STR 	5BH 			//030E 	10DB
		CLRF 	5CH 			//030F 	11DC
		LSLF 	5BH, 1H 		//0310 	05DB
		RLR 	5CH, 1H 			//0311 	1DDC
		LDR 	4BH, 0H 			//0312 	184B
		LCALL 	4AFH 			//0313 	34AF
		MOVLP 	0H 			//0314 	0180
		INCR 	3EH, 1H 		//0315 	1ABE
		LDWI 	2H 			//0316 	0002
		SUBWR 	3EH, 0H 		//0317 	123E
		BTSS 	3H, 0H 			//0318 	2C03
		LJUMP 	306H 			//0319 	3B06
		LDR 	4BH, 0H 			//031A 	184B
		LCALL 	59BH 			//031B 	359B
		MOVLP 	0H 			//031C 	0180
		MOVLB 	0H 			//031D 	1020
		LCALL 	576H 			//031E 	3576
		MOVLP 	0H 			//031F 	0180
		LDR 	2BH, 0H 			//0320 	182B
		MOVWI 	0H[1] 			//0321 	0FC0
		LDR 	2CH, 0H 			//0322 	182C
		MOVWI 	1H[1] 			//0323 	0FC1
		LSLF 	4BH, 0H 		//0324 	054B
		ADDWI 	A0H 			//0325 	0EA0
		STR 	6H 			//0326 	1086
		LDR 	2DH, 0H 			//0327 	182D
		MOVWI 	0H[1] 			//0328 	0FC0
		LDR 	2EH, 0H 			//0329 	182E
		MOVWI 	1H[1] 			//032A 	0FC1
		LJUMP 	339H 			//032B 	3B39
		CLRF 	40H 			//032C 	11C0
		LDWI 	FH 			//032D 	000F
		SUBWR 	40H, 0H 		//032E 	1240
		BTSC 	3H, 0H 			//032F 	2803
		LJUMP 	339H 			//0330 	3B39
		LDR 	40H, 0H 			//0331 	1840
		LCALL 	59BH 			//0332 	359B
		MOVLP 	0H 			//0333 	0180
		INCR 	40H, 1H 		//0334 	1AC0
		LDWI 	FH 			//0335 	000F
		SUBWR 	40H, 0H 		//0336 	1240
		BTSS 	3H, 0H 			//0337 	2C03
		LJUMP 	331H 			//0338 	3B31
		LDR 	3DH, 0H 			//0339 	183D
		BTSC 	3H, 2H 			//033A 	2903
		RET 					//033B 	1008
		LDR 	47H, 0H 			//033C 	1847
		LCALL 	588H 			//033D 	3588
		MOVLP 	0H 			//033E 	0180
		XORWR 	4BH, 0H 		//033F 	164B
		BTSS 	3H, 2H 			//0340 	2D03
		RET 					//0341 	1008
		MOVLB 	1H 			//0342 	1021
		LDR 	68H, 0H 			//0343 	1868
		SUBWR 	6CH, 0H 		//0344 	126C
		BTSS 	3H, 2H 			//0345 	2D03
		LJUMP 	349H 			//0346 	3B49
		LDR 	67H, 0H 			//0347 	1867
		SUBWR 	6BH, 0H 		//0348 	126B
		BTSS 	3H, 0H 			//0349 	2C03
		LJUMP 	3D7H 			//034A 	3BD7
		CLRF 	6BH 			//034B 	11EB
		CLRF 	6CH 			//034C 	11EC
		CLRF 	69H 			//034D 	11E9
		CLRF 	6AH 			//034E 	11EA
		MOVLB 	0H 			//034F 	1020
		CLRF 	40H 			//0350 	11C0
		LDWI 	FH 			//0351 	000F
		SUBWR 	40H, 0H 		//0352 	1240
		BTSC 	3H, 0H 			//0353 	2803
		LJUMP 	3D7H 			//0354 	3BD7
		CLRF 	3EH 			//0355 	11BE
		LDWI 	4H 			//0356 	0004
		SUBWR 	3EH, 0H 		//0357 	123E
		BTSC 	3H, 0H 			//0358 	2803
		LJUMP 	3D2H 			//0359 	3BD2
		LDR 	3EH, 0H 			//035A 	183E
		LCALL 	4A9H 			//035B 	34A9
		MOVLP 	0H 			//035C 	0180
		LCALL 	479H 			//035D 	3479
		MOVLP 	0H 			//035E 	0180
		LCALL 	539H 			//035F 	3539
		MOVLP 	0H 			//0360 	0180
		LCALL 	514H 			//0361 	3514
		MOVLP 	0H 			//0362 	0180
		STR 	66H 			//0363 	10E6
		LDR 	5CH, 0H 			//0364 	185C
		SUBWR 	66H, 0H 		//0365 	1266
		BTSS 	3H, 2H 			//0366 	2D03
		LJUMP 	36AH 			//0367 	3B6A
		LDR 	5BH, 0H 			//0368 	185B
		SUBWR 	65H, 0H 		//0369 	1265
		BTSC 	3H, 0H 			//036A 	2803
		LJUMP 	3A2H 			//036B 	3BA2
		LDR 	3EH, 0H 			//036C 	183E
		LCALL 	4A9H 			//036D 	34A9
		MOVLP 	0H 			//036E 	0180
		LCALL 	479H 			//036F 	3479
		MOVLP 	0H 			//0370 	0180
		STR 	5BH 			//0371 	10DB
		MOVIW 	1H[1] 			//0372 	0F41
		STR 	5CH 			//0373 	10DC
		LDR 	5BH, 0H 			//0374 	185B
		ADDWI 	FFH 			//0375 	0EFF
		STR 	5DH 			//0376 	10DD
		LDWI 	FFH 			//0377 	00FF
		LCALL 	4DFH 			//0378 	34DF
		MOVLP 	0H 			//0379 	0180
		LCALL 	4A9H 			//037A 	34A9
		MOVLP 	0H 			//037B 	0180
		STR 	55H 			//037C 	10D5
		LDWI 	23H 			//037D 	0023
		LCALL 	493H 			//037E 	3493
		MOVLP 	0H 			//037F 	0180
		MOVIW 	0H[1] 			//0380 	0F40
		LCALL 	539H 			//0381 	3539
		MOVLP 	0H 			//0382 	0180
		LSLF 	5DH, 1H 		//0383 	05DD
		RLR 	5EH, 1H 			//0384 	1DDE
		LDR 	40H, 0H 			//0385 	1840
		STR 	5FH 			//0386 	10DF
		LDWI 	78H 			//0387 	0078
		CLRF 	60H 			//0388 	11E0
		LSLF 	5FH, 1H 		//0389 	05DF
		RLR 	60H, 1H 			//038A 	1DE0
		LSLF 	5FH, 1H 		//038B 	05DF
		RLR 	60H, 1H 			//038C 	1DE0
		LSLF 	5FH, 1H 		//038D 	05DF
		RLR 	60H, 1H 			//038E 	1DE0
		STR 	61H 			//038F 	10E1
		LDWI 	23H 			//0390 	0023
		STR 	62H 			//0391 	10E2
		LDR 	5FH, 0H 			//0392 	185F
		ADDWR 	61H, 0H 		//0393 	1761
		STR 	63H 			//0394 	10E3
		LDR 	60H, 0H 			//0395 	1860
		ADDWFC 	62H, 0H 		//0396 	0D62
		STR 	64H 			//0397 	10E4
		LDR 	5DH, 0H 			//0398 	185D
		ADDWR 	63H, 0H 		//0399 	1763
		STR 	6H 			//039A 	1086
		LDR 	5EH, 0H 			//039B 	185E
		ADDWFC 	64H, 0H 		//039C 	0D64
		STR 	7H 			//039D 	1087
		MOVIW 	0H[1] 			//039E 	0F40
		STR 	65H 			//039F 	10E5
		MOVIW 	1H[1] 			//03A0 	0F41
		LJUMP 	3B8H 			//03A1 	3BB8
		LDR 	3EH, 0H 			//03A2 	183E
		LCALL 	4A9H 			//03A3 	34A9
		MOVLP 	0H 			//03A4 	0180
		LCALL 	479H 			//03A5 	3479
		MOVLP 	0H 			//03A6 	0180
		STR 	5BH 			//03A7 	10DB
		MOVIW 	1H[1] 			//03A8 	0F41
		STR 	5CH 			//03A9 	10DC
		LDR 	5BH, 0H 			//03AA 	185B
		ADDWI 	1H 			//03AB 	0E01
		STR 	5DH 			//03AC 	10DD
		LDWI 	0H 			//03AD 	0000
		LCALL 	4DFH 			//03AE 	34DF
		MOVLP 	0H 			//03AF 	0180
		LCALL 	4A9H 			//03B0 	34A9
		MOVLP 	0H 			//03B1 	0180
		LCALL 	479H 			//03B2 	3479
		MOVLP 	0H 			//03B3 	0180
		LCALL 	539H 			//03B4 	3539
		MOVLP 	0H 			//03B5 	0180
		LCALL 	514H 			//03B6 	3514
		MOVLP 	0H 			//03B7 	0180
		STR 	66H 			//03B8 	10E6
		LDR 	5CH, 0H 			//03B9 	185C
		SUBWR 	66H, 0H 		//03BA 	1266
		BTSS 	3H, 2H 			//03BB 	2D03
		LJUMP 	3BFH 			//03BC 	3BBF
		LDR 	5BH, 0H 			//03BD 	185B
		SUBWR 	65H, 0H 		//03BE 	1265
		BTSC 	3H, 0H 			//03BF 	2803
		LJUMP 	3CDH 			//03C0 	3BCD
		LDR 	3EH, 0H 			//03C1 	183E
		LCALL 	4A9H 			//03C2 	34A9
		MOVLP 	0H 			//03C3 	0180
		STR 	55H 			//03C4 	10D5
		LDWI 	23H 			//03C5 	0023
		LCALL 	493H 			//03C6 	3493
		MOVLP 	0H 			//03C7 	0180
		LDR 	3EH, 0H 			//03C8 	183E
		LCALL 	50EH 			//03C9 	350E
		MOVLP 	0H 			//03CA 	0180
		LCALL 	4AFH 			//03CB 	34AF
		MOVLP 	0H 			//03CC 	0180
		INCR 	3EH, 1H 		//03CD 	1ABE
		LDWI 	4H 			//03CE 	0004
		SUBWR 	3EH, 0H 		//03CF 	123E
		BTSS 	3H, 0H 			//03D0 	2C03
		LJUMP 	35AH 			//03D1 	3B5A
		INCR 	40H, 1H 		//03D2 	1AC0
		LDWI 	FH 			//03D3 	000F
		SUBWR 	40H, 0H 		//03D4 	1240
		BTSS 	3H, 0H 			//03D5 	2C03
		LJUMP 	355H 			//03D6 	3B55
		LDWI 	2H 			//03D7 	0002
		MOVLB 	0H 			//03D8 	1020
		XORWR 	3DH, 0H 		//03D9 	163D
		BTSS 	3H, 2H 			//03DA 	2D03
		RET 					//03DB 	1008
		MOVLB 	1H 			//03DC 	1021
		LDR 	47H, 0H 			//03DD 	1847
		BTSS 	3H, 2H 			//03DE 	2D03
		LJUMP 	3E3H 			//03DF 	3BE3
		CLRF 	47H 			//03E0 	11C7
		INCR 	47H, 1H 		//03E1 	1AC7
		RET 					//03E2 	1008
		MOVLB 	0H 			//03E3 	1020
		CLRF 	3DH 			//03E4 	11BD
		INCR 	3DH, 1H 		//03E5 	1ABD
		CLRF 	40H 			//03E6 	11C0
		LDWI 	FH 			//03E7 	000F
		SUBWR 	40H, 0H 		//03E8 	1240
		BTSC 	3H, 0H 			//03E9 	2803
		RET 					//03EA 	1008
		CLRF 	3EH 			//03EB 	11BE
		LDWI 	4H 			//03EC 	0004
		SUBWR 	3EH, 0H 		//03ED 	123E
		BTSC 	3H, 0H 			//03EE 	2803
		LJUMP 	401H 			//03EF 	3C01
		LDR 	3EH, 0H 			//03F0 	183E
		LCALL 	4A9H 			//03F1 	34A9
		MOVLP 	0H 			//03F2 	0180
		STR 	55H 			//03F3 	10D5
		LDWI 	23H 			//03F4 	0023
		LCALL 	493H 			//03F5 	3493
		MOVLP 	0H 			//03F6 	0180
		LDR 	3EH, 0H 			//03F7 	183E
		LCALL 	50EH 			//03F8 	350E
		MOVLP 	0H 			//03F9 	0180
		LCALL 	4AFH 			//03FA 	34AF
		MOVLP 	0H 			//03FB 	0180
		INCR 	3EH, 1H 		//03FC 	1ABE
		LDWI 	4H 			//03FD 	0004
		SUBWR 	3EH, 0H 		//03FE 	123E
		BTSS 	3H, 0H 			//03FF 	2C03
		LJUMP 	3F0H 			//0400 	3BF0
		INCR 	40H, 1H 		//0401 	1AC0
		LJUMP 	3E7H 			//0402 	3BE7
		CLRF 	50H 			//0403 	11D0
		CLRF 	51H 			//0404 	11D1
		BTSS 	4CH, 0H 		//0405 	2C4C
		LJUMP 	40BH 			//0406 	3C0B
		LDR 	4EH, 0H 			//0407 	184E
		ADDWR 	50H, 1H 		//0408 	17D0
		LDR 	4FH, 0H 			//0409 	184F
		ADDWFC 	51H, 1H 		//040A 	0DD1
		LSLF 	4EH, 1H 		//040B 	05CE
		RLR 	4FH, 1H 			//040C 	1DCF
		LSRF 	4DH, 1H 		//040D 	06CD
		RRR 	4CH, 1H 			//040E 	1CCC
		LDR 	4CH, 0H 			//040F 	184C
		IORWR 	4DH, 0H 		//0410 	144D
		BTSS 	3H, 2H 			//0411 	2D03
		LJUMP 	405H 			//0412 	3C05
		LDR 	51H, 0H 			//0413 	1851
		STR 	4DH 			//0414 	10CD
		LDR 	50H, 0H 			//0415 	1850
		STR 	4CH 			//0416 	10CC
		RET 					//0417 	1008
		STR 	52H 			//0418 	10D2
		STR 	4CH 			//0419 	10CC
		LDWI 	7EH 			//041A 	007E
		LCALL 	4D2H 			//041B 	34D2
		MOVLP 	0H 			//041C 	0180
		LCALL 	56DH 			//041D 	356D
		MOVLP 	0H 			//041E 	0180
		LDR 	32H, 0H 			//041F 	1832
		SUBWR 	51H, 0H 		//0420 	1251
		BTSS 	3H, 2H 			//0421 	2D03
		LJUMP 	425H 			//0422 	3C25
		LDR 	31H, 0H 			//0423 	1831
		SUBWR 	50H, 0H 		//0424 	1250
		BTSS 	3H, 0H 			//0425 	2C03
		LJUMP 	445H 			//0426 	3C45
		LDWI 	31H 			//0427 	0031
		STR 	6H 			//0428 	1086
		CLRF 	7H 			//0429 	1187
		LDR 	52H, 0H 			//042A 	1852
		STR 	4CH 			//042B 	10CC
		LDWI 	7EH 			//042C 	007E
		LCALL 	4D2H 			//042D 	34D2
		MOVLP 	0H 			//042E 	0180
		LCALL 	550H 			//042F 	3550
		MOVLP 	0H 			//0430 	0180
		BTSC 	3H, 2H 			//0431 	2903
		SUBWR 	50H, 0H 		//0432 	1250
		BTSC 	3H, 0H 			//0433 	2803
		LJUMP 	441H 			//0434 	3C41
		LDR 	52H, 0H 			//0435 	1852
		STR 	4CH 			//0436 	10CC
		LDWI 	7EH 			//0437 	007E
		LCALL 	4D2H 			//0438 	34D2
		MOVLP 	0H 			//0439 	0180
		STR 	6H 			//043A 	1086
		LDR 	4DH, 0H 			//043B 	184D
		ADDWFC 	4FH, 0H 		//043C 	0D4F
		STR 	7H 			//043D 	1087
		LDR 	31H, 0H 			//043E 	1831
		SUBWR 	1H, 0H 		//043F 	1201
		LJUMP 	442H 			//0440 	3C42
		LDWI 	FFH 			//0441 	00FF
		MOVLB 	1H 			//0442 	1021
		STR 	38H 			//0443 	10B8
		LJUMP 	447H 			//0444 	3C47
		MOVLB 	1H 			//0445 	1021
		CLRF 	38H 			//0446 	11B8
		MOVLB 	0H 			//0447 	1020
		LDR 	52H, 0H 			//0448 	1852
		STR 	4CH 			//0449 	10CC
		LDWI 	7CH 			//044A 	007C
		LCALL 	4D2H 			//044B 	34D2
		MOVLP 	0H 			//044C 	0180
		LCALL 	56DH 			//044D 	356D
		MOVLP 	0H 			//044E 	0180
		LDR 	30H, 0H 			//044F 	1830
		SUBWR 	51H, 0H 		//0450 	1251
		BTSS 	3H, 2H 			//0451 	2D03
		LJUMP 	455H 			//0452 	3C55
		LDR 	2FH, 0H 			//0453 	182F
		SUBWR 	50H, 0H 		//0454 	1250
		BTSS 	3H, 0H 			//0455 	2C03
		LJUMP 	476H 			//0456 	3C76
		LDWI 	2FH 			//0457 	002F
		STR 	6H 			//0458 	1086
		CLRF 	7H 			//0459 	1187
		LDR 	52H, 0H 			//045A 	1852
		STR 	4CH 			//045B 	10CC
		LDWI 	7CH 			//045C 	007C
		LCALL 	4D2H 			//045D 	34D2
		MOVLP 	0H 			//045E 	0180
		LCALL 	550H 			//045F 	3550
		MOVLP 	0H 			//0460 	0180
		BTSC 	3H, 2H 			//0461 	2903
		SUBWR 	50H, 0H 		//0462 	1250
		BTSC 	3H, 0H 			//0463 	2803
		LJUMP 	472H 			//0464 	3C72
		LDR 	52H, 0H 			//0465 	1852
		STR 	4CH 			//0466 	10CC
		LDWI 	7CH 			//0467 	007C
		LCALL 	4D2H 			//0468 	34D2
		STR 	6H 			//0469 	1086
		LDR 	4DH, 0H 			//046A 	184D
		ADDWFC 	4FH, 0H 		//046B 	0D4F
		STR 	7H 			//046C 	1087
		LDR 	2FH, 0H 			//046D 	182F
		SUBWR 	1H, 0H 		//046E 	1201
		MOVLB 	1H 			//046F 	1021
		STR 	37H 			//0470 	10B7
		RET 					//0471 	1008
		LDWI 	FFH 			//0472 	00FF
		MOVLB 	1H 			//0473 	1021
		STR 	37H 			//0474 	10B7
		RET 					//0475 	1008
		MOVLB 	1H 			//0476 	1021
		CLRF 	37H 			//0477 	11B7
		RET 					//0478 	1008
		STR 	55H 			//0479 	10D5
		LDWI 	78H 			//047A 	0078
		CLRF 	56H 			//047B 	11D6
		LSLF 	55H, 1H 		//047C 	05D5
		RLR 	56H, 1H 			//047D 	1DD6
		LSLF 	55H, 1H 		//047E 	05D5
		RLR 	56H, 1H 			//047F 	1DD6
		LSLF 	55H, 1H 		//0480 	05D5
		RLR 	56H, 1H 			//0481 	1DD6
		STR 	57H 			//0482 	10D7
		LDWI 	23H 			//0483 	0023
		STR 	58H 			//0484 	10D8
		LDR 	55H, 0H 			//0485 	1855
		ADDWR 	57H, 0H 		//0486 	1757
		STR 	59H 			//0487 	10D9
		LDR 	56H, 0H 			//0488 	1856
		ADDWFC 	58H, 0H 		//0489 	0D58
		STR 	5AH 			//048A 	10DA
		LDR 	53H, 0H 			//048B 	1853
		ADDWR 	59H, 0H 		//048C 	1759
		STR 	6H 			//048D 	1086
		LDR 	54H, 0H 			//048E 	1854
		ADDWFC 	5AH, 0H 		//048F 	0D5A
		STR 	7H 			//0490 	1087
		MOVIW 	0H[1] 			//0491 	0F40
		RET 					//0492 	1008
		CLRF 	56H 			//0493 	11D6
		LSLF 	55H, 1H 		//0494 	05D5
		RLR 	56H, 1H 			//0495 	1DD6
		LSLF 	55H, 1H 		//0496 	05D5
		RLR 	56H, 1H 			//0497 	1DD6
		LSLF 	55H, 1H 		//0498 	05D5
		RLR 	56H, 1H 			//0499 	1DD6
		CLRF 	57H 			//049A 	11D7
		STR 	58H 			//049B 	10D8
		LDR 	55H, 0H 			//049C 	1855
		ADDWR 	57H, 0H 		//049D 	1757
		STR 	59H 			//049E 	10D9
		LDR 	56H, 0H 			//049F 	1856
		ADDWFC 	58H, 0H 		//04A0 	0D58
		STR 	5AH 			//04A1 	10DA
		LDR 	53H, 0H 			//04A2 	1853
		ADDWR 	59H, 0H 		//04A3 	1759
		STR 	6H 			//04A4 	1086
		LDR 	54H, 0H 			//04A5 	1854
		ADDWFC 	5AH, 0H 		//04A6 	0D5A
		STR 	7H 			//04A7 	1087
		RET 					//04A8 	1008
		STR 	53H 			//04A9 	10D3
		CLRF 	54H 			//04AA 	11D4
		LSLF 	53H, 1H 		//04AB 	05D3
		RLR 	54H, 1H 			//04AC 	1DD4
		LDR 	40H, 0H 			//04AD 	1840
		RET 					//04AE 	1008
		STR 	5DH 			//04AF 	10DD
		LDWI 	78H 			//04B0 	0078
		CLRF 	5EH 			//04B1 	11DE
		LSLF 	5DH, 1H 		//04B2 	05DD
		RLR 	5EH, 1H 			//04B3 	1DDE
		LSLF 	5DH, 1H 		//04B4 	05DD
		RLR 	5EH, 1H 			//04B5 	1DDE
		LSLF 	5DH, 1H 		//04B6 	05DD
		RLR 	5EH, 1H 			//04B7 	1DDE
		STR 	5FH 			//04B8 	10DF
		LDWI 	23H 			//04B9 	0023
		STR 	60H 			//04BA 	10E0
		LDR 	5DH, 0H 			//04BB 	185D
		ADDWR 	5FH, 0H 		//04BC 	175F
		STR 	61H 			//04BD 	10E1
		LDR 	5EH, 0H 			//04BE 	185E
		ADDWFC 	60H, 0H 		//04BF 	0D60
		STR 	62H 			//04C0 	10E2
		LDR 	5BH, 0H 			//04C1 	185B
		ADDWR 	61H, 0H 		//04C2 	1761
		STR 	4H 			//04C3 	1084
		LDR 	5CH, 0H 			//04C4 	185C
		ADDWFC 	62H, 0H 		//04C5 	0D62
		STR 	5H 			//04C6 	1085
		MOVIW 	0H[1] 			//04C7 	0F40
		MOVWI 	0H[0] 			//04C8 	0F80
		MOVIW 	1H[1] 			//04C9 	0F41
		MOVWI 	1H[0] 			//04CA 	0F81
		RET 					//04CB 	1008
		STR 	53H 			//04CC 	10D3
		CLRF 	54H 			//04CD 	11D4
		LSLF 	53H, 1H 		//04CE 	05D3
		RLR 	54H, 1H 			//04CF 	1DD4
		LDR 	4BH, 0H 			//04D0 	184B
		RET 					//04D1 	1008
		CLRF 	4DH 			//04D2 	11CD
		LSLF 	4CH, 1H 		//04D3 	05CC
		RLR 	4DH, 1H 			//04D4 	1DCD
		LSLF 	4CH, 1H 		//04D5 	05CC
		RLR 	4DH, 1H 			//04D6 	1DCD
		LSLF 	4CH, 1H 		//04D7 	05CC
		RLR 	4DH, 1H 			//04D8 	1DCD
		STR 	4EH 			//04D9 	10CE
		LDWI 	23H 			//04DA 	0023
		STR 	4FH 			//04DB 	10CF
		LDR 	4CH, 0H 			//04DC 	184C
		ADDWR 	4EH, 0H 		//04DD 	174E
		RET 					//04DE 	1008
		ADDWFC 	5CH, 0H 		//04DF 	0D5C
		STR 	5EH 			//04E0 	10DE
		LDR 	3EH, 0H 			//04E1 	183E
		STR 	5FH 			//04E2 	10DF
		CLRF 	60H 			//04E3 	11E0
		LSLF 	5FH, 1H 		//04E4 	05DF
		RLR 	60H, 1H 			//04E5 	1DE0
		LDR 	40H, 0H 			//04E6 	1840
		STR 	61H 			//04E7 	10E1
		LDWI 	78H 			//04E8 	0078
		CLRF 	62H 			//04E9 	11E2
		LSLF 	61H, 1H 		//04EA 	05E1
		RLR 	62H, 1H 			//04EB 	1DE2
		LSLF 	61H, 1H 		//04EC 	05E1
		RLR 	62H, 1H 			//04ED 	1DE2
		LSLF 	61H, 1H 		//04EE 	05E1
		RLR 	62H, 1H 			//04EF 	1DE2
		STR 	63H 			//04F0 	10E3
		LDWI 	23H 			//04F1 	0023
		STR 	64H 			//04F2 	10E4
		LDR 	61H, 0H 			//04F3 	1861
		ADDWR 	63H, 0H 		//04F4 	1763
		STR 	65H 			//04F5 	10E5
		LDR 	62H, 0H 			//04F6 	1862
		ADDWFC 	64H, 0H 		//04F7 	0D64
		STR 	66H 			//04F8 	10E6
		LDR 	5FH, 0H 			//04F9 	185F
		ADDWR 	65H, 0H 		//04FA 	1765
		STR 	6H 			//04FB 	1086
		LDR 	60H, 0H 			//04FC 	1860
		ADDWFC 	66H, 0H 		//04FD 	0D66
		STR 	7H 			//04FE 	1087
		LDR 	5DH, 0H 			//04FF 	185D
		MOVWI 	0H[1] 			//0500 	0FC0
		LDR 	5EH, 0H 			//0501 	185E
		MOVWI 	1H[1] 			//0502 	0FC1
		LDR 	3EH, 0H 			//0503 	183E
		RET 					//0504 	1008
		LDR 	3EH, 0H 			//0505 	183E
		ADDWI 	B5H 			//0506 	0EB5
		STR 	6H 			//0507 	1086
		CLRF 	7H 			//0508 	1187
		LDR 	1H, 0H 			//0509 	1801
		STR 	21H 			//050A 	10A1
		CLRF 	22H 			//050B 	11A2
		LDR 	3EH, 0H 			//050C 	183E
		RET 					//050D 	1008
		STR 	5BH 			//050E 	10DB
		CLRF 	5CH 			//050F 	11DC
		LSLF 	5BH, 1H 		//0510 	05DB
		RLR 	5CH, 1H 			//0511 	1DDC
		LDR 	40H, 0H 			//0512 	1840
		RET 					//0513 	1008
		LSLF 	5DH, 1H 		//0514 	05DD
		RLR 	5EH, 1H 			//0515 	1DDE
		LDR 	40H, 0H 			//0516 	1840
		STR 	5FH 			//0517 	10DF
		LDWI 	23H 			//0518 	0023
		CLRF 	60H 			//0519 	11E0
		LSLF 	5FH, 1H 		//051A 	05DF
		RLR 	60H, 1H 			//051B 	1DE0
		LSLF 	5FH, 1H 		//051C 	05DF
		RLR 	60H, 1H 			//051D 	1DE0
		LSLF 	5FH, 1H 		//051E 	05DF
		RLR 	60H, 1H 			//051F 	1DE0
		CLRF 	61H 			//0520 	11E1
		STR 	62H 			//0521 	10E2
		LDR 	5FH, 0H 			//0522 	185F
		ADDWR 	61H, 0H 		//0523 	1761
		STR 	63H 			//0524 	10E3
		LDR 	60H, 0H 			//0525 	1860
		ADDWFC 	62H, 0H 		//0526 	0D62
		STR 	64H 			//0527 	10E4
		LDR 	5DH, 0H 			//0528 	185D
		ADDWR 	63H, 0H 		//0529 	1763
		STR 	6H 			//052A 	1086
		LDR 	5EH, 0H 			//052B 	185E
		ADDWFC 	64H, 0H 		//052C 	0D64
		STR 	7H 			//052D 	1087
		MOVIW 	0H[1] 			//052E 	0F40
		STR 	65H 			//052F 	10E5
		MOVIW 	1H[1] 			//0530 	0F41
		RET 					//0531 	1008
		LDR 	4BH, 0H 			//0532 	184B
		STR 	4CH 			//0533 	10CC
		LDWI 	6H 			//0534 	0006
		CLRF 	4DH 			//0535 	11CD
		STR 	4EH 			//0536 	10CE
		CLRF 	4FH 			//0537 	11CF
		RET 					//0538 	1008
		STR 	5BH 			//0539 	10DB
		MOVIW 	1H[1] 			//053A 	0F41
		STR 	5CH 			//053B 	10DC
		LDR 	3EH, 0H 			//053C 	183E
		STR 	5DH 			//053D 	10DD
		CLRF 	5EH 			//053E 	11DE
		RET 					//053F 	1008
		STR 	54H 			//0540 	10D4
		LDR 	4CH, 0H 			//0541 	184C
		ADDWR 	53H, 0H 		//0542 	1753
		STR 	55H 			//0543 	10D5
		LDR 	4DH, 0H 			//0544 	184D
		ADDWFC 	54H, 0H 		//0545 	0D54
		STR 	56H 			//0546 	10D6
		LDR 	3EH, 0H 			//0547 	183E
		ADDWR 	55H, 0H 		//0548 	1755
		STR 	4H 			//0549 	1084
		LDR 	56H, 0H 			//054A 	1856
		BTSC 	3H, 0H 			//054B 	2803
		INCR 	56H, 0H 		//054C 	1A56
		STR 	5H 			//054D 	1085
		LDR 	0H, 0H 			//054E 	1800
		RET 					//054F 	1008
		STR 	4H 			//0550 	1084
		LDR 	4DH, 0H 			//0551 	184D
		ADDWFC 	4FH, 0H 		//0552 	0D4F
		STR 	5H 			//0553 	1085
		MOVIW 	0H[1] 			//0554 	0F40
		SUBWR 	0H, 0H 		//0555 	1200
		STR 	50H 			//0556 	10D0
		ADDFSR 	0H, 1H 		//0557 	0101
		MOVIW 	1H[1] 			//0558 	0F41
		SUBWFB 	0H, 0H 		//0559 	0B00
		STR 	51H 			//055A 	10D1
		LDWI 	0H 			//055B 	0000
		SUBWR 	51H, 0H 		//055C 	1251
		RETW 	FFH 			//055D 	04FF
		LDR 	3EH, 0H 			//055E 	183E
		ADDWI 	B5H 			//055F 	0EB5
		STR 	6H 			//0560 	1086
		CLRF 	7H 			//0561 	1187
		RET 					//0562 	1008
		ADDWFC 	5H, 1H 		//0563 	0D85
		LDR 	0H, 0H 			//0564 	1800
		STR 	53H 			//0565 	10D3
		CLRF 	54H 			//0566 	11D4
		LSLF 	53H, 1H 		//0567 	05D3
		RLR 	54H, 1H 			//0568 	1DD4
		LDR 	54H, 0H 			//0569 	1854
		XORWI 	80H 			//056A 	0A80
		SUBWI 	80H 			//056B 	0C80
		RET 					//056C 	1008
		STR 	6H 			//056D 	1086
		LDR 	4DH, 0H 			//056E 	184D
		ADDWFC 	4FH, 0H 		//056F 	0D4F
		STR 	7H 			//0570 	1087
		MOVIW 	0H[1] 			//0571 	0F40
		STR 	50H 			//0572 	10D0
		MOVIW 	1H[1] 			//0573 	0F41
		STR 	51H 			//0574 	10D1
		RET 					//0575 	1008
		LSLF 	4BH, 0H 		//0576 	054B
		ADDWI 	20H 			//0577 	0E20
		STR 	6H 			//0578 	1086
		LDWI 	1H 			//0579 	0001
		STR 	7H 			//057A 	1087
		RET 					//057B 	1008
		LDR 	38H, 0H 			//057C 	1838
		STR 	54H 			//057D 	10D4
		LDR 	37H, 0H 			//057E 	1837
		STR 	53H 			//057F 	10D3
		INCR 	4BH, 0H 		//0580 	1A4B
		RET 					//0581 	1008
		LSLF 	4BH, 0H 		//0582 	054B
		ADDWI 	A0H 			//0583 	0EA0
		STR 	6H 			//0584 	1086
		LDWI 	1H 			//0585 	0001
		STR 	7H 			//0586 	1087
		RET 					//0587 	1008
		ADDWI 	3EH 			//0588 	0E3E
		STR 	6H 			//0589 	1086
		LDWI 	1H 			//058A 	0001
		STR 	7H 			//058B 	1087
		LDR 	1H, 0H 			//058C 	1801
		RET 					//058D 	1008
		COMR 	53H, 1H 		//058E 	19D3
		COMR 	54H, 1H 		//058F 	19D4
		LDR 	53H, 0H 			//0590 	1853
		ANDWR 	37H, 1H 		//0591 	15B7
		LDR 	54H, 0H 			//0592 	1854
		ANDWR 	38H, 1H 		//0593 	15B8
		RET 					//0594 	1008
		LSLF 	3EH, 0H 		//0595 	053E
		ADDWI 	5CH 			//0596 	0E5C
		STR 	6H 			//0597 	1086
		LDWI 	1H 			//0598 	0001
		STR 	7H 			//0599 	1087
		RET 					//059A 	1008
		ADDWI 	4DH 			//059B 	0E4D
		STR 	6H 			//059C 	1086
		LDWI 	1H 			//059D 	0001
		STR 	7H 			//059E 	1087
		CLRF 	1H 			//059F 	1181
		RET 					//05A0 	1008
		LDR 	38H, 0H 			//05A1 	1838
		STR 	34H 			//05A2 	10B4
		LDR 	37H, 0H 			//05A3 	1837
		STR 	33H 			//05A4 	10B3
		MOVLB 	1H 			//05A5 	1021
		RET 					//05A6 	1008
		MOVLB 	0H 			//05A7 	1020
		CLRF 	3EH 			//05A8 	11BE
		LDWI 	2H 			//05A9 	0002
		SUBWR 	3EH, 0H 		//05AA 	123E
		RET 					//05AB 	1008

		//;rx.c: 56: switch(RFs.RFsp)
		LJUMP 	610H 			//05AC 	3E10

		//;rx.c: 57: {
		//;rx.c: 58: case synH:
		//;rx.c: 59: if(PB4)
		MOVLB 	0H 			//05AD 	1020
		BTSS 	DH, 4H 			//05AE 	2E0D
		LJUMP 	5B4H 			//05AF 	3DB4

		//;rx.c: 60: {
		//;rx.c: 61: RFs.tRFH++;
		MOVLB 	1H 			//05B0 	1021
		INCR 	27H, 1H 		//05B1 	1AA7

		//;rx.c: 62: if(RFs.tRFH>6)
		LDWI 	7H 			//05B2 	0007

		//;rx.c: 63: {
		//;rx.c: 64: RFs.RFsp=rfReset;
		//;rx.c: 65: break;
		LJUMP 	620H 			//05B3 	3E20

		//;rx.c: 75: }
		//;rx.c: 76: RFs.tRFL=0;
		MOVLB 	1H 			//05B4 	1021
		CLRF 	28H 			//05B5 	11A8

		//;rx.c: 77: RFs.RFsp=synL;
		LDWI 	3H 			//05B6 	0003
		STR 	26H 			//05B7 	10A6

		//;rx.c: 78: break;
		RET 					//05B8 	1008

		//;rx.c: 79: }
		//;rx.c: 80: case synL:
		//;rx.c: 81: if(PB4==0)
		MOVLB 	0H 			//05B9 	1020
		BTSC 	DH, 4H 			//05BA 	2A0D
		LJUMP 	5C0H 			//05BB 	3DC0

		//;rx.c: 82: {
		//;rx.c: 83: RFs.tRFL++;
		MOVLB 	1H 			//05BC 	1021
		INCR 	28H, 1H 		//05BD 	1AA8

		//;rx.c: 84: if(RFs.tRFL>135)
		LDWI 	88H 			//05BE 	0088

		//;rx.c: 85: {
		//;rx.c: 86: RFs.RFsp=rfReset;
		//;rx.c: 87: break;
		LJUMP 	61CH 			//05BF 	3E1C

		//;rx.c: 91: else
		//;rx.c: 92: {
		//;rx.c: 93: if(RFs.tRFL<105)
		LDWI 	69H 			//05C0 	0069
		MOVLB 	1H 			//05C1 	1021
		SUBWR 	28H, 0H 		//05C2 	1228
		BTSC 	3H, 0H 			//05C3 	2803
		LJUMP 	5C7H 			//05C4 	3DC7

		//;rx.c: 94: {
		//;rx.c: 95: RFs.RFsp=rfReset;
		CLRF 	26H 			//05C5 	11A6

		//;rx.c: 96: break;
		RET 					//05C6 	1008

		//;rx.c: 97: }
		//;rx.c: 98: RFs.tRFH=0;
		CLRF 	27H 			//05C7 	11A7

		//;rx.c: 99: RFs.RFsp=rfH;
		LDWI 	4H 			//05C8 	0004
		STR 	26H 			//05C9 	10A6

		//;rx.c: 100: break;
		RET 					//05CA 	1008

		//;rx.c: 101: }
		//;rx.c: 102: case rfH:
		//;rx.c: 103: if(PB4)
		MOVLB 	0H 			//05CB 	1020
		BTSS 	DH, 4H 			//05CC 	2E0D
		LJUMP 	5D2H 			//05CD 	3DD2

		//;rx.c: 104: {
		//;rx.c: 105: RFs.tRFH++;
		MOVLB 	1H 			//05CE 	1021
		INCR 	27H, 1H 		//05CF 	1AA7

		//;rx.c: 106: if(RFs.tRFH>16)
		LDWI 	11H 			//05D0 	0011

		//;rx.c: 107: {
		//;rx.c: 108: RFs.RFsp=rfReset;
		//;rx.c: 109: break;
		LJUMP 	620H 			//05D1 	3E20

		//;rx.c: 113: else
		//;rx.c: 114: {
		//;rx.c: 115: RFs.tRFL=0;
		MOVLB 	1H 			//05D2 	1021
		CLRF 	28H 			//05D3 	11A8

		//;rx.c: 116: RFs.RFsp=rfL;
		LDWI 	5H 			//05D4 	0005
		STR 	26H 			//05D5 	10A6

		//;rx.c: 117: break;
		RET 					//05D6 	1008

		//;rx.c: 118: }
		//;rx.c: 119: case rfL:
		//;rx.c: 120: if(PB4==0)
		MOVLB 	0H 			//05D7 	1020
		BTSC 	DH, 4H 			//05D8 	2A0D
		LJUMP 	5DEH 			//05D9 	3DDE

		//;rx.c: 121: {
		//;rx.c: 122: RFs.tRFL++;
		MOVLB 	1H 			//05DA 	1021
		INCR 	28H, 1H 		//05DB 	1AA8

		//;rx.c: 123: if(RFs.tRFL>16)
		LDWI 	11H 			//05DC 	0011

		//;rx.c: 124: {
		//;rx.c: 125: RFs.RFsp=rfReset;
		//;rx.c: 126: break;
		LJUMP 	61CH 			//05DD 	3E1C

		//;rx.c: 130: else
		//;rx.c: 131: {
		//;rx.c: 132: buf_rec[RFs.Bptr]<<=1;
		MOVLB 	1H 			//05DE 	1021
		LDR 	2AH, 0H 			//05DF 	182A
		ADDWI 	ACH 			//05E0 	0EAC
		STR 	6H 			//05E1 	1086
		CLRF 	7H 			//05E2 	1187
		LSLF 	1H, 1H 			//05E3 	0581

		//;rx.c: 133: if(RFs.tRFH>=RFs.tRFL)
		LDR 	28H, 0H 			//05E4 	1828
		SUBWR 	27H, 0H 		//05E5 	1227
		BTSS 	3H, 0H 			//05E6 	2C03
		LJUMP 	5ECH 			//05E7 	3DEC

		//;rx.c: 134: {
		//;rx.c: 135: buf_rec[RFs.Bptr]|=1;
		LDR 	2AH, 0H 			//05E8 	182A
		ADDWI 	ACH 			//05E9 	0EAC
		STR 	6H 			//05EA 	1086
		BSR 	1H, 0H 			//05EB 	2401

		//;rx.c: 136: }
		//;rx.c: 137: RFs.tRFH=0;
		CLRF 	27H 			//05EC 	11A7

		//;rx.c: 138: RFs.tRFL=0;
		CLRF 	28H 			//05ED 	11A8

		//;rx.c: 139: if (( ++RFs.BitCount & 7) == 0)
		LDWI 	A9H 			//05EE 	00A9
		STR 	6H 			//05EF 	1086
		CLRF 	7H 			//05F0 	1187
		MOVIW 	0H[1] 			//05F1 	0F40
		ADDWI 	1H 			//05F2 	0E01
		MOVWI 	0H[1] 			//05F3 	0FC0
		ANDWI 	7H 			//05F4 	0907
		BTSC 	3H, 2H 			//05F5 	2903

		//;rx.c: 140: {
		//;rx.c: 141: RFs.Bptr++;
		INCR 	2AH, 1H 		//05F6 	1AAA

		//;rx.c: 142: }
		//;rx.c: 143: if(RFs.BitCount==24)
		LDWI 	18H 			//05F7 	0018
		XORWR 	29H, 0H 		//05F8 	1629
		BTSS 	3H, 2H 			//05F9 	2D03
		LJUMP 	5FFH 			//05FA 	3DFF

		//;rx.c: 144: {
		//;rx.c: 145: RFs.RFsp=rfReset;
		CLRF 	26H 			//05FB 	11A6

		//;rx.c: 146: RFs.RecBufFull=1;
		LDWI 	1H 			//05FC 	0001
		STR 	2BH 			//05FD 	10AB

		//;rx.c: 147: break;
		RET 					//05FE 	1008

		//;rx.c: 148: }
		//;rx.c: 149: RFs.RFsp=rfH;
		LDWI 	4H 			//05FF 	0004
		STR 	26H 			//0600 	10A6

		//;rx.c: 150: break;
		RET 					//0601 	1008

		//;rx.c: 151: }
		//;rx.c: 152: case rffall:
		//;rx.c: 153: if(PB4==0)
		MOVLB 	0H 			//0602 	1020
		BTSS 	DH, 4H 			//0603 	2E0D
		RET 					//0604 	1008

		//;rx.c: 155: else
		//;rx.c: 156: {
		//;rx.c: 157: RFs.RFsp=synH;
		LDWI 	2H 			//0605 	0002
		MOVLB 	1H 			//0606 	1021
		STR 	26H 			//0607 	10A6

		//;rx.c: 158: break;
		RET 					//0608 	1008

		//;rx.c: 162: RFs.tRFL= 0;
		CLRF 	28H 			//0609 	11A8

		//;rx.c: 163: RFs.tRFH= 0;
		CLRF 	27H 			//060A 	11A7

		//;rx.c: 164: RFs.BitCount=0;
		CLRF 	29H 			//060B 	11A9

		//;rx.c: 165: RFs.Bptr=0;
		CLRF 	2AH 			//060C 	11AA

		//;rx.c: 166: RFs.RFsp= rffall;
		LDWI 	1H 			//060D 	0001
		STR 	26H 			//060E 	10A6

		//;rx.c: 167: }
		RET 					//060F 	1008
		MOVLB 	1H 			//0610 	1021
		LDR 	26H, 0H 			//0611 	1826
		STR 	4H 			//0612 	1084
		LDWI 	6H 			//0613 	0006
		SUBWR 	4H, 0H 		//0614 	1204
		BTSC 	3H, 0H 			//0615 	2803
		LJUMP 	609H 			//0616 	3E09
		MOVLP 	6H 			//0617 	0186
		LSLF 	4H, 0H 			//0618 	0504
		ADDWI 	B0H 			//0619 	0EB0
		STR 	2H 			//061A 	1082
		RET 					//061B 	1008
		SUBWR 	28H, 0H 		//061C 	1228
		BTSC 	3H, 0H 			//061D 	2803
		CLRF 	26H 			//061E 	11A6
		RET 					//061F 	1008
		SUBWR 	27H, 0H 		//0620 	1227
		BTSC 	3H, 0H 			//0621 	2803
		CLRF 	26H 			//0622 	11A6
		RET 					//0623 	1008

		//;CHIPINIT.C: 8: TimeUs++;
		MOVLB 	1H 			//0624 	1021
		INCR 	5FH, 1H 		//0625 	1ADF

		//;CHIPINIT.C: 11: if(TimeUs<20)return;
		LDWI 	14H 			//0626 	0014
		SUBWR 	5FH, 0H 		//0627 	125F
		BTSS 	3H, 0H 			//0628 	2C03
		RET 					//0629 	1008

		//;CHIPINIT.C: 12: TimeUs=0;Time2Ms++;
		CLRF 	5FH 			//062A 	11DF
		INCR 	5EH, 1H 		//062B 	1ADE

		//;CHIPINIT.C: 13: SysFunFlg0.OneBits.b1=1;
		MOVLB 	0H 			//062C 	1020
		BSR 	49H, 1H 			//062D 	24C9

		//;CHIPINIT.C: 16: if(Time2Ms<5)return;
		LDWI 	5H 			//062E 	0005
		MOVLB 	1H 			//062F 	1021
		SUBWR 	5EH, 0H 		//0630 	125E
		BTSS 	3H, 0H 			//0631 	2C03
		RET 					//0632 	1008

		//;CHIPINIT.C: 17: Time2Ms=0;Time10Ms++;
		CLRF 	5EH 			//0633 	11DE
		INCR 	5DH, 1H 		//0634 	1ADD

		//;CHIPINIT.C: 19: SysFunFlg0.OneBits.b0=1;
		MOVLB 	0H 			//0635 	1020
		BSR 	49H, 0H 			//0636 	2449

		//;CHIPINIT.C: 22: if(Time10Ms<10)return;
		LDWI 	AH 			//0637 	000A
		MOVLB 	1H 			//0638 	1021
		SUBWR 	5DH, 0H 		//0639 	125D
		BTSS 	3H, 0H 			//063A 	2C03
		RET 					//063B 	1008

		//;CHIPINIT.C: 23: Time10Ms=0;Time100Ms++;
		CLRF 	5DH 			//063C 	11DD
		INCR 	5CH, 1H 		//063D 	1ADC

		//;CHIPINIT.C: 28: if(Time100Ms<10)return;
		LDWI 	AH 			//063E 	000A
		SUBWR 	5CH, 0H 		//063F 	125C
		BTSS 	3H, 0H 			//0640 	2C03
		RET 					//0641 	1008

		//;CHIPINIT.C: 29: Time100Ms=0;TimeSec++;
		CLRF 	5CH 			//0642 	11DC
		INCR 	5BH, 1H 		//0643 	1ADB

		//;CHIPINIT.C: 34: if(TimeSec<60)return;
		LDWI 	3CH 			//0644 	003C
		SUBWR 	5BH, 0H 		//0645 	125B
		BTSS 	3H, 0H 			//0646 	2C03
		RET 					//0647 	1008

		//;CHIPINIT.C: 35: TimeSec=0;
		CLRF 	5BH 			//0648 	11DB

		//;CHIPINIT.C: 37: if((SysFunFlg0.OneBits.b7)&&(SysFunFlg1.OneBits.b2==0))
		MOVLB 	0H 			//0649 	1020
		BTSC 	49H, 7H 		//064A 	2BC9
		BTSC 	48H, 2H 		//064B 	2948
		RET 					//064C 	1008

		//;CHIPINIT.C: 38: {
		//;CHIPINIT.C: 39: if(TimeMin)TimeMin--;
		LDR 	77H, 0H 			//064D 	1877
		BTSS 	3H, 2H 			//064E 	2D03
		DECR 	77H, 1H 		//064F 	13F7
		RET 					//0650 	1008

		//;uart.c: 29: static unsigned char ByteNum;
		//;uart.c: 31: if(ByteNum<6)
		LDWI 	6H 			//0651 	0006
		MOVLB 	1H 			//0652 	1021
		SUBWR 	4EH, 0H 		//0653 	124E
		BTSC 	3H, 0H 			//0654 	2803
		LJUMP 	660H 			//0655 	3E60

		//;uart.c: 32: {
		//;uart.c: 33: URDATAL =send_buf[ByteNum++];
		LDR 	4EH, 0H 			//0656 	184E
		ADDWI 	A0H 			//0657 	0EA0
		STR 	6H 			//0658 	1086
		CLRF 	7H 			//0659 	1187
		LDR 	1H, 0H 			//065A 	1801
		MOVLB 	9H 			//065B 	1029
		STR 	CH 			//065C 	108C
		MOVLB 	1H 			//065D 	1021
		INCR 	4EH, 1H 		//065E 	1ACE

		//;uart.c: 34: }
		RET 					//065F 	1008

		//;uart.c: 35: else
		//;uart.c: 36: {
		//;uart.c: 37: URTE=0;
		MOVLB 	9H 			//0660 	1029
		BCR 	EH, 1H 			//0661 	208E

		//;uart.c: 38: UartSetEmptyInt(0);
		LDWI 	0H 			//0662 	0000
		LCALL 	669H 			//0663 	3669

		//;uart.c: 39: ByteNum = 0;
		MOVLB 	1H 			//0664 	1021
		CLRF 	4EH 			//0665 	11CE

		//;uart.c: 40: SysFunFlg0.OneBits.b6 = 0;
		MOVLB 	0H 			//0666 	1020
		BCR 	49H, 6H 			//0667 	2349
		RET 					//0668 	1008
		STR 	70H 			//0669 	10F0

		//;uart.c: 46: URTE = value;
		BTSS 	70H, 0H 		//066A 	2C70
		LJUMP 	66FH 			//066B 	3E6F
		MOVLB 	9H 			//066C 	1029
		BSR 	EH, 1H 			//066D 	248E
		RET 					//066E 	1008
		MOVLB 	9H 			//066F 	1029
		BCR 	EH, 1H 			//0670 	208E
		RET 					//0671 	1008
		MOVLP 	16H 			//0672 	0196
		LJUMP 	6C7H 			//0673 	3EC7
		MOVLP 	15H 			//0674 	0195
		LJUMP 	5BEH 			//0675 	3DBE
		MOVLP 	15H 			//0676 	0195
		LJUMP 	5C8H 			//0677 	3DC8
		MOVLP 	15H 			//0678 	0195
		LJUMP 	5E7H 			//0679 	3DE7
		MOVLP 	16H 			//067A 	0196
		LJUMP 	606H 			//067B 	3E06
		MOVLP 	16H 			//067C 	0196
		LJUMP 	61EH 			//067D 	3E1E
		MOVLP 	16H 			//067E 	0196
		LJUMP 	63DH 			//067F 	3E3D
		MOVLP 	16H 			//0680 	0196
		LJUMP 	663H 			//0681 	3E63
		MOVLP 	16H 			//0682 	0196
		LJUMP 	674H 			//0683 	3E74
		MOVLP 	16H 			//0684 	0196
		LJUMP 	6A1H 			//0685 	3EA1
		MOVLP 	16H 			//0686 	0196
		LJUMP 	6EDH 			//0687 	3EED
		MOVLP 	16H 			//0688 	0196
		LJUMP 	6F0H 			//0689 	3EF0
		MOVLP 	17H 			//068A 	0197
		LJUMP 	70FH 			//068B 	3F0F
		MOVLP 	17H 			//068C 	0197
		LJUMP 	72EH 			//068D 	3F2E
		MOVLP 	17H 			//068E 	0197
		LJUMP 	74DH 			//068F 	3F4D
		MOVLP 	17H 			//0690 	0197
		LJUMP 	75EH 			//0691 	3F5E
		MOVLB 	0H 			//0692 	1020
		STR 	4CH 			//0693 	10CC
		MOVLB 	7H 			//0694 	1027
		STR 	12H 			//0695 	1092
		MOVLB 	0H 			//0696 	1020
		LDR 	4CH, 0H 			//0697 	184C
		MOVLB 	7H 			//0698 	1027
		STR 	13H 			//0699 	1093
		MOVLB 	0H 			//069A 	1020
		LDR 	4CH, 0H 			//069B 	184C
		MOVLB 	7H 			//069C 	1027
		STR 	15H 			//069D 	1095
		LDWI 	10H 			//069E 	0010
		STR 	17H 			//069F 	1097
		LDWI 	18H 			//06A0 	0018
		STR 	19H 			//06A1 	1099
		STR 	1DH 			//06A2 	109D
		RET 					//06A3 	1008
		CLRWDT 			//06A4 	1064
		LDWI 	1H 			//06A5 	0001
		CLRF 	0H 			//06A6 	1180
		ADDFSR 	0H, 1H 		//06A7 	0101
		SUBWR 	7EH, 1H 		//06A8 	12FE
		LDWI 	0H 			//06A9 	0000
		SUBWFB 	7FH, 1H 		//06AA 	0BFF
		LDR 	7FH, 0H 			//06AB 	187F
		IORWR 	7EH, 0H 		//06AC 	147E
		BTSC 	3H, 2H 			//06AD 	2903
		RETW 	0H 			//06AE 	0400
		LJUMP 	6A5H 			//06AF 	3EA5
		MOVLP 	6H 			//06B0 	0186
		LJUMP 	609H 			//06B1 	3E09
		MOVLP 	6H 			//06B2 	0186
		LJUMP 	602H 			//06B3 	3E02
		MOVLP 	5H 			//06B4 	0185
		LJUMP 	5ADH 			//06B5 	3DAD
		MOVLP 	5H 			//06B6 	0185
		LJUMP 	5B9H 			//06B7 	3DB9
		MOVLP 	5H 			//06B8 	0185
		LJUMP 	5CBH 			//06B9 	3DCB
		MOVLP 	5H 			//06BA 	0185
		LJUMP 	5D7H 			//06BB 	3DD7
		MOVLP 	17H 			//06BC 	0197
		LJUMP 	7B6H 			//06BD 	3FB6
		MOVLP 	17H 			//06BE 	0197
		LJUMP 	7B8H 			//06BF 	3FB8
		MOVLP 	17H 			//06C0 	0197
		LJUMP 	7BAH 			//06C1 	3FBA
		MOVLP 	17H 			//06C2 	0197
		LJUMP 	7BCH 			//06C3 	3FBC
		MOVLP 	17H 			//06C4 	0197
		LJUMP 	7BEH 			//06C5 	3FBE
		MOVLP 	17H 			//06C6 	0197
		LJUMP 	7C0H 			//06C7 	3FC0
		MOVLP 	17H 			//06C8 	0197
		LJUMP 	782H 			//06C9 	3F82
		MOVLP 	17H 			//06CA 	0197
		LJUMP 	784H 			//06CB 	3F84
		MOVLP 	17H 			//06CC 	0197
		LJUMP 	786H 			//06CD 	3F86
		MOVLP 	17H 			//06CE 	0197
		LJUMP 	788H 			//06CF 	3F88
		MOVLP 	17H 			//06D0 	0197
		LJUMP 	78AH 			//06D1 	3F8A
		MOVLP 	17H 			//06D2 	0197
		ORG		06D3H
		LJUMP 	78CH 			//06D3 	3F8C
		ORG		0816H
		MOVLB 	1H 			//0816 	1021
		LDWI 	B0H 			//0817 	00B0
		STR 	65H 			//0818 	10E5
		LDWI 	4H 			//0819 	0004
		STR 	66H 			//081A 	10E6
		LDWI 	58H 			//081B 	0058
		STR 	67H 			//081C 	10E7
		LDWI 	2H 			//081D 	0002
		STR 	68H 			//081E 	10E8
		LDWI 	B0H 			//081F 	00B0
		STR 	69H 			//0820 	10E9
		LDWI 	4H 			//0821 	0004
		STR 	6AH 			//0822 	10EA
		LDWI 	E8H 			//0823 	00E8
		STR 	6BH 			//0824 	10EB
		LDWI 	3H 			//0825 	0003
		STR 	6CH 			//0826 	10EC
		LDWI 	23H 			//0827 	0023
		CLRF 	4H 			//0828 	1184
		STR 	5H 			//0829 	1085
		LDWI 	F0H 			//082A 	00F0
		STR 	7EH 			//082B 	10FE
		CLRF 	7FH 			//082C 	11FF
		MOVLP 	6H 			//082D 	0186
		LCALL 	6A4H 			//082E 	36A4
		MOVLP 	8H 			//082F 	0188
		LDWI 	2BH 			//0830 	002B
		CLRF 	76H 			//0831 	11F6
		CLRF 	77H 			//0832 	11F7
		CLRF 	78H 			//0833 	11F8
		CLRF 	79H 			//0834 	11F9
		CLRF 	7AH 			//0835 	11FA
		CLRF 	7BH 			//0836 	11FB
		STR 	4H 			//0837 	1084
		LDWI 	0H 			//0838 	0000
		STR 	5H 			//0839 	1085
		LDWI 	21H 			//083A 	0021
		STR 	7EH 			//083B 	10FE
		LDWI 	0H 			//083C 	0000
		STR 	7FH 			//083D 	10FF
		MOVLP 	6H 			//083E 	0186
		LCALL 	6A4H 			//083F 	36A4
		MOVLP 	8H 			//0840 	0188
		LDWI 	A0H 			//0841 	00A0
		STR 	4H 			//0842 	1084
		LDWI 	0H 			//0843 	0000
		STR 	5H 			//0844 	1085
		LDWI 	45H 			//0845 	0045
		STR 	7EH 			//0846 	10FE
		LDWI 	0H 			//0847 	0000
		STR 	7FH 			//0848 	10FF
		MOVLP 	6H 			//0849 	0186
		LCALL 	6A4H 			//084A 	36A4
		MOVLP 	8H 			//084B 	0188
		LDWI 	20H 			//084C 	0020
		STR 	4H 			//084D 	1084
		LDWI 	1H 			//084E 	0001
		STR 	5H 			//084F 	1085
		LDWI 	40H 			//0850 	0040
		STR 	7EH 			//0851 	10FE
		LDWI 	0H 			//0852 	0000
		STR 	7FH 			//0853 	10FF
		MOVLP 	6H 			//0854 	0186
		LCALL 	6A4H 			//0855 	36A4
		MOVLP 	8H 			//0856 	0188
		LDWI 	A0H 			//0857 	00A0
		STR 	4H 			//0858 	1084
		LDWI 	1H 			//0859 	0001
		STR 	5H 			//085A 	1085
		LDWI 	1EH 			//085B 	001E
		STR 	7EH 			//085C 	10FE
		LDWI 	0H 			//085D 	0000
		STR 	7FH 			//085E 	10FF
		MOVLP 	6H 			//085F 	0186
		LCALL 	6A4H 			//0860 	36A4
		MOVLP 	8H 			//0861 	0188
		BCR 	7EH, 0H 			//0862 	207E
		MOVLB 	0H 			//0863 	1020
		LJUMP 	65H 			//0864 	3865

		//;main.c: 154: Sys_init();
		LCALL 	3CFH 			//0865 	33CF
		MOVLP 	8H 			//0866 	0188

		//;main.c: 155: TouchInital();
		LCALL 	726H 			//0867 	3726
		MOVLP 	8H 			//0868 	0188

		//;main.c: 157: Time2Initial();
		LCALL 	3F9H 			//0869 	33F9
		MOVLP 	8H 			//086A 	0188

		//;main.c: 158: Time4Initial();
		LCALL 	407H 			//086B 	3407
		MOVLP 	8H 			//086C 	0188

		//;main.c: 160: UART_INITIAL();
		LCALL 	787H 			//086D 	3787
		MOVLP 	8H 			//086E 	0188

		//;main.c: 162: User_init();
		LCALL 	3B2H 			//086F 	33B2
		MOVLP 	8H 			//0870 	0188

		//;main.c: 163: Display_init();
		LCALL 	B6H 			//0871 	30B6
		MOVLP 	8H 			//0872 	0188

		//;main.c: 164: {BuzzerCnt=400;};
		LCALL 	67EH 			//0873 	367E
		MOVLP 	8H 			//0874 	0188

		//;main.c: 167: {
		//;main.c: 169: readrf();
		LCALL 	7D4H 			//0875 	37D4
		MOVLP 	8H 			//0876 	0188

		//;main.c: 171: if(TouchTimeCount>=(1000/12/(fmd_channels+1)))
		MOVLB 	0H 			//0877 	1020
		LDR 	47H, 0H 			//0878 	1847
		STR 	4CH 			//0879 	10CC
		CLRF 	4DH 			//087A 	11CD
		INCR 	4CH, 1H 		//087B 	1ACC
		BTSC 	3H, 2H 			//087C 	2903
		INCR 	4DH, 1H 		//087D 	1ACD
		LDWI 	53H 			//087E 	0053
		STR 	4EH 			//087F 	10CE
		CLRF 	4FH 			//0880 	11CF
		LCALL 	794H 			//0881 	3794
		MOVLP 	8H 			//0882 	0188
		LDR 	4DH, 0H 			//0883 	184D
		MOVLB 	1H 			//0884 	1021
		SUBWR 	44H, 0H 		//0885 	1244
		BTSS 	3H, 2H 			//0886 	2D03
		LJUMP 	8CH 			//0887 	388C
		MOVLB 	0H 			//0888 	1020
		LDR 	4CH, 0H 			//0889 	184C
		MOVLB 	1H 			//088A 	1021
		SUBWR 	43H, 0H 		//088B 	1243
		BTSS 	3H, 0H 			//088C 	2C03
		LJUMP 	94H 			//088D 	3894

		//;main.c: 172: {
		//;main.c: 173: TouchTimeCount=0;
		CLRF 	43H 			//088E 	11C3
		CLRF 	44H 			//088F 	11C4

		//;main.c: 175: readKeyNum = TouchKeyScan();
		LCALL 	BBH 			//0890 	30BB
		MOVLP 	8H 			//0891 	0188
		LCALL 	59AH 			//0892 	359A
		MOVLP 	8H 			//0893 	0188

		//;main.c: 176: }
		//;main.c: 177: if(SysFunFlg0.OneBits.b1)
		MOVLB 	0H 			//0894 	1020
		BTSS 	49H, 1H 		//0895 	2CC9
		LJUMP 	9AH 			//0896 	389A

		//;main.c: 178: {
		//;main.c: 179: SysFunFlg0.OneBits.b1=0;
		BCR 	49H, 1H 			//0897 	20C9

		//;main.c: 180: RxData_Drive();
		LCALL 	684H 			//0898 	3684
		MOVLP 	8H 			//0899 	0188

		//;main.c: 182: }
		//;main.c: 183: if(SysFunFlg0.OneBits.b0)
		MOVLB 	0H 			//089A 	1020
		BTSS 	49H, 0H 		//089B 	2C49
		LJUMP 	75H 			//089C 	3875

		//;main.c: 184: {
		//;main.c: 185: SysFunFlg0.OneBits.b0=0;
		BCR 	49H, 0H 			//089D 	2049

		//;main.c: 187: KeyScanHandler();
		LCALL 	366H 			//089E 	3366
		MOVLP 	8H 			//089F 	0188

		//;main.c: 188: dokey();
		LCALL 	59FH 			//08A0 	359F
		MOVLP 	8H 			//08A1 	0188

		//;main.c: 190: SysPowerHandler();
		LCALL 	3BCH 			//08A2 	33BC
		MOVLP 	8H 			//08A3 	0188

		//;main.c: 191: ChkTimeEventStatus();
		LCALL 	A7H 			//08A4 	30A7
		MOVLP 	8H 			//08A5 	0188
		LJUMP 	75H 			//08A6 	3875

		//;sub.c: 347: if(dsp_100ms)
		MOVLB 	1H 			//08A7 	1021
		LDR 	52H, 0H 			//08A8 	1852
		BTSS 	3H, 2H 			//08A9 	2D03

		//;sub.c: 348: {
		//;sub.c: 349: if(--dsp_100ms==0)
		DECRSZ 	52H, 1H 		//08AA 	1BD2
		RET 					//08AB 	1008

		//;sub.c: 350: {
		//;sub.c: 351: if(SysFunFlg0.OneBits.b7)
		MOVLB 	0H 			//08AC 	1020
		BTSS 	49H, 7H 		//08AD 	2FC9
		LJUMP 	B3H 			//08AE 	38B3

		//;sub.c: 352: DspIndex=DsTime;
		LDWI 	5H 			//08AF 	0005
		MOVLB 	1H 			//08B0 	1021
		STR 	60H 			//08B1 	10E0
		RET 					//08B2 	1008

		//;sub.c: 354: else
		//;sub.c: 355: DspIndex=DsSty;
		MOVLB 	1H 			//08B3 	1021
		CLRF 	60H 			//08B4 	11E0
		RET 					//08B5 	1008

		//;CHIPINIT.C: 696: {LATB1=0;LATD1=0;LATC1=0;LATC0=0; LATA0=0;LATB0=0;LATB3=0; LATA4=0;LAT
		//+                          D2=0;LATA1=0;LATD3=0;}
		MOVLP 	17H 			//08B6 	0197
		LCALL 	7EDH 			//08B7 	37ED

		//;CHIPINIT.C: 697: INTCON |= 0B11000000;
		LDWI 	C0H 			//08B8 	00C0
		IORWR 	BH, 1H 		//08B9 	148B
		RET 					//08BA 	1008
		LCALL 	101H 			//08BB 	3101
		MOVLP 	8H 			//08BC 	0188
		LDWI 	2H 			//08BD 	0002
		XORWR 	3DH, 0H 		//08BE 	163D
		BTSC 	3H, 2H 			//08BF 	2903
		LJUMP 	FBH 			//08C0 	38FB
		LDR 	3FH, 0H 			//08C1 	183F
		BTSS 	3H, 2H 			//08C2 	2D03
		LJUMP 	FBH 			//08C3 	38FB
		LDR 	35H, 0H 			//08C4 	1835
		IORWR 	36H, 0H 		//08C5 	1436
		BTSC 	3H, 2H 			//08C6 	2903
		LJUMP 	ECH 			//08C7 	38EC
		CLRF 	3DH 			//08C8 	11BD
		MOVLB 	1H 			//08C9 	1021
		INCR 	3DH, 1H 		//08CA 	1ABD
		BTSC 	3H, 2H 			//08CB 	2903
		INCR 	3EH, 1H 		//08CC 	1ABE
		LDWI 	0H 			//08CD 	0000
		SUBWR 	3EH, 0H 		//08CE 	123E
		LDWI 	CH 			//08CF 	000C
		BTSC 	3H, 2H 			//08D0 	2903
		SUBWR 	3DH, 0H 		//08D1 	123D
		BTSS 	3H, 0H 			//08D2 	2C03
		LJUMP 	E6H 			//08D3 	38E6
		CLRF 	3DH 			//08D4 	11BD
		CLRF 	3EH 			//08D5 	11BE
		INCR 	3BH, 1H 		//08D6 	1ABB
		BTSC 	3H, 2H 			//08D7 	2903
		INCR 	3CH, 1H 		//08D8 	1ABC
		LDR 	3AH, 0H 			//08D9 	183A
		SUBWR 	3CH, 0H 		//08DA 	123C
		BTSS 	3H, 2H 			//08DB 	2D03
		LJUMP 	DFH 			//08DC 	38DF
		LDR 	39H, 0H 			//08DD 	1839
		SUBWR 	3BH, 0H 		//08DE 	123B
		BTSS 	3H, 0H 			//08DF 	2C03
		LJUMP 	E6H 			//08E0 	38E6
		LDWI 	2H 			//08E1 	0002
		CLRF 	3BH 			//08E2 	11BB
		CLRF 	3CH 			//08E3 	11BC
		MOVLB 	0H 			//08E4 	1020
		STR 	3DH 			//08E5 	10BD
		MOVLB 	1H 			//08E6 	1021
		CLRF 	6BH 			//08E7 	11EB
		CLRF 	6CH 			//08E8 	11EC
		CLRF 	69H 			//08E9 	11E9
		CLRF 	6AH 			//08EA 	11EA
		LJUMP 	FBH 			//08EB 	38FB
		MOVLB 	1H 			//08EC 	1021
		INCR 	6BH, 1H 		//08ED 	1AEB
		BTSC 	3H, 2H 			//08EE 	2903
		INCR 	6CH, 1H 		//08EF 	1AEC
		INCR 	69H, 1H 		//08F0 	1AE9
		BTSC 	3H, 2H 			//08F1 	2903
		INCR 	6AH, 1H 		//08F2 	1AEA
		MOVLB 	0H 			//08F3 	1020
		CLRF 	3DH 			//08F4 	11BD
		INCR 	3DH, 1H 		//08F5 	1ABD
		MOVLB 	1H 			//08F6 	1021
		CLRF 	3DH 			//08F7 	11BD
		CLRF 	3EH 			//08F8 	11BE
		CLRF 	3BH 			//08F9 	11BB
		CLRF 	3CH 			//08FA 	11BC
		MOVLB 	0H 			//08FB 	1020
		LDR 	36H, 0H 			//08FC 	1836
		STR 	6CH 			//08FD 	10EC
		LDR 	35H, 0H 			//08FE 	1835
		STR 	6BH 			//08FF 	10EB
		RET 					//0900 	1008
		LDWI 	4H 			//0901 	0004
		MOVLB 	0H 			//0902 	1020
		SUBWR 	4BH, 0H 		//0903 	124B
		BTSC 	3H, 0H 			//0904 	2803
		LJUMP 	11AH 			//0905 	391A
		LCALL 	58AH 			//0906 	358A
		MOVLP 	8H 			//0907 	0188
		LCALL 	30CH 			//0908 	330C
		MOVLP 	8H 			//0909 	0188
		LCALL 	4ACH 			//090A 	34AC
		MOVLP 	8H 			//090B 	0188
		LCALL 	41CH 			//090C 	341C
		MOVLP 	8H 			//090D 	0188
		BTSC 	3H, 0H 			//090E 	2803
		INCR 	26H, 1H 		//090F 	1AA6
		LCALL 	325H 			//0910 	3325
		MOVLP 	8H 			//0911 	0188
		LCALL 	464H 			//0912 	3464
		MOVLP 	8H 			//0913 	0188
		LDWI 	3H 			//0914 	0003
		LCALL 	30CH 			//0915 	330C
		MOVLP 	8H 			//0916 	0188
		LCALL 	4ACH 			//0917 	34AC
		MOVLP 	8H 			//0918 	0188
		LJUMP 	16DH 			//0919 	396D
		LDWI 	8H 			//091A 	0008
		SUBWR 	4BH, 0H 		//091B 	124B
		BTSC 	3H, 0H 			//091C 	2803
		LJUMP 	132H 			//091D 	3932
		LCALL 	58AH 			//091E 	358A
		MOVLP 	8H 			//091F 	0188
		LCALL 	30CH 			//0920 	330C
		MOVLP 	8H 			//0921 	0188
		LCALL 	480H 			//0922 	3480
		MOVLP 	8H 			//0923 	0188
		LCALL 	41CH 			//0924 	341C
		MOVLP 	8H 			//0925 	0188
		BTSC 	3H, 0H 			//0926 	2803
		INCR 	26H, 1H 		//0927 	1AA6
		LCALL 	325H 			//0928 	3325
		MOVLP 	8H 			//0929 	0188
		LCALL 	464H 			//092A 	3464
		MOVLP 	8H 			//092B 	0188
		LDWI 	3H 			//092C 	0003
		LCALL 	30CH 			//092D 	330C
		MOVLP 	8H 			//092E 	0188
		LCALL 	480H 			//092F 	3480
		MOVLP 	8H 			//0930 	0188
		LJUMP 	16DH 			//0931 	396D
		LDWI 	CH 			//0932 	000C
		SUBWR 	4BH, 0H 		//0933 	124B
		BTSC 	3H, 0H 			//0934 	2803
		LJUMP 	158H 			//0935 	3958
		LCALL 	58FH 			//0936 	358F
		MOVLP 	8H 			//0937 	0188
		LCALL 	30CH 			//0938 	330C
		MOVLP 	8H 			//0939 	0188
		LDWI 	3FH 			//093A 	003F
		MOVLB 	7H 			//093B 	1027
		STR 	14H 			//093C 	1094
		LCALL 	506H 			//093D 	3506
		MOVLP 	8H 			//093E 	0188
		LDWI 	3FH 			//093F 	003F
		STR 	15H 			//0940 	1095
		LCALL 	526H 			//0941 	3526
		MOVLP 	8H 			//0942 	0188
		LCALL 	41CH 			//0943 	341C
		MOVLP 	8H 			//0944 	0188
		BTSC 	3H, 0H 			//0945 	2803
		INCR 	26H, 1H 		//0946 	1AA6
		LCALL 	325H 			//0947 	3325
		MOVLP 	8H 			//0948 	0188
		LCALL 	464H 			//0949 	3464
		MOVLP 	8H 			//094A 	0188
		LDWI 	1FH 			//094B 	001F
		LCALL 	30CH 			//094C 	330C
		MOVLP 	8H 			//094D 	0188
		LDWI 	52H 			//094E 	0052
		MOVLB 	7H 			//094F 	1027
		STR 	14H 			//0950 	1094
		LCALL 	506H 			//0951 	3506
		MOVLP 	8H 			//0952 	0188
		LDWI 	52H 			//0953 	0052
		STR 	15H 			//0954 	1095
		LCALL 	526H 			//0955 	3526
		MOVLP 	8H 			//0956 	0188
		LJUMP 	16DH 			//0957 	396D
		LCALL 	58FH 			//0958 	358F
		MOVLP 	8H 			//0959 	0188
		LCALL 	30CH 			//095A 	330C
		MOVLP 	8H 			//095B 	0188
		LDWI 	3FH 			//095C 	003F
		LCALL 	4D6H 			//095D 	34D6
		MOVLP 	8H 			//095E 	0188
		LCALL 	41CH 			//095F 	341C
		MOVLP 	8H 			//0960 	0188
		BTSC 	3H, 0H 			//0961 	2803
		INCR 	26H, 1H 		//0962 	1AA6
		LCALL 	325H 			//0963 	3325
		MOVLP 	8H 			//0964 	0188
		LCALL 	464H 			//0965 	3464
		MOVLP 	8H 			//0966 	0188
		LDWI 	1FH 			//0967 	001F
		LCALL 	30CH 			//0968 	330C
		MOVLP 	8H 			//0969 	0188
		LDWI 	52H 			//096A 	0052
		LCALL 	4D6H 			//096B 	34D6
		MOVLP 	8H 			//096C 	0188
		LCALL 	41CH 			//096D 	341C
		MOVLP 	8H 			//096E 	0188
		BTSC 	3H, 0H 			//096F 	2803
		INCR 	26H, 1H 		//0970 	1AA6
		LCALL 	325H 			//0971 	3325
		MOVLP 	8H 			//0972 	0188
		LCALL 	447H 			//0973 	3447
		MOVLP 	8H 			//0974 	0188
		MOVLB 	1H 			//0975 	1021
		LDR 	66H, 0H 			//0976 	1866
		XORWR 	6AH, 0H 		//0977 	166A
		BTSS 	3H, 2H 			//0978 	2D03
		LJUMP 	17CH 			//0979 	397C
		LDR 	65H, 0H 			//097A 	1865
		XORWR 	69H, 0H 		//097B 	1669
		BTSC 	3H, 2H 			//097C 	2903
		LJUMP 	183H 			//097D 	3983
		LDWI 	2H 			//097E 	0002
		MOVLB 	0H 			//097F 	1020
		XORWR 	3DH, 0H 		//0980 	163D
		BTSS 	3H, 2H 			//0981 	2D03
		LJUMP 	186H 			//0982 	3986
		MOVLB 	0H 			//0983 	1020
		CLRF 	3CH 			//0984 	11BC
		INCR 	3CH, 1H 		//0985 	1ABC
		LDR 	4BH, 0H 			//0986 	184B
		LCALL 	1E6H 			//0987 	31E6
		MOVLB 	0H 			//0988 	1020
		CLRF 	3CH 			//0989 	11BC
		MOVLP 	0H 			//098A 	0180
		LCALL 	39H 			//098B 	3039
		MOVLP 	8H 			//098C 	0188
		MOVLB 	1H 			//098D 	1021
		LDR 	4CH, 0H 			//098E 	184C
		MOVLB 	0H 			//098F 	1020
		XORWR 	4BH, 0H 		//0990 	164B
		BTSS 	3H, 2H 			//0991 	2D03
		LJUMP 	1D9H 			//0992 	39D9
		BTSS 	76H, 2H 		//0993 	2D76
		LJUMP 	197H 			//0994 	3997
		BCR 	76H, 2H 			//0995 	2176
		LJUMP 	1D9H 			//0996 	39D9
		CLRF 	40H 			//0997 	11C0
		CLRF 	3EH 			//0998 	11BE
		LDWI 	FH 			//0999 	000F
		SUBWR 	3EH, 0H 		//099A 	123E
		BTSC 	3H, 0H 			//099B 	2803
		LJUMP 	1ABH 			//099C 	39AB
		LCALL 	594H 			//099D 	3594
		MOVLP 	8H 			//099E 	0188
		LJUMP 	1A2H 			//099F 	39A2
		LSRF 	68H, 1H 		//09A0 	06E8
		RRR 	67H, 1H 			//09A1 	1CE7
		DECRSZ 	9H, 1H 		//09A2 	1B89
		LJUMP 	1A0H 			//09A3 	39A0
		BTSC 	67H, 0H 		//09A4 	2867
		INCR 	40H, 1H 		//09A5 	1AC0
		INCR 	3EH, 1H 		//09A6 	1ABE
		LDWI 	FH 			//09A7 	000F
		SUBWR 	3EH, 0H 		//09A8 	123E
		BTSS 	3H, 0H 			//09A9 	2C03
		LJUMP 	19DH 			//09AA 	399D
		LDWI 	FFH 			//09AB 	00FF
		BCR 	76H, 0H 			//09AC 	2076
		BCR 	76H, 1H 			//09AD 	20F6
		STR 	3EH 			//09AE 	10BE
		LCALL 	594H 			//09AF 	3594
		MOVLP 	8H 			//09B0 	0188
		LJUMP 	1B4H 			//09B1 	39B4
		LSRF 	68H, 1H 		//09B2 	06E8
		RRR 	67H, 1H 			//09B3 	1CE7
		DECRSZ 	9H, 1H 		//09B4 	1B89
		LJUMP 	1B2H 			//09B5 	39B2
		BTSC 	67H, 0H 		//09B6 	2867
		BSR 	76H, 1H 			//09B7 	24F6
		MOVLB 	1H 			//09B8 	1021
		DECRSZ 	49H, 0H 		//09B9 	1B49
		LJUMP 	1D4H 			//09BA 	39D4
		DECR 	48H, 0H 		//09BB 	1348
		BTSC 	3H, 2H 			//09BC 	2903
		LJUMP 	1D4H 			//09BD 	39D4
		LDWI 	5H 			//09BE 	0005
		SUBWR 	4AH, 0H 		//09BF 	124A
		BTSC 	3H, 0H 			//09C0 	2803
		LJUMP 	1D4H 			//09C1 	39D4
		MOVLB 	0H 			//09C2 	1020
		LDR 	37H, 0H 			//09C3 	1837
		IORWR 	38H, 0H 		//09C4 	1438
		BTSC 	3H, 2H 			//09C5 	2903
		LJUMP 	1CCH 			//09C6 	39CC
		MOVLB 	1H 			//09C7 	1021
		CLRF 	4DH 			//09C8 	11CD
		MOVLB 	0H 			//09C9 	1020
		CLRF 	43H 			//09CA 	11C3
		LJUMP 	1CFH 			//09CB 	39CF
		CLRF 	43H 			//09CC 	11C3
		MOVLB 	1H 			//09CD 	1021
		CLRF 	4DH 			//09CE 	11CD
		MOVLB 	0H 			//09CF 	1020
		LDR 	38H, 0H 			//09D0 	1838
		STR 	36H 			//09D1 	10B6
		LDR 	37H, 0H 			//09D2 	1837
		STR 	35H 			//09D3 	10B5
		LDWI 	2H 			//09D4 	0002
		MOVLB 	1H 			//09D5 	1021
		STR 	48H 			//09D6 	10C8
		BCR 	76H, 3H 			//09D7 	21F6
		CLRF 	4AH 			//09D8 	11CA
		MOVLB 	0H 			//09D9 	1020
		LDR 	47H, 0H 			//09DA 	1847
		SUBWR 	3FH, 0H 		//09DB 	123F
		BTSC 	3H, 0H 			//09DC 	2803
		LJUMP 	1E0H 			//09DD 	39E0
		INCR 	3FH, 1H 		//09DE 	1ABF
		LJUMP 	1E1H 			//09DF 	39E1
		CLRF 	3FH 			//09E0 	11BF
		LDR 	3FH, 0H 			//09E1 	183F
		MOVLP 	5H 			//09E2 	0185
		LCALL 	588H 			//09E3 	3588
		STR 	4BH 			//09E4 	10CB
		RET 					//09E5 	1008
		STR 	51H 			//09E6 	10D1
		MOVLB 	7H 			//09E7 	1027
		CLRF 	FH 			//09E8 	118F
		CLRF 	10H 			//09E9 	1190
		CLRF 	11H 			//09EA 	1191
		LDWI 	4H 			//09EB 	0004
		MOVLB 	0H 			//09EC 	1020
		SUBWR 	51H, 0H 		//09ED 	1251
		BTSC 	3H, 0H 			//09EE 	2803
		LJUMP 	251H 			//09EF 	3A51
		LDWI 	1H 			//09F0 	0001
		LCALL 	30CH 			//09F1 	330C
		MOVLP 	8H 			//09F2 	0188
		LDWI 	4H 			//09F3 	0004
		MOVLB 	7H 			//09F4 	1027
		STR 	12H 			//09F5 	1092
		LCALL 	4FEH 			//09F6 	34FE
		MOVLP 	8H 			//09F7 	0188
		IORWI 	3H 			//09F8 	0803
		LCALL 	56AH 			//09F9 	356A
		MOVLP 	8H 			//09FA 	0188
		LCALL 	51EH 			//09FB 	351E
		MOVLP 	8H 			//09FC 	0188
		BTSS 	3H, 2H 			//09FD 	2D03
		LJUMP 	212H 			//09FE 	3A12
		MOVLB 	7H 			//09FF 	1027
		BSR 	DH, 5H 			//0A00 	268D
		BTSS 	DH, 6H 			//0A01 	2F0D
		LJUMP 	201H 			//0A02 	3A01
		MOVLB 	1FH 			//0A03 	103F
		LDR 	15H, 0H 			//0A04 	1815
		MOVLB 	0H 			//0A05 	1020
		STR 	22H 			//0A06 	10A2
		CLRF 	21H 			//0A07 	11A1
		MOVLB 	1FH 			//0A08 	103F
		LDR 	14H, 0H 			//0A09 	1814
		LCALL 	41CH 			//0A0A 	341C
		MOVLP 	8H 			//0A0B 	0188
		BTSC 	3H, 0H 			//0A0C 	2803
		INCR 	26H, 1H 		//0A0D 	1AA6
		LCALL 	325H 			//0A0E 	3325
		MOVLP 	8H 			//0A0F 	0188
		LCALL 	42AH 			//0A10 	342A
		MOVLP 	8H 			//0A11 	0188
		LDWI 	2H 			//0A12 	0002
		LCALL 	30CH 			//0A13 	330C
		MOVLP 	8H 			//0A14 	0188
		LDWI 	4H 			//0A15 	0004
		MOVLB 	7H 			//0A16 	1027
		STR 	12H 			//0A17 	1092
		LCALL 	4FEH 			//0A18 	34FE
		MOVLP 	8H 			//0A19 	0188
		LCALL 	56AH 			//0A1A 	356A
		MOVLP 	8H 			//0A1B 	0188
		STR 	15H 			//0A1C 	1095
		LDWI 	C0H 			//0A1D 	00C0
		LCALL 	57EH 			//0A1E 	357E
		MOVLP 	8H 			//0A1F 	0188
		BTSS 	3H, 2H 			//0A20 	2D03
		LJUMP 	24FH 			//0A21 	3A4F
		MOVLB 	7H 			//0A22 	1027
		BSR 	DH, 5H 			//0A23 	268D
		BTSS 	DH, 6H 			//0A24 	2F0D
		LJUMP 	224H 			//0A25 	3A24
		MOVLB 	1FH 			//0A26 	103F
		LDR 	15H, 0H 			//0A27 	1815
		MOVLB 	0H 			//0A28 	1020
		STR 	22H 			//0A29 	10A2
		CLRF 	21H 			//0A2A 	11A1
		MOVLB 	1FH 			//0A2B 	103F
		LDR 	14H, 0H 			//0A2C 	1814
		LCALL 	41CH 			//0A2D 	341C
		MOVLP 	8H 			//0A2E 	0188
		BTSC 	3H, 0H 			//0A2F 	2803
		INCR 	26H, 1H 		//0A30 	1AA6
		LCALL 	325H 			//0A31 	3325
		MOVLB 	0H 			//0A32 	1020
		LDR 	28H, 0H 			//0A33 	1828
		STR 	30H 			//0A34 	10B0
		LDR 	27H, 0H 			//0A35 	1827
		STR 	2FH 			//0A36 	10AF
		LDR 	4BH, 0H 			//0A37 	184B
		STR 	4DH 			//0A38 	10CD
		LDWI 	4H 			//0A39 	0004
		CLRF 	4EH 			//0A3A 	11CE
		LSLF 	4DH, 1H 		//0A3B 	05CD
		RLR 	4EH, 1H 			//0A3C 	1DCE
		LSLF 	4DH, 1H 		//0A3D 	05CD
		RLR 	4EH, 1H 			//0A3E 	1DCE
		LSLF 	4DH, 1H 		//0A3F 	05CD
		RLR 	4EH, 1H 			//0A40 	1DCE
		STR 	4FH 			//0A41 	10CF
		LDWI 	23H 			//0A42 	0023
		STR 	50H 			//0A43 	10D0
		LDR 	4DH, 0H 			//0A44 	184D
		ADDWR 	4FH, 0H 		//0A45 	174F
		STR 	6H 			//0A46 	1086
		LDR 	4EH, 0H 			//0A47 	184E
		ADDWFC 	50H, 0H 		//0A48 	0D50
		STR 	7H 			//0A49 	1087
		LDR 	2FH, 0H 			//0A4A 	182F
		MOVWI 	0H[1] 			//0A4B 	0FC0
		LDR 	30H, 0H 			//0A4C 	1830
		MOVWI 	1H[1] 			//0A4D 	0FC1
		RET 					//0A4E 	1008
		LDWI 	0H 			//0A4F 	0000
		LJUMP 	30CH 			//0A50 	3B0C
		LDWI 	8H 			//0A51 	0008
		SUBWR 	51H, 0H 		//0A52 	1251
		BTSC 	3H, 0H 			//0A53 	2803
		LJUMP 	295H 			//0A54 	3A95
		LDWI 	1H 			//0A55 	0001
		LCALL 	30CH 			//0A56 	330C
		MOVLP 	8H 			//0A57 	0188
		LDWI 	4H 			//0A58 	0004
		MOVLB 	7H 			//0A59 	1027
		STR 	13H 			//0A5A 	1093
		LCALL 	4FEH 			//0A5B 	34FE
		MOVLP 	8H 			//0A5C 	0188
		IORWI 	3H 			//0A5D 	0803
		LCALL 	535H 			//0A5E 	3535
		MOVLP 	8H 			//0A5F 	0188
		LCALL 	51EH 			//0A60 	351E
		MOVLP 	8H 			//0A61 	0188
		BTSS 	3H, 2H 			//0A62 	2D03
		LJUMP 	277H 			//0A63 	3A77
		MOVLB 	7H 			//0A64 	1027
		BSR 	DH, 5H 			//0A65 	268D
		BTSS 	DH, 6H 			//0A66 	2F0D
		LJUMP 	266H 			//0A67 	3A66
		MOVLB 	1FH 			//0A68 	103F
		LDR 	1DH, 0H 			//0A69 	181D
		MOVLB 	0H 			//0A6A 	1020
		STR 	22H 			//0A6B 	10A2
		CLRF 	21H 			//0A6C 	11A1
		MOVLB 	1FH 			//0A6D 	103F
		LDR 	1CH, 0H 			//0A6E 	181C
		LCALL 	41CH 			//0A6F 	341C
		MOVLP 	8H 			//0A70 	0188
		BTSC 	3H, 0H 			//0A71 	2803
		INCR 	26H, 1H 		//0A72 	1AA6
		LCALL 	325H 			//0A73 	3325
		MOVLP 	8H 			//0A74 	0188
		LCALL 	42AH 			//0A75 	342A
		MOVLP 	8H 			//0A76 	0188
		LDWI 	2H 			//0A77 	0002
		LCALL 	30CH 			//0A78 	330C
		MOVLP 	8H 			//0A79 	0188
		LDWI 	4H 			//0A7A 	0004
		MOVLB 	7H 			//0A7B 	1027
		STR 	13H 			//0A7C 	1093
		LCALL 	4FEH 			//0A7D 	34FE
		MOVLP 	8H 			//0A7E 	0188
		LCALL 	535H 			//0A7F 	3535
		MOVLP 	8H 			//0A80 	0188
		STR 	15H 			//0A81 	1095
		LDWI 	C0H 			//0A82 	00C0
		LCALL 	57EH 			//0A83 	357E
		MOVLP 	8H 			//0A84 	0188
		BTSS 	3H, 2H 			//0A85 	2D03
		LJUMP 	293H 			//0A86 	3A93
		MOVLB 	7H 			//0A87 	1027
		BSR 	DH, 5H 			//0A88 	268D
		BTSS 	DH, 6H 			//0A89 	2F0D
		LJUMP 	289H 			//0A8A 	3A89
		MOVLB 	1FH 			//0A8B 	103F
		LDR 	1DH, 0H 			//0A8C 	181D
		MOVLB 	0H 			//0A8D 	1020
		STR 	22H 			//0A8E 	10A2
		CLRF 	21H 			//0A8F 	11A1
		MOVLB 	1FH 			//0A90 	103F
		LDR 	1CH, 0H 			//0A91 	181C
		LJUMP 	22DH 			//0A92 	3A2D
		LDWI 	0H 			//0A93 	0000
		LJUMP 	30CH 			//0A94 	3B0C
		LDWI 	CH 			//0A95 	000C
		SUBWR 	51H, 0H 		//0A96 	1251
		LDWI 	1H 			//0A97 	0001
		BTSC 	3H, 0H 			//0A98 	2803
		LJUMP 	2D6H 			//0A99 	3AD6
		LCALL 	30CH 			//0A9A 	330C
		MOVLP 	8H 			//0A9B 	0188
		LDWI 	4H 			//0A9C 	0004
		MOVLB 	7H 			//0A9D 	1027
		STR 	14H 			//0A9E 	1094
		LCALL 	4FEH 			//0A9F 	34FE
		MOVLP 	8H 			//0AA0 	0188
		IORWI 	3H 			//0AA1 	0803
		LCALL 	559H 			//0AA2 	3559
		MOVLP 	8H 			//0AA3 	0188
		LCALL 	51EH 			//0AA4 	351E
		MOVLP 	8H 			//0AA5 	0188
		BTSS 	3H, 2H 			//0AA6 	2D03
		LJUMP 	2BBH 			//0AA7 	3ABB
		MOVLB 	7H 			//0AA8 	1027
		BSR 	DH, 5H 			//0AA9 	268D
		BTSS 	DH, 6H 			//0AAA 	2F0D
		LJUMP 	2AAH 			//0AAB 	3AAA
		MOVLB 	1FH 			//0AAC 	103F
		LDR 	25H, 0H 			//0AAD 	1825
		MOVLB 	0H 			//0AAE 	1020
		STR 	22H 			//0AAF 	10A2
		CLRF 	21H 			//0AB0 	11A1
		MOVLB 	1FH 			//0AB1 	103F
		LDR 	24H, 0H 			//0AB2 	1824
		LCALL 	41CH 			//0AB3 	341C
		MOVLP 	8H 			//0AB4 	0188
		BTSC 	3H, 0H 			//0AB5 	2803
		INCR 	26H, 1H 		//0AB6 	1AA6
		LCALL 	325H 			//0AB7 	3325
		MOVLP 	8H 			//0AB8 	0188
		LCALL 	42AH 			//0AB9 	342A
		MOVLP 	8H 			//0ABA 	0188
		LDWI 	2H 			//0ABB 	0002
		LCALL 	30CH 			//0ABC 	330C
		MOVLP 	8H 			//0ABD 	0188
		LDWI 	4H 			//0ABE 	0004
		MOVLB 	7H 			//0ABF 	1027
		STR 	14H 			//0AC0 	1094
		LCALL 	4FEH 			//0AC1 	34FE
		MOVLP 	8H 			//0AC2 	0188
		LCALL 	559H 			//0AC3 	3559
		MOVLP 	8H 			//0AC4 	0188
		STR 	15H 			//0AC5 	1095
		LDWI 	C0H 			//0AC6 	00C0
		LCALL 	57EH 			//0AC7 	357E
		MOVLP 	8H 			//0AC8 	0188
		BTSS 	3H, 2H 			//0AC9 	2D03
		LJUMP 	2D4H 			//0ACA 	3AD4
		MOVLB 	7H 			//0ACB 	1027
		BSR 	DH, 5H 			//0ACC 	268D
		BTSS 	DH, 6H 			//0ACD 	2F0D
		LJUMP 	2CDH 			//0ACE 	3ACD
		MOVLB 	1FH 			//0ACF 	103F
		LDR 	25H, 0H 			//0AD0 	1825
		LCALL 	584H 			//0AD1 	3584
		MOVLP 	8H 			//0AD2 	0188
		LJUMP 	22DH 			//0AD3 	3A2D
		LDWI 	0H 			//0AD4 	0000
		LJUMP 	30CH 			//0AD5 	3B0C
		LCALL 	30CH 			//0AD6 	330C
		MOVLP 	8H 			//0AD7 	0188
		LDWI 	4H 			//0AD8 	0004
		MOVLB 	7H 			//0AD9 	1027
		STR 	14H 			//0ADA 	1094
		LDWI 	3H 			//0ADB 	0003
		STR 	1AH 			//0ADC 	109A
		LCALL 	579H 			//0ADD 	3579
		MOVLP 	8H 			//0ADE 	0188
		LCALL 	4FEH 			//0ADF 	34FE
		MOVLP 	8H 			//0AE0 	0188
		IORWI 	3H 			//0AE1 	0803
		LCALL 	546H 			//0AE2 	3546
		MOVLP 	8H 			//0AE3 	0188
		BTSS 	3H, 2H 			//0AE4 	2D03
		LJUMP 	2F6H 			//0AE5 	3AF6
		MOVLB 	7H 			//0AE6 	1027
		BSR 	DH, 5H 			//0AE7 	268D
		BTSS 	DH, 6H 			//0AE8 	2F0D
		LJUMP 	2E8H 			//0AE9 	3AE8
		MOVLB 	1FH 			//0AEA 	103F
		LDR 	25H, 0H 			//0AEB 	1825
		LCALL 	584H 			//0AEC 	3584
		MOVLP 	8H 			//0AED 	0188
		LCALL 	41CH 			//0AEE 	341C
		MOVLP 	8H 			//0AEF 	0188
		BTSC 	3H, 0H 			//0AF0 	2803
		INCR 	26H, 1H 		//0AF1 	1AA6
		LCALL 	325H 			//0AF2 	3325
		MOVLP 	8H 			//0AF3 	0188
		LCALL 	42AH 			//0AF4 	342A
		MOVLP 	8H 			//0AF5 	0188
		LDWI 	2H 			//0AF6 	0002
		LCALL 	30CH 			//0AF7 	330C
		MOVLP 	8H 			//0AF8 	0188
		LDWI 	4H 			//0AF9 	0004
		MOVLB 	7H 			//0AFA 	1027
		STR 	14H 			//0AFB 	1094
		CLRF 	1AH 			//0AFC 	119A
		LCALL 	579H 			//0AFD 	3579
		MOVLP 	8H 			//0AFE 	0188
		LCALL 	4FEH 			//0AFF 	34FE
		MOVLP 	8H 			//0B00 	0188
		LCALL 	546H 			//0B01 	3546
		MOVLP 	8H 			//0B02 	0188
		BTSS 	3H, 2H 			//0B03 	2D03
		LJUMP 	30AH 			//0B04 	3B0A
		MOVLB 	7H 			//0B05 	1027
		BSR 	DH, 5H 			//0B06 	268D
		BTSC 	DH, 6H 			//0B07 	2B0D
		LJUMP 	2CFH 			//0B08 	3ACF
		LJUMP 	307H 			//0B09 	3B07
		LDWI 	0H 			//0B0A 	0000
		LJUMP 	30CH 			//0B0B 	3B0C
		MOVLB 	0H 			//0B0C 	1020
		STR 	4CH 			//0B0D 	10CC
		COMR 	4CH, 0H 		//0B0E 	194C
		MOVLB 	7H 			//0B0F 	1027
		STR 	CH 			//0B10 	108C
		CLRF 	DH 			//0B11 	118D
		LDWI 	3H 			//0B12 	0003
		STR 	EH 			//0B13 	108E
		CLRF 	12H 			//0B14 	1192
		CLRF 	16H 			//0B15 	1196
		CLRF 	17H 			//0B16 	1197
		CLRF 	13H 			//0B17 	1193
		CLRF 	18H 			//0B18 	1198
		CLRF 	19H 			//0B19 	1199
		CLRF 	14H 			//0B1A 	1194
		CLRF 	1AH 			//0B1B 	119A
		CLRF 	1BH 			//0B1C 	119B
		CLRF 	15H 			//0B1D 	1195
		CLRF 	1CH 			//0B1E 	119C
		CLRF 	1DH 			//0B1F 	119D
		MOVLB 	1H 			//0B20 	1021
		LDR 	1AH, 0H 			//0B21 	181A
		IORWI 	80H 			//0B22 	0880
		STR 	1AH 			//0B23 	109A
		RET 					//0B24 	1008
		CLRF 	24H 			//0B25 	11A4
		CLRF 	27H 			//0B26 	11A7
		CLRF 	28H 			//0B27 	11A8
		CLRF 	29H 			//0B28 	11A9
		CLRF 	20H 			//0B29 	11A0
		BCR 	3H, 0H 			//0B2A 	2003
		RLR 	20H, 1H 			//0B2B 	1DA0
		RLR 	21H, 1H 			//0B2C 	1DA1
		RLR 	22H, 1H 			//0B2D 	1DA2
		BCR 	3H, 0H 			//0B2E 	2003
		RLR 	20H, 1H 			//0B2F 	1DA0
		RLR 	21H, 1H 			//0B30 	1DA1
		RLR 	22H, 1H 			//0B31 	1DA2
		BCR 	3H, 0H 			//0B32 	2003
		RLR 	20H, 1H 			//0B33 	1DA0
		RLR 	21H, 1H 			//0B34 	1DA1
		RLR 	22H, 1H 			//0B35 	1DA2
		LDR 	25H, 0H 			//0B36 	1825
		IORWR 	26H, 0H 		//0B37 	1426
		BTSC 	3H, 2H 			//0B38 	2903
		LJUMP 	365H 			//0B39 	3B65
		LDWI 	8H 			//0B3A 	0008
		STR 	2AH 			//0B3B 	10AA
		INCR 	2AH, 1H 		//0B3C 	1AAA
		BTSC 	26H, 7H 		//0B3D 	2BA6
		LJUMP 	344H 			//0B3E 	3B44
		BCR 	3H, 0H 			//0B3F 	2003
		RLR 	24H, 1H 			//0B40 	1DA4
		RLR 	25H, 1H 			//0B41 	1DA5
		RLR 	26H, 1H 			//0B42 	1DA6
		LJUMP 	33CH 			//0B43 	3B3C
		BCR 	3H, 0H 			//0B44 	2003
		RLR 	27H, 1H 			//0B45 	1DA7
		RLR 	28H, 1H 			//0B46 	1DA8
		RLR 	29H, 1H 			//0B47 	1DA9
		LDR 	26H, 0H 			//0B48 	1826
		SUBWR 	22H, 0H 		//0B49 	1222
		BTSS 	3H, 2H 			//0B4A 	2D03
		LJUMP 	352H 			//0B4B 	3B52
		LDR 	25H, 0H 			//0B4C 	1825
		SUBWR 	21H, 0H 		//0B4D 	1221
		BTSS 	3H, 2H 			//0B4E 	2D03
		LJUMP 	352H 			//0B4F 	3B52
		LDR 	24H, 0H 			//0B50 	1824
		SUBWR 	20H, 0H 		//0B51 	1220
		BTSS 	3H, 0H 			//0B52 	2C03
		LJUMP 	35FH 			//0B53 	3B5F
		LDR 	24H, 0H 			//0B54 	1824
		SUBWR 	20H, 1H 		//0B55 	12A0
		LDR 	25H, 0H 			//0B56 	1825
		BTSS 	3H, 0H 			//0B57 	2C03
		INCRSZ 	25H, 0H 		//0B58 	1F25
		SUBWR 	21H, 1H 		//0B59 	12A1
		LDR 	26H, 0H 			//0B5A 	1826
		BTSS 	3H, 0H 			//0B5B 	2C03
		INCRSZ 	26H, 0H 		//0B5C 	1F26
		SUBWR 	22H, 1H 		//0B5D 	12A2
		BSR 	27H, 0H 			//0B5E 	2427
		BCR 	3H, 0H 			//0B5F 	2003
		RRR 	26H, 1H 			//0B60 	1CA6
		RRR 	25H, 1H 			//0B61 	1CA5
		RRR 	24H, 1H 			//0B62 	1CA4
		DECRSZ 	2AH, 1H 		//0B63 	1BAA
		LJUMP 	344H 			//0B64 	3B44
		RET 					//0B65 	1008

		//;sub.c: 73: readKeyNum = TouchKeyScan();
		LCALL 	BBH 			//0B66 	30BB
		MOVLP 	8H 			//0B67 	0188
		LCALL 	59AH 			//0B68 	359A
		MOVLP 	8H 			//0B69 	0188

		//;sub.c: 75: if((SysFunFlg2.OneBits.b7 == 0) && readKeyNum)
		BTSC 	46H, 7H 		//0B6A 	2BC6
		LJUMP 	385H 			//0B6B 	3B85
		LDR 	39H, 0H 			//0B6C 	1839
		IORWR 	3AH, 0H 		//0B6D 	143A
		BTSC 	3H, 2H 			//0B6E 	2903
		LJUMP 	385H 			//0B6F 	3B85

		//;sub.c: 76: {
		//;sub.c: 81: if(readKeyNum&0x4000)
		BTSS 	3AH, 6H 		//0B70 	2F3A
		LJUMP 	374H 			//0B71 	3B74

		//;sub.c: 82: {
		//;sub.c: 83: KeyType=ModeKey;
		LDWI 	2H 			//0B72 	0002
		LJUMP 	384H 			//0B73 	3B84

		//;sub.c: 84: }
		//;sub.c: 85: else if(readKeyNum&0x0080)
		BTSS 	39H, 7H 		//0B74 	2FB9
		LJUMP 	379H 			//0B75 	3B79

		//;sub.c: 86: {
		//;sub.c: 87: KeyType=PowerKey;
		CLRF 	45H 			//0B76 	11C5
		INCR 	45H, 1H 		//0B77 	1AC5

		//;sub.c: 88: }
		LJUMP 	385H 			//0B78 	3B85

		//;sub.c: 89: else if(readKeyNum&0x0800)
		BTSS 	3AH, 3H 		//0B79 	2DBA
		LJUMP 	37DH 			//0B7A 	3B7D

		//;sub.c: 90: {
		//;sub.c: 91: KeyType=ManualKey;
		LDWI 	6H 			//0B7B 	0006
		LJUMP 	384H 			//0B7C 	3B84

		//;sub.c: 92: }
		//;sub.c: 93: else if(readKeyNum&0x2000)
		BTSS 	3AH, 5H 		//0B7D 	2EBA
		LJUMP 	381H 			//0B7E 	3B81

		//;sub.c: 94: {
		//;sub.c: 95: KeyType=StrongKey;
		LDWI 	5H 			//0B7F 	0005
		LJUMP 	384H 			//0B80 	3B84

		//;sub.c: 96: }
		//;sub.c: 97: else if(readKeyNum&0x1000)
		BTSS 	3AH, 4H 		//0B81 	2E3A
		LJUMP 	385H 			//0B82 	3B85

		//;sub.c: 98: {
		//;sub.c: 99: KeyType=HeatKey;
		LDWI 	4H 			//0B83 	0004
		STR 	45H 			//0B84 	10C5

		//;sub.c: 100: }
		//;sub.c: 101: }
		//;sub.c: 102: if((readKeyNum&0x1000)&&(!SysFunFlg0.OneBits.b7))
		BTSC 	3AH, 4H 		//0B85 	2A3A
		BTSC 	49H, 7H 		//0B86 	2BC9
		LJUMP 	397H 			//0B87 	3B97

		//;sub.c: 103: {
		//;sub.c: 104: if(++TmpCnt==500)
		MOVLB 	1H 			//0B88 	1021
		INCR 	41H, 1H 		//0B89 	1AC1
		BTSC 	3H, 2H 			//0B8A 	2903
		INCR 	42H, 1H 		//0B8B 	1AC2
		LDWI 	F4H 			//0B8C 	00F4
		XORWR 	41H, 0H 		//0B8D 	1641
		BTSS 	3H, 2H 			//0B8E 	2D03
		LJUMP 	397H 			//0B8F 	3B97
		DECR 	42H, 0H 		//0B90 	1342
		BTSS 	3H, 2H 			//0B91 	2D03
		LJUMP 	397H 			//0B92 	3B97

		//;sub.c: 105: {
		//;sub.c: 106: SysFunFlg1.OneBits.b2=1;
		MOVLB 	0H 			//0B93 	1020
		BSR 	48H, 2H 			//0B94 	2548

		//;sub.c: 107: KeyType=PowerKey;
		CLRF 	45H 			//0B95 	11C5
		INCR 	45H, 1H 		//0B96 	1AC5

		//;sub.c: 108: }
		//;sub.c: 109: }
		//;sub.c: 111: if(readKeyNum)
		MOVLB 	0H 			//0B97 	1020
		LDR 	39H, 0H 			//0B98 	1839
		IORWR 	3AH, 0H 		//0B99 	143A
		BTSC 	3H, 2H 			//0B9A 	2903
		LJUMP 	3A2H 			//0B9B 	3BA2

		//;sub.c: 112: {SysFunFlg2.OneBits.b7=1;KeyCnt=10;}
		BSR 	46H, 7H 			//0B9C 	27C6
		LDWI 	AH 			//0B9D 	000A
		MOVLB 	1H 			//0B9E 	1021
		STR 	3FH 			//0B9F 	10BF
		CLRF 	40H 			//0BA0 	11C0
		RET 					//0BA1 	1008

		//;sub.c: 113: else
		//;sub.c: 114: {
		//;sub.c: 115: if(KeyCnt)KeyCnt--;
		MOVLB 	1H 			//0BA2 	1021
		LDR 	3FH, 0H 			//0BA3 	183F
		IORWR 	40H, 0H 		//0BA4 	1440
		BTSC 	3H, 2H 			//0BA5 	2903
		LJUMP 	3ACH 			//0BA6 	3BAC
		LDWI 	1H 			//0BA7 	0001
		SUBWR 	3FH, 1H 		//0BA8 	12BF
		LDWI 	0H 			//0BA9 	0000
		SUBWFB 	40H, 1H 		//0BAA 	0BC0
		RET 					//0BAB 	1008

		//;sub.c: 116: else
		//;sub.c: 117: {SysFunFlg2.OneBits.b7=0;TmpCnt=0;}
		MOVLB 	0H 			//0BAC 	1020
		BCR 	46H, 7H 			//0BAD 	23C6
		MOVLB 	1H 			//0BAE 	1021
		CLRF 	41H 			//0BAF 	11C1
		CLRF 	42H 			//0BB0 	11C2
		RET 					//0BB1 	1008

		//;CHIPINIT.C: 671: SysFunFlg0.AllBits = 0;
		MOVLB 	0H 			//0BB2 	1020
		CLRF 	49H 			//0BB3 	11C9

		//;CHIPINIT.C: 672: SysFunFlg1.AllBits = 0;
		CLRF 	48H 			//0BB4 	11C8

		//;CHIPINIT.C: 673: SysFunFlg2.AllBits = 0;
		CLRF 	46H 			//0BB5 	11C6

		//;CHIPINIT.C: 674: TIME_INIT();
		LCALL 	418H 			//0BB6 	3418

		//;CHIPINIT.C: 676: KeyType=NullKey;
		MOVLB 	0H 			//0BB7 	1020
		CLRF 	45H 			//0BB8 	11C5

		//;CHIPINIT.C: 677: DspIndex =DsSty;
		MOVLB 	1H 			//0BB9 	1021
		CLRF 	60H 			//0BBA 	11E0
		RET 					//0BBB 	1008

		//;sub.c: 169: if(SysFunFlg1.OneBits.b2)
		MOVLB 	0H 			//0BBC 	1020
		BTSS 	48H, 2H 		//0BBD 	2D48

		//;sub.c: 171: if(SysFunFlg2.OneBits.b7)
		BTSC 	46H, 7H 		//0BBE 	2BC6
		RET 					//0BBF 	1008

		//;sub.c: 173: if(SysFunFlg0.OneBits.b6)
		BTSC 	49H, 6H 		//0BC0 	2B49
		RET 					//0BC1 	1008

		//;sub.c: 176: if((TimeMin==0)&&(PwrType))
		LDR 	77H, 0H 			//0BC2 	1877
		BTSS 	3H, 2H 			//0BC3 	2D03
		LJUMP 	3CAH 			//0BC4 	3BCA
		LDR 	7BH, 0H 			//0BC5 	187B
		BTSC 	3H, 2H 			//0BC6 	2903
		LJUMP 	3CAH 			//0BC7 	3BCA

		//;sub.c: 177: {
		//;sub.c: 178: KeyType=PowerKey;
		CLRF 	45H 			//0BC8 	11C5
		INCR 	45H, 1H 		//0BC9 	1AC5

		//;sub.c: 179: }
		//;sub.c: 180: if(PwrType)
		LDR 	7BH, 0H 			//0BCA 	187B
		BTSC 	3H, 2H 			//0BCB 	2903

		//;sub.c: 182: if(SysFunFlg0.OneBits.b7)
		BTSS 	49H, 7H 		//0BCC 	2FC9
		RET 					//0BCD 	1008

		//;sub.c: 183: {
		//;sub.c: 184: User_init();
		LJUMP 	3B2H 			//0BCE 	3BB2

		//;CHIPINIT.C: 92: OSCCON = 0B01100001;
		LDWI 	61H 			//0BCF 	0061
		MOVLB 	1H 			//0BD0 	1021
		STR 	19H 			//0BD1 	1099

		//;CHIPINIT.C: 103: INTCON = 0B00000000;
		CLRF 	BH 			//0BD2 	118B

		//;CHIPINIT.C: 105: PORTA = 0B00000000;
		MOVLB 	0H 			//0BD3 	1020
		CLRF 	CH 			//0BD4 	118C

		//;CHIPINIT.C: 106: TRISA = 0B00000000;
		MOVLB 	1H 			//0BD5 	1021
		CLRF 	CH 			//0BD6 	118C

		//;CHIPINIT.C: 107: PORTB = 0B00000000;
		MOVLB 	0H 			//0BD7 	1020
		CLRF 	DH 			//0BD8 	118D

		//;CHIPINIT.C: 108: TRISB = 0B00010000;
		LDWI 	10H 			//0BD9 	0010
		MOVLB 	1H 			//0BDA 	1021
		STR 	DH 			//0BDB 	108D

		//;CHIPINIT.C: 109: PORTC = 0B00000000;
		MOVLB 	0H 			//0BDC 	1020
		CLRF 	EH 			//0BDD 	118E

		//;CHIPINIT.C: 110: TRISC = 0B00000000;
		MOVLB 	1H 			//0BDE 	1021
		CLRF 	EH 			//0BDF 	118E

		//;CHIPINIT.C: 111: PORTD = 0B00000000;
		MOVLB 	0H 			//0BE0 	1020
		CLRF 	FH 			//0BE1 	118F

		//;CHIPINIT.C: 112: TRISD = 0B00000000;
		MOVLB 	1H 			//0BE2 	1021
		CLRF 	FH 			//0BE3 	118F

		//;CHIPINIT.C: 114: WPUA = 0B00000000;
		MOVLB 	3H 			//0BE4 	1023
		CLRF 	CH 			//0BE5 	118C

		//;CHIPINIT.C: 115: WPUB = 0B00010000;
		STR 	DH 			//0BE6 	108D

		//;CHIPINIT.C: 116: WPUC = 0B00000000;
		CLRF 	EH 			//0BE7 	118E

		//;CHIPINIT.C: 117: WPUD = 0B00000000;
		CLRF 	FH 			//0BE8 	118F

		//;CHIPINIT.C: 119: WPDA = 0B00000000;
		MOVLB 	4H 			//0BE9 	1024
		CLRF 	CH 			//0BEA 	118C

		//;CHIPINIT.C: 120: WPDB = 0B00000000;
		CLRF 	DH 			//0BEB 	118D

		//;CHIPINIT.C: 121: WPDC = 0B00000000;
		CLRF 	EH 			//0BEC 	118E

		//;CHIPINIT.C: 122: WPDD = 0B00000000;
		CLRF 	FH 			//0BED 	118F

		//;CHIPINIT.C: 124: PSRC0 = 0B11111111;
		LDWI 	FFH 			//0BEE 	00FF
		MOVLB 	2H 			//0BEF 	1022
		STR 	1AH 			//0BF0 	109A

		//;CHIPINIT.C: 128: PSRC1 = 0B11111111;
		STR 	1BH 			//0BF1 	109B

		//;CHIPINIT.C: 132: PSINK0 = 0B11111111;
		MOVLB 	3H 			//0BF2 	1023
		STR 	1AH 			//0BF3 	109A

		//;CHIPINIT.C: 133: PSINK1 = 0B11111111;
		STR 	1BH 			//0BF4 	109B

		//;CHIPINIT.C: 134: PSINK2 = 0B11111111;
		STR 	1CH 			//0BF5 	109C

		//;CHIPINIT.C: 135: PSINK3 = 0B11111111;
		STR 	1DH 			//0BF6 	109D

		//;CHIPINIT.C: 137: ANSELA = 0B00000000;
		CLRF 	17H 			//0BF7 	1197
		RET 					//0BF8 	1008

		//;CHIPINIT.C: 482: PCKEN |=0B00000100;
		BSR 	1AH, 2H 			//0BF9 	251A

		//;CHIPINIT.C: 483: CKOCON=0B00100000;
		LDWI 	20H 			//0BFA 	0020
		STR 	15H 			//0BFB 	1095

		//;CHIPINIT.C: 484: TCKSRC|=0B00010000;
		MOVLB 	6H 			//0BFC 	1026
		BSR 	1FH, 4H 			//0BFD 	261F

		//;CHIPINIT.C: 485: TIM2CR1 =0B10000101;
		LDWI 	85H 			//0BFE 	0085
		STR 	CH 			//0BFF 	108C

		//;CHIPINIT.C: 486: TIM2IER =0B00000001;
		LDWI 	1H 			//0C00 	0001
		STR 	DH 			//0C01 	108D

		//;CHIPINIT.C: 489: TIM2ARRH =0x06;
		LDWI 	6H 			//0C02 	0006
		STR 	19H 			//0C03 	1099

		//;CHIPINIT.C: 490: TIM2ARRL =0x40;
		LDWI 	40H 			//0C04 	0040
		STR 	1AH 			//0C05 	109A
		RET 					//0C06 	1008

		//;CHIPINIT.C: 502: PCKEN |=0B00001000;
		MOVLB 	1H 			//0C07 	1021
		BSR 	1AH, 3H 			//0C08 	259A

		//;CHIPINIT.C: 506: TIM4CR1 =0B00000101;
		LDWI 	5H 			//0C09 	0005
		MOVLB 	2H 			//0C0A 	1022
		STR 	11H 			//0C0B 	1091

		//;CHIPINIT.C: 537: TIM4IER =0B00000001;
		LDWI 	1H 			//0C0C 	0001
		STR 	12H 			//0C0D 	1092

		//;CHIPINIT.C: 540: TIM4SR =0B00000000;
		CLRF 	13H 			//0C0E 	1193

		//;CHIPINIT.C: 549: TIM4EGR =0B00000000;
		CLRF 	14H 			//0C0F 	1194

		//;CHIPINIT.C: 554: TIM4CNTR=0;
		CLRF 	15H 			//0C10 	1195

		//;CHIPINIT.C: 556: TIM4PSCR=0B00000110;
		LDWI 	6H 			//0C11 	0006
		STR 	16H 			//0C12 	1096

		//;CHIPINIT.C: 560: TIM4ARR =124;
		LDWI 	7CH 			//0C13 	007C
		STR 	17H 			//0C14 	1097

		//;CHIPINIT.C: 561: TIM4ARR =22;
		LDWI 	16H 			//0C15 	0016
		STR 	17H 			//0C16 	1097
		RET 					//0C17 	1008

		//;CHIPINIT.C: 663: Time100Ms=0;
		MOVLB 	1H 			//0C18 	1021
		CLRF 	5CH 			//0C19 	11DC

		//;CHIPINIT.C: 664: TimeSec=0;
		CLRF 	5BH 			//0C1A 	11DB
		RET 					//0C1B 	1008
		MOVLB 	0H 			//0C1C 	1020
		ADDWR 	21H, 1H 		//0C1D 	17A1
		BTSC 	3H, 0H 			//0C1E 	2803
		INCR 	22H, 1H 		//0C1F 	1AA2
		MOVLB 	1FH 			//0C20 	103F
		LDR 	2DH, 0H 			//0C21 	182D
		MOVLB 	0H 			//0C22 	1020
		STR 	26H 			//0C23 	10A6
		CLRF 	25H 			//0C24 	11A5
		MOVLB 	1FH 			//0C25 	103F
		LDR 	2CH, 0H 			//0C26 	182C
		MOVLB 	0H 			//0C27 	1020
		ADDWR 	25H, 1H 		//0C28 	17A5
		RET 					//0C29 	1008
		MOVLB 	0H 			//0C2A 	1020
		LDR 	28H, 0H 			//0C2B 	1828
		STR 	32H 			//0C2C 	10B2
		LDR 	27H, 0H 			//0C2D 	1827
		STR 	31H 			//0C2E 	10B1
		LDR 	4BH, 0H 			//0C2F 	184B
		STR 	4DH 			//0C30 	10CD
		LDWI 	6H 			//0C31 	0006
		CLRF 	4EH 			//0C32 	11CE
		LSLF 	4DH, 1H 		//0C33 	05CD
		RLR 	4EH, 1H 			//0C34 	1DCE
		LSLF 	4DH, 1H 		//0C35 	05CD
		RLR 	4EH, 1H 			//0C36 	1DCE
		LSLF 	4DH, 1H 		//0C37 	05CD
		RLR 	4EH, 1H 			//0C38 	1DCE
		STR 	4FH 			//0C39 	10CF
		LDWI 	23H 			//0C3A 	0023
		STR 	50H 			//0C3B 	10D0
		LDR 	4DH, 0H 			//0C3C 	184D
		ADDWR 	4FH, 0H 		//0C3D 	174F
		STR 	6H 			//0C3E 	1086
		LDR 	4EH, 0H 			//0C3F 	184E
		ADDWFC 	50H, 0H 		//0C40 	0D50
		STR 	7H 			//0C41 	1087
		LDR 	31H, 0H 			//0C42 	1831
		MOVWI 	0H[1] 			//0C43 	0FC0
		LDR 	32H, 0H 			//0C44 	1832
		MOVWI 	1H[1] 			//0C45 	0FC1
		RET 					//0C46 	1008
		MOVLB 	0H 			//0C47 	1020
		LDR 	28H, 0H 			//0C48 	1828
		STR 	2EH 			//0C49 	10AE
		LDR 	27H, 0H 			//0C4A 	1827
		STR 	2DH 			//0C4B 	10AD
		LDR 	4BH, 0H 			//0C4C 	184B
		STR 	67H 			//0C4D 	10E7
		LDWI 	2H 			//0C4E 	0002
		CLRF 	68H 			//0C4F 	11E8
		LSLF 	67H, 1H 		//0C50 	05E7
		RLR 	68H, 1H 			//0C51 	1DE8
		LSLF 	67H, 1H 		//0C52 	05E7
		RLR 	68H, 1H 			//0C53 	1DE8
		LSLF 	67H, 1H 		//0C54 	05E7
		RLR 	68H, 1H 			//0C55 	1DE8
		STR 	69H 			//0C56 	10E9
		LDWI 	23H 			//0C57 	0023
		STR 	6AH 			//0C58 	10EA
		LDR 	67H, 0H 			//0C59 	1867
		ADDWR 	69H, 0H 		//0C5A 	1769
		STR 	6H 			//0C5B 	1086
		LDR 	68H, 0H 			//0C5C 	1868
		ADDWFC 	6AH, 0H 		//0C5D 	0D6A
		STR 	7H 			//0C5E 	1087
		LDR 	2DH, 0H 			//0C5F 	182D
		MOVWI 	0H[1] 			//0C60 	0FC0
		LDR 	2EH, 0H 			//0C61 	182E
		MOVWI 	1H[1] 			//0C62 	0FC1
		RET 					//0C63 	1008
		MOVLB 	0H 			//0C64 	1020
		LDR 	28H, 0H 			//0C65 	1828
		STR 	2CH 			//0C66 	10AC
		LDR 	27H, 0H 			//0C67 	1827
		STR 	2BH 			//0C68 	10AB
		LDR 	4BH, 0H 			//0C69 	184B
		STR 	67H 			//0C6A 	10E7
		LDWI 	23H 			//0C6B 	0023
		CLRF 	68H 			//0C6C 	11E8
		LSLF 	67H, 1H 		//0C6D 	05E7
		RLR 	68H, 1H 			//0C6E 	1DE8
		LSLF 	67H, 1H 		//0C6F 	05E7
		RLR 	68H, 1H 			//0C70 	1DE8
		LSLF 	67H, 1H 		//0C71 	05E7
		RLR 	68H, 1H 			//0C72 	1DE8
		CLRF 	69H 			//0C73 	11E9
		STR 	6AH 			//0C74 	10EA
		LDR 	67H, 0H 			//0C75 	1867
		ADDWR 	69H, 0H 		//0C76 	1769
		STR 	6H 			//0C77 	1086
		LDR 	68H, 0H 			//0C78 	1868
		ADDWFC 	6AH, 0H 		//0C79 	0D6A
		STR 	7H 			//0C7A 	1087
		LDR 	2BH, 0H 			//0C7B 	182B
		MOVWI 	0H[1] 			//0C7C 	0FC0
		LDR 	2CH, 0H 			//0C7D 	182C
		MOVWI 	1H[1] 			//0C7E 	0FC1
		RET 					//0C7F 	1008
		LDWI 	4H 			//0C80 	0004
		MOVLB 	7H 			//0C81 	1027
		STR 	13H 			//0C82 	1093
		MOVLB 	0H 			//0C83 	1020
		LDR 	4BH, 0H 			//0C84 	184B
		STR 	67H 			//0C85 	10E7
		RRR 	67H, 1H 			//0C86 	1CE7
		RRR 	67H, 1H 			//0C87 	1CE7
		RRR 	67H, 0H 			//0C88 	1C67
		ANDWI 	C0H 			//0C89 	09C0
		IORWI 	3H 			//0C8A 	0803
		MOVLB 	7H 			//0C8B 	1027
		STR 	18H 			//0C8C 	1098
		LDWI 	1H 			//0C8D 	0001
		MOVLB 	0H 			//0C8E 	1020
		STR 	67H 			//0C8F 	10E7
		LDR 	4BH, 0H 			//0C90 	184B
		ADDWI 	FCH 			//0C91 	0EFC
		INCR 	9H, 1H 			//0C92 	1A89
		LJUMP 	495H 			//0C93 	3C95
		LSLF 	67H, 1H 		//0C94 	05E7
		DECRSZ 	9H, 1H 		//0C95 	1B89
		LJUMP 	494H 			//0C96 	3C94
		LDR 	67H, 0H 			//0C97 	1867
		IORWI 	10H 			//0C98 	0810
		MOVLB 	7H 			//0C99 	1027
		STR 	19H 			//0C9A 	1099
		LDWI 	4H 			//0C9B 	0004
		STR 	15H 			//0C9C 	1095
		LDWI 	C3H 			//0C9D 	00C3
		STR 	1CH 			//0C9E 	109C
		LDWI 	18H 			//0C9F 	0018
		STR 	1DH 			//0CA0 	109D
		BSR 	DH, 5H 			//0CA1 	268D
		BTSS 	DH, 6H 			//0CA2 	2F0D
		LJUMP 	4A2H 			//0CA3 	3CA2
		MOVLB 	1FH 			//0CA4 	103F
		LDR 	1DH, 0H 			//0CA5 	181D
		MOVLB 	0H 			//0CA6 	1020
		STR 	22H 			//0CA7 	10A2
		CLRF 	21H 			//0CA8 	11A1
		MOVLB 	1FH 			//0CA9 	103F
		LDR 	1CH, 0H 			//0CAA 	181C
		RET 					//0CAB 	1008
		LDWI 	4H 			//0CAC 	0004
		MOVLB 	7H 			//0CAD 	1027
		STR 	12H 			//0CAE 	1092
		MOVLB 	0H 			//0CAF 	1020
		LDR 	4BH, 0H 			//0CB0 	184B
		STR 	67H 			//0CB1 	10E7
		RRR 	67H, 1H 			//0CB2 	1CE7
		RRR 	67H, 1H 			//0CB3 	1CE7
		RRR 	67H, 0H 			//0CB4 	1C67
		ANDWI 	C0H 			//0CB5 	09C0
		IORWI 	3H 			//0CB6 	0803
		MOVLB 	7H 			//0CB7 	1027
		STR 	16H 			//0CB8 	1096
		LDWI 	1H 			//0CB9 	0001
		MOVLB 	0H 			//0CBA 	1020
		STR 	67H 			//0CBB 	10E7
		INCR 	4BH, 0H 		//0CBC 	1A4B
		LJUMP 	4BFH 			//0CBD 	3CBF
		LSLF 	67H, 1H 		//0CBE 	05E7
		DECRSZ 	9H, 1H 		//0CBF 	1B89
		LJUMP 	4BEH 			//0CC0 	3CBE
		LDR 	67H, 0H 			//0CC1 	1867
		IORWI 	10H 			//0CC2 	0810
		MOVLB 	7H 			//0CC3 	1027
		STR 	17H 			//0CC4 	1097
		LDWI 	4H 			//0CC5 	0004
		STR 	15H 			//0CC6 	1095
		LDWI 	C3H 			//0CC7 	00C3
		STR 	1CH 			//0CC8 	109C
		LDWI 	18H 			//0CC9 	0018
		STR 	1DH 			//0CCA 	109D
		BSR 	DH, 5H 			//0CCB 	268D
		BTSS 	DH, 6H 			//0CCC 	2F0D
		LJUMP 	4CCH 			//0CCD 	3CCC
		MOVLB 	1FH 			//0CCE 	103F
		LDR 	15H, 0H 			//0CCF 	1815
		MOVLB 	0H 			//0CD0 	1020
		STR 	22H 			//0CD1 	10A2
		CLRF 	21H 			//0CD2 	11A1
		MOVLB 	1FH 			//0CD3 	103F
		LDR 	14H, 0H 			//0CD4 	1814
		RET 					//0CD5 	1008
		MOVLB 	7H 			//0CD6 	1027
		STR 	14H 			//0CD7 	1094
		STR 	15H 			//0CD8 	1095
		CLRF 	1AH 			//0CD9 	119A
		LDWI 	90H 			//0CDA 	0090
		STR 	1BH 			//0CDB 	109B
		MOVLB 	0H 			//0CDC 	1020
		LDR 	4BH, 0H 			//0CDD 	184B
		STR 	67H 			//0CDE 	10E7
		RRR 	67H, 1H 			//0CDF 	1CE7
		RRR 	67H, 1H 			//0CE0 	1CE7
		RRR 	67H, 0H 			//0CE1 	1C67
		ANDWI 	C0H 			//0CE2 	09C0
		MOVLB 	7H 			//0CE3 	1027
		STR 	1CH 			//0CE4 	109C
		LDWI 	1H 			//0CE5 	0001
		MOVLB 	0H 			//0CE6 	1020
		STR 	67H 			//0CE7 	10E7
		LDR 	4BH, 0H 			//0CE8 	184B
		ADDWI 	F4H 			//0CE9 	0EF4
		INCR 	9H, 1H 			//0CEA 	1A89
		LJUMP 	4EDH 			//0CEB 	3CED
		LSLF 	67H, 1H 		//0CEC 	05E7
		DECRSZ 	9H, 1H 		//0CED 	1B89
		LJUMP 	4ECH 			//0CEE 	3CEC
		LDR 	67H, 0H 			//0CEF 	1867
		IORWI 	98H 			//0CF0 	0898
		MOVLB 	7H 			//0CF1 	1027
		STR 	1DH 			//0CF2 	109D
		BSR 	DH, 5H 			//0CF3 	268D
		BTSS 	DH, 6H 			//0CF4 	2F0D
		LJUMP 	4F4H 			//0CF5 	3CF4
		MOVLB 	1FH 			//0CF6 	103F
		LDR 	25H, 0H 			//0CF7 	1825
		MOVLB 	0H 			//0CF8 	1020
		STR 	22H 			//0CF9 	10A2
		CLRF 	21H 			//0CFA 	11A1
		MOVLB 	1FH 			//0CFB 	103F
		LDR 	24H, 0H 			//0CFC 	1824
		RET 					//0CFD 	1008
		MOVLB 	0H 			//0CFE 	1020
		LDR 	51H, 0H 			//0CFF 	1851
		STR 	4DH 			//0D00 	10CD
		RRR 	4DH, 1H 			//0D01 	1CCD
		RRR 	4DH, 1H 			//0D02 	1CCD
		RRR 	4DH, 0H 			//0D03 	1C4D
		ANDWI 	C0H 			//0D04 	09C0
		RET 					//0D05 	1008
		MOVLB 	0H 			//0D06 	1020
		LDR 	4BH, 0H 			//0D07 	184B
		STR 	67H 			//0D08 	10E7
		RRR 	67H, 1H 			//0D09 	1CE7
		RRR 	67H, 1H 			//0D0A 	1CE7
		RRR 	67H, 0H 			//0D0B 	1C67
		ANDWI 	C0H 			//0D0C 	09C0
		MOVLB 	7H 			//0D0D 	1027
		STR 	1AH 			//0D0E 	109A
		LDWI 	1H 			//0D0F 	0001
		MOVLB 	0H 			//0D10 	1020
		STR 	67H 			//0D11 	10E7
		LDR 	4BH, 0H 			//0D12 	184B
		ADDWI 	F8H 			//0D13 	0EF8
		INCR 	9H, 1H 			//0D14 	1A89
		LJUMP 	517H 			//0D15 	3D17
		LSLF 	67H, 1H 		//0D16 	05E7
		DECRSZ 	9H, 1H 		//0D17 	1B89
		LJUMP 	516H 			//0D18 	3D16
		LDR 	67H, 0H 			//0D19 	1867
		IORWI 	10H 			//0D1A 	0810
		MOVLB 	7H 			//0D1B 	1027
		STR 	1BH 			//0D1C 	109B
		RET 					//0D1D 	1008
		STR 	15H 			//0D1E 	1095
		LDWI 	C3H 			//0D1F 	00C3
		STR 	1CH 			//0D20 	109C
		LDWI 	18H 			//0D21 	0018
		STR 	1DH 			//0D22 	109D
		MOVLB 	0H 			//0D23 	1020
		DECR 	3CH, 0H 		//0D24 	133C
		RET 					//0D25 	1008
		LDWI 	C0H 			//0D26 	00C0
		STR 	1CH 			//0D27 	109C
		LDWI 	18H 			//0D28 	0018
		STR 	1DH 			//0D29 	109D
		BSR 	DH, 5H 			//0D2A 	268D
		BTSS 	DH, 6H 			//0D2B 	2F0D
		LJUMP 	52BH 			//0D2C 	3D2B
		MOVLB 	1FH 			//0D2D 	103F
		LDR 	25H, 0H 			//0D2E 	1825
		MOVLB 	0H 			//0D2F 	1020
		STR 	22H 			//0D30 	10A2
		CLRF 	21H 			//0D31 	11A1
		MOVLB 	1FH 			//0D32 	103F
		LDR 	24H, 0H 			//0D33 	1824
		RET 					//0D34 	1008
		MOVLB 	7H 			//0D35 	1027
		STR 	18H 			//0D36 	1098
		LDWI 	1H 			//0D37 	0001
		MOVLB 	0H 			//0D38 	1020
		STR 	4DH 			//0D39 	10CD
		LDR 	51H, 0H 			//0D3A 	1851
		ADDWI 	FCH 			//0D3B 	0EFC
		INCR 	9H, 1H 			//0D3C 	1A89
		LJUMP 	53FH 			//0D3D 	3D3F
		LSLF 	4DH, 1H 		//0D3E 	05CD
		DECRSZ 	9H, 1H 		//0D3F 	1B89
		LJUMP 	53EH 			//0D40 	3D3E
		LDR 	4DH, 0H 			//0D41 	184D
		IORWI 	10H 			//0D42 	0810
		MOVLB 	7H 			//0D43 	1027
		STR 	19H 			//0D44 	1099
		RETW 	4H 			//0D45 	0404
		MOVLB 	7H 			//0D46 	1027
		STR 	1CH 			//0D47 	109C
		LDWI 	1H 			//0D48 	0001
		MOVLB 	0H 			//0D49 	1020
		STR 	4DH 			//0D4A 	10CD
		LDR 	51H, 0H 			//0D4B 	1851
		ADDWI 	F4H 			//0D4C 	0EF4
		INCR 	9H, 1H 			//0D4D 	1A89
		LJUMP 	550H 			//0D4E 	3D50
		LSLF 	4DH, 1H 		//0D4F 	05CD
		DECRSZ 	9H, 1H 		//0D50 	1B89
		LJUMP 	54FH 			//0D51 	3D4F
		LDR 	4DH, 0H 			//0D52 	184D
		IORWI 	98H 			//0D53 	0898
		MOVLB 	7H 			//0D54 	1027
		STR 	1DH 			//0D55 	109D
		MOVLB 	0H 			//0D56 	1020
		DECR 	3CH, 0H 		//0D57 	133C
		RET 					//0D58 	1008
		MOVLB 	7H 			//0D59 	1027
		STR 	1AH 			//0D5A 	109A
		LDWI 	1H 			//0D5B 	0001
		MOVLB 	0H 			//0D5C 	1020
		STR 	4DH 			//0D5D 	10CD
		LDR 	51H, 0H 			//0D5E 	1851
		ADDWI 	F8H 			//0D5F 	0EF8
		INCR 	9H, 1H 			//0D60 	1A89
		LJUMP 	563H 			//0D61 	3D63
		LSLF 	4DH, 1H 		//0D62 	05CD
		DECRSZ 	9H, 1H 		//0D63 	1B89
		LJUMP 	562H 			//0D64 	3D62
		LDR 	4DH, 0H 			//0D65 	184D
		IORWI 	10H 			//0D66 	0810
		MOVLB 	7H 			//0D67 	1027
		STR 	1BH 			//0D68 	109B
		RETW 	4H 			//0D69 	0404
		MOVLB 	7H 			//0D6A 	1027
		STR 	16H 			//0D6B 	1096
		LDWI 	1H 			//0D6C 	0001
		MOVLB 	0H 			//0D6D 	1020
		STR 	4DH 			//0D6E 	10CD
		INCR 	51H, 0H 		//0D6F 	1A51
		LJUMP 	572H 			//0D70 	3D72
		LSLF 	4DH, 1H 		//0D71 	05CD
		DECRSZ 	9H, 1H 		//0D72 	1B89
		LJUMP 	571H 			//0D73 	3D71
		LDR 	4DH, 0H 			//0D74 	184D
		IORWI 	10H 			//0D75 	0810
		MOVLB 	7H 			//0D76 	1027
		STR 	17H 			//0D77 	1097
		RETW 	4H 			//0D78 	0404
		LDWI 	90H 			//0D79 	0090
		STR 	1BH 			//0D7A 	109B
		LDWI 	4H 			//0D7B 	0004
		STR 	15H 			//0D7C 	1095
		RET 					//0D7D 	1008
		STR 	1CH 			//0D7E 	109C
		LDWI 	18H 			//0D7F 	0018
		STR 	1DH 			//0D80 	109D
		MOVLB 	0H 			//0D81 	1020
		DECR 	3CH, 0H 		//0D82 	133C
		RET 					//0D83 	1008
		MOVLB 	0H 			//0D84 	1020
		STR 	22H 			//0D85 	10A2
		CLRF 	21H 			//0D86 	11A1
		MOVLB 	1FH 			//0D87 	103F
		LDR 	24H, 0H 			//0D88 	1824
		RET 					//0D89 	1008
		MOVLB 	7H 			//0D8A 	1027
		CLRF 	FH 			//0D8B 	118F
		CLRF 	10H 			//0D8C 	1190
		CLRF 	11H 			//0D8D 	1191
		RETW 	3H 			//0D8E 	0403
		MOVLB 	7H 			//0D8F 	1027
		CLRF 	FH 			//0D90 	118F
		CLRF 	10H 			//0D91 	1190
		CLRF 	11H 			//0D92 	1191
		RETW 	3EH 			//0D93 	043E
		LDR 	34H, 0H 			//0D94 	1834
		STR 	68H 			//0D95 	10E8
		LDR 	33H, 0H 			//0D96 	1833
		STR 	67H 			//0D97 	10E7
		INCR 	3EH, 0H 		//0D98 	1A3E
		RET 					//0D99 	1008
		LDR 	6CH, 0H 			//0D9A 	186C
		STR 	3AH 			//0D9B 	10BA
		LDR 	6BH, 0H 			//0D9C 	186B
		STR 	39H 			//0D9D 	10B9
		RET 					//0D9E 	1008

		//;sub.c: 122: if(KeyType==NullKey)return;
		MOVLB 	0H 			//0D9F 	1020
		LDR 	45H, 0H 			//0DA0 	1845
		BTSC 	3H, 2H 			//0DA1 	2903
		RET 					//0DA2 	1008
		LJUMP 	5B3H 			//0DA3 	3DB3

		//;sub.c: 127: PowerKeyExec();
		LCALL 	62AH 			//0DA4 	362A
		MOVLP 	8H 			//0DA5 	0188

		//;sub.c: 128: break;
		LJUMP 	5CAH 			//0DA6 	3DCA

		//;sub.c: 130: ModeKeyExec();
		LCALL 	60EH 			//0DA7 	360E
		MOVLP 	8H 			//0DA8 	0188

		//;sub.c: 131: break;
		LJUMP 	5CAH 			//0DA9 	3DCA

		//;sub.c: 136: HeatKeyExec();
		LCALL 	5D9H 			//0DAA 	35D9
		MOVLP 	8H 			//0DAB 	0188

		//;sub.c: 137: break;
		LJUMP 	5CAH 			//0DAC 	3DCA

		//;sub.c: 139: StrongKeyExec();
		LCALL 	659H 			//0DAD 	3659
		MOVLP 	8H 			//0DAE 	0188

		//;sub.c: 140: break;
		LJUMP 	5CAH 			//0DAF 	3DCA

		//;sub.c: 142: ManualKeyExec();
		LCALL 	5EAH 			//0DB0 	35EA
		MOVLP 	8H 			//0DB1 	0188

		//;sub.c: 143: break;
		LJUMP 	5CAH 			//0DB2 	3DCA
		LDR 	45H, 0H 			//0DB3 	1845
		XORWI 	1H 			//0DB4 	0A01
		BTSC 	3H, 2H 			//0DB5 	2903
		LJUMP 	5A4H 			//0DB6 	3DA4
		XORWI 	3H 			//0DB7 	0A03
		BTSC 	3H, 2H 			//0DB8 	2903
		LJUMP 	5A7H 			//0DB9 	3DA7
		XORWI 	1H 			//0DBA 	0A01
		BTSC 	3H, 2H 			//0DBB 	2903
		LJUMP 	5CAH 			//0DBC 	3DCA
		XORWI 	7H 			//0DBD 	0A07
		BTSC 	3H, 2H 			//0DBE 	2903
		LJUMP 	5AAH 			//0DBF 	3DAA
		XORWI 	1H 			//0DC0 	0A01
		BTSC 	3H, 2H 			//0DC1 	2903
		LJUMP 	5ADH 			//0DC2 	3DAD
		XORWI 	3H 			//0DC3 	0A03
		BTSC 	3H, 2H 			//0DC4 	2903
		LJUMP 	5B0H 			//0DC5 	3DB0
		XORWI 	1H 			//0DC6 	0A01
		BTSC 	3H, 2H 			//0DC7 	2903
		LJUMP 	5CAH 			//0DC8 	3DCA
		LJUMP 	5CAH 			//0DC9 	3DCA

		//;sub.c: 151: if((KeyType!=PowerKey)&&(!SysFunFlg0.OneBits.b7))
		MOVLB 	0H 			//0DCA 	1020
		DECR 	45H, 0H 		//0DCB 	1345
		BTSC 	3H, 2H 			//0DCC 	2903
		LJUMP 	5D0H 			//0DCD 	3DD0
		BTSS 	49H, 7H 		//0DCE 	2FC9
		RET 					//0DCF 	1008

		//;sub.c: 155: else
		//;sub.c: 156: {
		//;sub.c: 157: {BuzzerCnt=400;};
		LCALL 	67EH 			//0DD0 	367E
		MOVLP 	8H 			//0DD1 	0188

		//;sub.c: 158: refresh_data();
		LCALL 	665H 			//0DD2 	3665

		//;sub.c: 159: }
		//;sub.c: 161: KeyType=NullKey;
		MOVLB 	0H 			//0DD3 	1020
		LDWI 	FAH 			//0DD4 	00FA
		CLRF 	45H 			//0DD5 	11C5

		//;sub.c: 163: dsp_100ms=250;
		MOVLB 	1H 			//0DD6 	1021
		STR 	52H 			//0DD7 	10D2
		RET 					//0DD8 	1008

		//;main.c: 102: if(SysFunFlg0.OneBits.b7)
		MOVLB 	0H 			//0DD9 	1020
		BTSS 	49H, 7H 		//0DDA 	2FC9
		RET 					//0DDB 	1008

		//;main.c: 103: {
		//;main.c: 104: if(HeatType >= Mode1)
		MOVLB 	1H 			//0DDC 	1021
		LDR 	64H, 0H 			//0DDD 	1864
		BTSC 	3H, 2H 			//0DDE 	2903
		LJUMP 	5E2H 			//0DDF 	3DE2

		//;main.c: 105: {
		//;main.c: 106: HeatType = Mode0;
		CLRF 	64H 			//0DE0 	11E4

		//;main.c: 107: }
		LJUMP 	5E3H 			//0DE1 	3DE3

		//;main.c: 108: else HeatType+=1;
		INCR 	64H, 1H 		//0DE2 	1AE4

		//;main.c: 110: if(PwrType==1)PwrType=2;
		DECRSZ 	7BH, 0H 		//0DE3 	1B7B
		LJUMP 	5E7H 			//0DE4 	3DE7
		LDWI 	2H 			//0DE5 	0002
		STR 	7BH 			//0DE6 	10FB

		//;main.c: 112: DspIndex =DsHeat;
		LDWI 	3H 			//0DE7 	0003
		STR 	60H 			//0DE8 	10E0
		RET 					//0DE9 	1008

		//;main.c: 79: if(SysFunFlg0.OneBits.b7)
		MOVLB 	0H 			//0DEA 	1020
		BTSS 	49H, 7H 		//0DEB 	2FC9
		RET 					//0DEC 	1008

		//;main.c: 80: {
		//;main.c: 81: if(ManualType==0)
		LDR 	78H, 0H 			//0DED 	1878
		BTSS 	3H, 2H 			//0DEE 	2D03
		LJUMP 	5FEH 			//0DEF 	3DFE

		//;main.c: 82: {
		//;main.c: 83: if(LastManual==0){LastManual=1; HeatType=1;}
		MOVLB 	1H 			//0DF0 	1021
		LDR 	62H, 0H 			//0DF1 	1862
		BTSS 	3H, 2H 			//0DF2 	2D03
		LJUMP 	5F8H 			//0DF3 	3DF8
		CLRF 	62H 			//0DF4 	11E2
		INCR 	62H, 1H 		//0DF5 	1AE2
		CLRF 	64H 			//0DF6 	11E4
		INCR 	64H, 1H 		//0DF7 	1AE4

		//;main.c: 84: ManualType=LastManual;
		LDR 	62H, 0H 			//0DF8 	1862
		STR 	78H 			//0DF9 	10F8

		//;main.c: 86: LastMode=ModeType;
		LDR 	7AH, 0H 			//0DFA 	187A
		STR 	63H 			//0DFB 	10E3

		//;main.c: 87: ModeType=0;
		CLRF 	7AH 			//0DFC 	11FA

		//;main.c: 88: }
		LJUMP 	609H 			//0DFD 	3E09

		//;main.c: 89: else if(ManualType >= 3)
		LDWI 	3H 			//0DFE 	0003
		SUBWR 	78H, 0H 		//0DFF 	1278
		BTSS 	3H, 0H 			//0E00 	2C03
		LJUMP 	608H 			//0E01 	3E08

		//;main.c: 90: {
		//;main.c: 91: ManualType = 1; HeatType=1;
		CLRF 	78H 			//0E02 	11F8
		INCR 	78H, 1H 		//0E03 	1AF8
		MOVLB 	1H 			//0E04 	1021
		CLRF 	64H 			//0E05 	11E4
		INCR 	64H, 1H 		//0E06 	1AE4

		//;main.c: 92: }
		LJUMP 	609H 			//0E07 	3E09

		//;main.c: 93: else ManualType+=1;
		INCR 	78H, 1H 		//0E08 	1AF8
		LDWI 	4H 			//0E09 	0004

		//;main.c: 95: ModeType=0;
		CLRF 	7AH 			//0E0A 	11FA

		//;main.c: 97: DspIndex =DsManual;
		MOVLB 	1H 			//0E0B 	1021
		STR 	60H 			//0E0C 	10E0
		RET 					//0E0D 	1008

		//;main.c: 56: if(SysFunFlg0.OneBits.b7)
		MOVLB 	0H 			//0E0E 	1020
		BTSS 	49H, 7H 		//0E0F 	2FC9
		RET 					//0E10 	1008

		//;main.c: 57: {
		//;main.c: 58: if(PwrType==1)PwrType=2;
		DECRSZ 	7BH, 0H 		//0E11 	1B7B
		LJUMP 	615H 			//0E12 	3E15
		LDWI 	2H 			//0E13 	0002
		STR 	7BH 			//0E14 	10FB

		//;main.c: 60: if(ModeType==0)
		LDR 	7AH, 0H 			//0E15 	187A
		BTSS 	3H, 2H 			//0E16 	2D03
		LJUMP 	624H 			//0E17 	3E24

		//;main.c: 61: {
		//;main.c: 62: if(LastMode==0)LastMode=1;
		MOVLB 	1H 			//0E18 	1021
		LDR 	63H, 0H 			//0E19 	1863
		BTSS 	3H, 2H 			//0E1A 	2D03
		LJUMP 	61EH 			//0E1B 	3E1E
		CLRF 	63H 			//0E1C 	11E3
		INCR 	63H, 1H 		//0E1D 	1AE3

		//;main.c: 63: ModeType=LastMode;
		LDR 	63H, 0H 			//0E1E 	1863
		STR 	7AH 			//0E1F 	10FA

		//;main.c: 65: LastManual=ManualType;
		LDR 	78H, 0H 			//0E20 	1878
		STR 	62H 			//0E21 	10E2

		//;main.c: 66: ManualType=0;
		CLRF 	78H 			//0E22 	11F8

		//;main.c: 67: }
		LJUMP 	629H 			//0E23 	3E29

		//;main.c: 68: else if(ModeType >= Mode3)
		LDWI 	3H 			//0E24 	0003
		SUBWR 	7AH, 0H 		//0E25 	127A
		BTSC 	3H, 0H 			//0E26 	2803

		//;main.c: 69: {
		//;main.c: 70: ModeType = Mode1;
		CLRF 	7AH 			//0E27 	11FA

		//;main.c: 72: else ModeType+=1;
		//;main.c: 71: }
		INCR 	7AH, 1H 		//0E28 	1AFA

		//;main.c: 74: DspIndex =DsMode;
		LJUMP 	722H 			//0E29 	3F22

		//;main.c: 14: SysFunFlg0.OneBits.b7 = ~SysFunFlg0.OneBits.b7;
		LDWI 	80H 			//0E2A 	0080
		MOVLB 	0H 			//0E2B 	1020
		XORWR 	49H, 1H 		//0E2C 	16C9

		//;main.c: 16: if(SysFunFlg0.OneBits.b7)
		BTSS 	49H, 7H 		//0E2D 	2FC9
		LJUMP 	64BH 			//0E2E 	3E4B

		//;main.c: 17: {
		//;main.c: 18: PwrType=2;
		LDWI 	2H 			//0E2F 	0002
		STR 	7BH 			//0E30 	10FB

		//;main.c: 19: ModeType=2;
		STR 	7AH 			//0E31 	10FA

		//;main.c: 20: HeatType=1;
		MOVLB 	1H 			//0E32 	1021
		LDWI 	2H 			//0E33 	0002
		CLRF 	64H 			//0E34 	11E4
		INCR 	64H, 1H 		//0E35 	1AE4

		//;main.c: 21: StrongType=2;
		STR 	79H 			//0E36 	10F9

		//;main.c: 22: ManualType=0;
		CLRF 	78H 			//0E37 	11F8

		//;main.c: 24: LastMode=ModeType;
		LDR 	7AH, 0H 			//0E38 	187A
		STR 	63H 			//0E39 	10E3

		//;main.c: 25: LastManual=ManualType;
		LDR 	78H, 0H 			//0E3A 	1878
		STR 	62H 			//0E3B 	10E2

		//;main.c: 26: if(SysFunFlg1.OneBits.b2)
		MOVLB 	0H 			//0E3C 	1020
		BTSS 	48H, 2H 		//0E3D 	2D48
		LJUMP 	645H 			//0E3E 	3E45

		//;main.c: 27: {
		//;main.c: 28: TimeType=88;
		LDWI 	58H 			//0E3F 	0058
		MOVLB 	1H 			//0E40 	1021
		STR 	61H 			//0E41 	10E1

		//;main.c: 29: DspIndex =DsTime;
		LDWI 	5H 			//0E42 	0005
		STR 	60H 			//0E43 	10E0

		//;main.c: 30: }
		LJUMP 	655H 			//0E44 	3E55

		//;main.c: 31: else
		//;main.c: 32: {
		//;main.c: 33: TimeType=15;
		LDWI 	FH 			//0E45 	000F
		MOVLB 	1H 			//0E46 	1021
		STR 	61H 			//0E47 	10E1

		//;main.c: 34: DspIndex =DsMode;
		CLRF 	60H 			//0E48 	11E0
		INCR 	60H, 1H 		//0E49 	1AE0
		LJUMP 	655H 			//0E4A 	3E55

		//;main.c: 38: else
		//;main.c: 39: {
		//;main.c: 40: PwrType=0;
		CLRF 	7BH 			//0E4B 	11FB

		//;main.c: 41: ModeType=0;
		CLRF 	7AH 			//0E4C 	11FA

		//;main.c: 42: HeatType=0;
		MOVLB 	1H 			//0E4D 	1021
		CLRF 	64H 			//0E4E 	11E4

		//;main.c: 43: StrongType=0;
		CLRF 	79H 			//0E4F 	11F9

		//;main.c: 44: TimeType=0;
		CLRF 	61H 			//0E50 	11E1

		//;main.c: 45: ManualType=0;
		CLRF 	78H 			//0E51 	11F8

		//;main.c: 46: DspIndex =DsSty;
		CLRF 	60H 			//0E52 	11E0

		//;main.c: 47: SysFunFlg1.OneBits.b2=0;
		MOVLB 	0H 			//0E53 	1020
		BCR 	48H, 2H 			//0E54 	2148

		//;main.c: 48: }
		//;main.c: 50: TimeMin=TimeType;
		MOVLB 	1H 			//0E55 	1021
		LDR 	61H, 0H 			//0E56 	1861
		STR 	77H 			//0E57 	10F7

		//;main.c: 51: TIME_INIT();
		LJUMP 	418H 			//0E58 	3C18

		//;main.c: 117: if(SysFunFlg0.OneBits.b7)
		MOVLB 	0H 			//0E59 	1020
		BTSS 	49H, 7H 		//0E5A 	2FC9
		RET 					//0E5B 	1008

		//;main.c: 118: {
		//;main.c: 121: if(StrongType >= Mode3)
		LDWI 	3H 			//0E5C 	0003
		SUBWR 	79H, 0H 		//0E5D 	1279
		BTSC 	3H, 0H 			//0E5E 	2803

		//;main.c: 122: {
		//;main.c: 123: StrongType = Mode1;
		CLRF 	79H 			//0E5F 	11F9
		LDWI 	2H 			//0E60 	0002

		//;main.c: 125: else StrongType+=1;
		//;main.c: 124: }
		INCR 	79H, 1H 		//0E61 	1AF9

		//;main.c: 127: DspIndex =DsStrong;
		MOVLB 	1H 			//0E62 	1021
		STR 	60H 			//0E63 	10E0
		RET 					//0E64 	1008

		//;sub.c: 40: send_buf[cmd_pwr] =PwrType;
		LDR 	7BH, 0H 			//0E65 	187B
		STR 	20H 			//0E66 	10A0

		//;sub.c: 41: send_buf[cmd_mode] =ModeType;
		LDR 	7AH, 0H 			//0E67 	187A
		STR 	21H 			//0E68 	10A1

		//;sub.c: 42: send_buf[cmd_str] =StrongType;
		LDR 	79H, 0H 			//0E69 	1879
		STR 	22H 			//0E6A 	10A2

		//;sub.c: 43: send_buf[cmd_heat] =HeatType;
		LDR 	64H, 0H 			//0E6B 	1864
		STR 	23H 			//0E6C 	10A3

		//;sub.c: 44: send_buf[cmd_Manual] =ManualType;
		LDR 	78H, 0H 			//0E6D 	1878
		STR 	24H 			//0E6E 	10A4

		//;sub.c: 45: send_buf[cmd_Time] =TimeMin;
		LDR 	77H, 0H 			//0E6F 	1877
		STR 	25H 			//0E70 	10A5

		//;sub.c: 46: SysFunFlg0.OneBits.b6 = 1;
		MOVLB 	0H 			//0E71 	1020
		BSR 	49H, 6H 			//0E72 	2749

		//;sub.c: 47: UartSetEmptyInt(1);
		LDWI 	1H 			//0E73 	0001
		LJUMP 	675H 			//0E74 	3E75
		STR 	4CH 			//0E75 	10CC

		//;uart.c: 46: URTE = value;
		BTSS 	4CH, 0H 		//0E76 	2C4C
		LJUMP 	67BH 			//0E77 	3E7B
		MOVLB 	9H 			//0E78 	1029
		BSR 	EH, 1H 			//0E79 	248E
		RET 					//0E7A 	1008
		MOVLB 	9H 			//0E7B 	1029
		BCR 	EH, 1H 			//0E7C 	208E
		RET 					//0E7D 	1008
		LDWI 	90H 			//0E7E 	0090
		MOVLB 	1H 			//0E7F 	1021
		STR 	45H 			//0E80 	10C5
		LDWI 	1H 			//0E81 	0001
		STR 	46H 			//0E82 	10C6
		RET 					//0E83 	1008

		//;rx.c: 200: KeepRfData();
		LCALL 	6FAH 			//0E84 	36FA
		MOVLP 	8H 			//0E85 	0188

		//;rx.c: 201: if(RxData==0x01)
		DECRSZ 	5AH, 0H 		//0E86 	1B5A
		LJUMP 	690H 			//0E87 	3E90

		//;rx.c: 202: {
		//;rx.c: 203: if(keybit5)return;
		LDR 	53H, 0H 			//0E88 	1853
		BTSS 	3H, 2H 			//0E89 	2D03
		RET 					//0E8A 	1008

		//;rx.c: 204: keybit5=1;
		CLRF 	53H 			//0E8B 	11D3
		INCR 	53H, 1H 		//0E8C 	1AD3

		//;rx.c: 206: tRFtemp=PowerKey;
		CLRF 	58H 			//0E8D 	11D8
		INCR 	58H, 1H 		//0E8E 	1AD8

		//;rx.c: 207: }else keybit5=0;
		LJUMP 	691H 			//0E8F 	3E91
		CLRF 	53H 			//0E90 	11D3

		//;rx.c: 209: if(RxData==0x0E)
		LDWI 	EH 			//0E91 	000E
		XORWR 	5AH, 0H 		//0E92 	165A
		BTSS 	3H, 2H 			//0E93 	2D03
		LJUMP 	6A0H 			//0E94 	3EA0

		//;rx.c: 210: {
		//;rx.c: 211: if(keybit1)return;
		LDR 	57H, 0H 			//0E95 	1857
		BTSS 	3H, 2H 			//0E96 	2D03
		RET 					//0E97 	1008

		//;rx.c: 212: keybit1=1;
		CLRF 	57H 			//0E98 	11D7
		INCR 	57H, 1H 		//0E99 	1AD7

		//;rx.c: 214: if(PwrType)tRFtemp=ModeKey;
		LDR 	7BH, 0H 			//0E9A 	187B
		BTSC 	3H, 2H 			//0E9B 	2903
		LJUMP 	6A1H 			//0E9C 	3EA1
		LDWI 	2H 			//0E9D 	0002
		STR 	58H 			//0E9E 	10D8
		LJUMP 	6A1H 			//0E9F 	3EA1
		CLRF 	57H 			//0EA0 	11D7

		//;rx.c: 217: if(RxData==0x0A)
		LDWI 	AH 			//0EA1 	000A
		XORWR 	5AH, 0H 		//0EA2 	165A
		BTSS 	3H, 2H 			//0EA3 	2D03
		LJUMP 	6B0H 			//0EA4 	3EB0

		//;rx.c: 218: {
		//;rx.c: 219: if(keybit2)return;
		LDR 	56H, 0H 			//0EA5 	1856
		BTSS 	3H, 2H 			//0EA6 	2D03
		RET 					//0EA7 	1008

		//;rx.c: 220: keybit2=1;
		CLRF 	56H 			//0EA8 	11D6
		INCR 	56H, 1H 		//0EA9 	1AD6

		//;rx.c: 222: if(PwrType)tRFtemp=StrongKey;
		LDR 	7BH, 0H 			//0EAA 	187B
		BTSC 	3H, 2H 			//0EAB 	2903
		LJUMP 	6B1H 			//0EAC 	3EB1
		LDWI 	5H 			//0EAD 	0005
		STR 	58H 			//0EAE 	10D8
		LJUMP 	6B1H 			//0EAF 	3EB1
		CLRF 	56H 			//0EB0 	11D6

		//;rx.c: 225: if(RxData==0x08)
		LDWI 	8H 			//0EB1 	0008
		XORWR 	5AH, 0H 		//0EB2 	165A
		BTSS 	3H, 2H 			//0EB3 	2D03
		LJUMP 	6C0H 			//0EB4 	3EC0

		//;rx.c: 226: {
		//;rx.c: 227: if(keybit3)return;
		LDR 	55H, 0H 			//0EB5 	1855
		BTSS 	3H, 2H 			//0EB6 	2D03
		RET 					//0EB7 	1008

		//;rx.c: 228: keybit3=1;
		CLRF 	55H 			//0EB8 	11D5
		INCR 	55H, 1H 		//0EB9 	1AD5

		//;rx.c: 230: if(PwrType)tRFtemp=ManualKey;
		LDR 	7BH, 0H 			//0EBA 	187B
		BTSC 	3H, 2H 			//0EBB 	2903
		LJUMP 	6C1H 			//0EBC 	3EC1
		LDWI 	6H 			//0EBD 	0006
		STR 	58H 			//0EBE 	10D8
		LJUMP 	6C1H 			//0EBF 	3EC1
		CLRF 	55H 			//0EC0 	11D5

		//;rx.c: 233: if(RxData==0x0C)
		LDWI 	CH 			//0EC1 	000C
		XORWR 	5AH, 0H 		//0EC2 	165A
		BTSS 	3H, 2H 			//0EC3 	2D03
		LJUMP 	6D0H 			//0EC4 	3ED0

		//;rx.c: 234: {
		//;rx.c: 235: if(keybit4)return;
		LDR 	54H, 0H 			//0EC5 	1854
		BTSS 	3H, 2H 			//0EC6 	2D03
		RET 					//0EC7 	1008

		//;rx.c: 236: keybit4=1;
		CLRF 	54H 			//0EC8 	11D4
		INCR 	54H, 1H 		//0EC9 	1AD4

		//;rx.c: 238: if(PwrType)tRFtemp=HeatKey;
		LDR 	7BH, 0H 			//0ECA 	187B
		BTSC 	3H, 2H 			//0ECB 	2903
		LJUMP 	6D1H 			//0ECC 	3ED1
		LDWI 	4H 			//0ECD 	0004
		STR 	58H 			//0ECE 	10D8
		LJUMP 	6D1H 			//0ECF 	3ED1
		CLRF 	54H 			//0ED0 	11D4

		//;rx.c: 241: if(tRFtemp)
		LDR 	58H, 0H 			//0ED1 	1858
		BTSC 	3H, 2H 			//0ED2 	2903
		RET 					//0ED3 	1008
		LJUMP 	6E3H 			//0ED4 	3EE3

		//;rx.c: 246: {
		//;rx.c: 247: case PowerKey:
		//;rx.c: 248: KeyType=PowerKey;
		MOVLB 	0H 			//0ED5 	1020
		CLRF 	45H 			//0ED6 	11C5
		INCR 	45H, 1H 		//0ED7 	1AC5

		//;rx.c: 250: break;
		LJUMP 	6F4H 			//0ED8 	3EF4

		//;rx.c: 252: KeyType=ModeKey;
		LDWI 	2H 			//0ED9 	0002
		LJUMP 	6E0H 			//0EDA 	3EE0

		//;rx.c: 254: break;
		//;rx.c: 256: KeyType=StrongKey;
		LDWI 	5H 			//0EDB 	0005
		LJUMP 	6E0H 			//0EDC 	3EE0

		//;rx.c: 258: break;
		//;rx.c: 260: KeyType=HeatKey;
		LDWI 	4H 			//0EDD 	0004
		LJUMP 	6E0H 			//0EDE 	3EE0

		//;rx.c: 262: break;
		//;rx.c: 264: KeyType=ManualKey;
		LDWI 	6H 			//0EDF 	0006
		MOVLB 	0H 			//0EE0 	1020
		STR 	45H 			//0EE1 	10C5

		//;rx.c: 266: break;
		LJUMP 	6F4H 			//0EE2 	3EF4
		LDR 	58H, 0H 			//0EE3 	1858
		XORWI 	1H 			//0EE4 	0A01
		BTSC 	3H, 2H 			//0EE5 	2903
		LJUMP 	6D5H 			//0EE6 	3ED5
		XORWI 	3H 			//0EE7 	0A03
		BTSC 	3H, 2H 			//0EE8 	2903
		LJUMP 	6D9H 			//0EE9 	3ED9
		XORWI 	6H 			//0EEA 	0A06
		BTSC 	3H, 2H 			//0EEB 	2903
		LJUMP 	6DDH 			//0EEC 	3EDD
		XORWI 	1H 			//0EED 	0A01
		BTSC 	3H, 2H 			//0EEE 	2903
		LJUMP 	6DBH 			//0EEF 	3EDB
		XORWI 	3H 			//0EF0 	0A03
		BTSC 	3H, 2H 			//0EF1 	2903
		LJUMP 	6DFH 			//0EF2 	3EDF
		LJUMP 	6F4H 			//0EF3 	3EF4

		//;rx.c: 270: refresh_display(tRFtemp);
		MOVLB 	1H 			//0EF4 	1021
		LDR 	58H, 0H 			//0EF5 	1858
		LCALL 	703H 			//0EF6 	3703

		//;rx.c: 271: tRFtemp=NullKey;
		MOVLB 	1H 			//0EF7 	1021
		CLRF 	58H 			//0EF8 	11D8
		RET 					//0EF9 	1008

		//;rx.c: 188: if (RemOutTime > 0)
		MOVLB 	1H 			//0EFA 	1021
		LDR 	59H, 0H 			//0EFB 	1859
		BTSC 	3H, 2H 			//0EFC 	2903
		RET 					//0EFD 	1008

		//;rx.c: 189: {
		//;rx.c: 190: RemOutTime--;
		DECR 	59H, 1H 		//0EFE 	13D9

		//;rx.c: 191: if (RemOutTime == 0)
		LDR 	59H, 0H 			//0EFF 	1859
		BTSC 	3H, 2H 			//0F00 	2903

		//;rx.c: 192: {
		//;rx.c: 193: RxData= 0;
		CLRF 	5AH 			//0F01 	11DA
		RET 					//0F02 	1008
		MOVLB 	0H 			//0F03 	1020
		STR 	4CH 			//0F04 	10CC

		//;sub.c: 52: switch (keybuf)
		LJUMP 	713H 			//0F05 	3F13

		//;sub.c: 53: {
		//;sub.c: 54: case ModeKey:
		//;sub.c: 55: DspIndex=DsMode;
		//;sub.c: 56: break;
		LJUMP 	722H 			//0F06 	3F22

		//;sub.c: 58: DspIndex =DsStrong;
		LDWI 	2H 			//0F07 	0002
		MOVLB 	1H 			//0F08 	1021
		STR 	60H 			//0F09 	10E0

		//;sub.c: 59: break;
		RET 					//0F0A 	1008

		//;sub.c: 61: DspIndex =DsHeat;
		LDWI 	3H 			//0F0B 	0003
		MOVLB 	1H 			//0F0C 	1021
		STR 	60H 			//0F0D 	10E0

		//;sub.c: 62: break;
		RET 					//0F0E 	1008

		//;sub.c: 64: DspIndex =DsManual;
		LDWI 	4H 			//0F0F 	0004
		MOVLB 	1H 			//0F10 	1021
		STR 	60H 			//0F11 	10E0
		RET 					//0F12 	1008
		LDR 	4CH, 0H 			//0F13 	184C
		XORWI 	2H 			//0F14 	0A02
		BTSC 	3H, 2H 			//0F15 	2903
		LJUMP 	706H 			//0F16 	3F06
		XORWI 	6H 			//0F17 	0A06
		BTSC 	3H, 2H 			//0F18 	2903
		LJUMP 	70BH 			//0F19 	3F0B
		XORWI 	1H 			//0F1A 	0A01
		BTSC 	3H, 2H 			//0F1B 	2903
		LJUMP 	707H 			//0F1C 	3F07
		XORWI 	3H 			//0F1D 	0A03
		BTSC 	3H, 2H 			//0F1E 	2903
		LJUMP 	70FH 			//0F1F 	3F0F
		LJUMP 	721H 			//0F20 	3F21
		RET 					//0F21 	1008
		MOVLB 	1H 			//0F22 	1021
		CLRF 	60H 			//0F23 	11E0
		INCR 	60H, 1H 		//0F24 	1AE0
		RET 					//0F25 	1008
		MOVLB 	1H 			//0F26 	1021
		BSR 	DH, 7H 			//0F27 	278D
		BSR 	DH, 2H 			//0F28 	250D
		BSR 	EH, 4H 			//0F29 	260E
		BSR 	EH, 5H 			//0F2A 	268E
		BSR 	EH, 6H 			//0F2B 	270E
		BSR 	EH, 7H 			//0F2C 	278E
		LDWI 	2H 			//0F2D 	0002
		CLRF 	47H 			//0F2E 	11C7
		MOVLB 	0H 			//0F2F 	1020
		STR 	3DH 			//0F30 	10BD
		LDWI 	90H 			//0F31 	0090
		MOVLB 	1H 			//0F32 	1021
		STR 	6BH 			//0F33 	10EB
		LDWI 	1H 			//0F34 	0001
		STR 	6CH 			//0F35 	10EC
		LDWI 	58H 			//0F36 	0058
		STR 	69H 			//0F37 	10E9
		LDWI 	2H 			//0F38 	0002
		STR 	6AH 			//0F39 	10EA
		LDWI 	1H 			//0F3A 	0001
		CLRF 	4AH 			//0F3B 	11CA
		MOVLB 	0H 			//0F3C 	1020
		STR 	3EH 			//0F3D 	10BE
		LDWI 	B8H 			//0F3E 	00B8
		STR 	4H 			//0F3F 	1084
		LDWI 	98H 			//0F40 	0098
		STR 	5H 			//0F41 	1085
		LDR 	0H, 0H 			//0F42 	1800
		STR 	3EH 			//0F43 	10BE
		LDWI 	B4H 			//0F44 	00B4
		STR 	4H 			//0F45 	1084
		LDWI 	98H 			//0F46 	0098
		STR 	5H 			//0F47 	1085
		LDR 	0H, 0H 			//0F48 	1800
		STR 	3EH 			//0F49 	10BE
		CLRF 	3EH 			//0F4A 	11BE
		MOVLB 	1H 			//0F4B 	1021
		LDR 	3DH, 0H 			//0F4C 	183D
		MOVLB 	0H 			//0F4D 	1020
		STR 	3EH 			//0F4E 	10BE
		MOVLB 	1H 			//0F4F 	1021
		LDR 	3BH, 0H 			//0F50 	183B
		MOVLB 	0H 			//0F51 	1020
		STR 	3EH 			//0F52 	10BE
		LDWI 	4H 			//0F53 	0004
		MOVLB 	1H 			//0F54 	1021
		STR 	39H 			//0F55 	10B9
		CLRF 	3AH 			//0F56 	11BA
		MOVLB 	0H 			//0F57 	1020
		CLRF 	4BH 			//0F58 	11CB
		CLRF 	3FH 			//0F59 	11BF
		CLRF 	3EH 			//0F5A 	11BE
		LDWI 	FH 			//0F5B 	000F
		SUBWR 	3EH, 0H 		//0F5C 	123E
		BTSC 	3H, 0H 			//0F5D 	2803
		LJUMP 	77AH 			//0F5E 	3F7A
		LDWI 	80H 			//0F5F 	0080
		STR 	4CH 			//0F60 	10CC
		LDWI 	78H 			//0F61 	0078
		STR 	4DH 			//0F62 	10CD
		INCR 	3EH, 0H 		//0F63 	1A3E
		LJUMP 	767H 			//0F64 	3F67
		LSRF 	4DH, 1H 		//0F65 	06CD
		RRR 	4CH, 1H 			//0F66 	1CCC
		DECRSZ 	9H, 1H 		//0F67 	1B89
		LJUMP 	765H 			//0F68 	3F65
		BTSS 	4CH, 0H 		//0F69 	2C4C
		LJUMP 	775H 			//0F6A 	3F75
		LDR 	4BH, 0H 			//0F6B 	184B
		STR 	47H 			//0F6C 	10C7
		LDR 	4BH, 0H 			//0F6D 	184B
		ADDWI 	3EH 			//0F6E 	0E3E
		STR 	6H 			//0F6F 	1086
		LDWI 	1H 			//0F70 	0001
		STR 	7H 			//0F71 	1087
		LDR 	3EH, 0H 			//0F72 	183E
		STR 	1H 			//0F73 	1081
		INCR 	4BH, 1H 		//0F74 	1ACB
		INCR 	3EH, 1H 		//0F75 	1ABE
		LDWI 	FH 			//0F76 	000F
		SUBWR 	3EH, 0H 		//0F77 	123E
		BTSS 	3H, 0H 			//0F78 	2C03
		LJUMP 	75FH 			//0F79 	3F5F
		MOVLB 	2H 			//0F7A 	1022
		LDR 	3EH, 0H 			//0F7B 	183E
		MOVLB 	0H 			//0F7C 	1020
		STR 	4BH 			//0F7D 	10CB
		LDWI 	6H 			//0F7E 	0006
		MOVLB 	1H 			//0F7F 	1021
		STR 	67H 			//0F80 	10E7
		CLRF 	68H 			//0F81 	11E8
		LDR 	68H, 0H 			//0F82 	1868
		STR 	66H 			//0F83 	10E6
		LDR 	67H, 0H 			//0F84 	1867
		STR 	65H 			//0F85 	10E5
		RET 					//0F86 	1008

		//;CHIPINIT.C: 595: PCKEN |=0B00100000;
		MOVLB 	1H 			//0F87 	1021
		BSR 	1AH, 5H 			//0F88 	269A

		//;CHIPINIT.C: 597: URIER =0B00000000;
		MOVLB 	9H 			//0F89 	1029
		CLRF 	EH 			//0F8A 	118E

		//;CHIPINIT.C: 598: URLCR =0B00000001;
		LDWI 	1H 			//0F8B 	0001
		STR 	FH 			//0F8C 	108F

		//;CHIPINIT.C: 599: URMCR =0B00011000;
		LDWI 	18H 			//0F8D 	0018
		STR 	11H 			//0F8E 	1091

		//;CHIPINIT.C: 602: URDLL =52;
		LDWI 	34H 			//0F8F 	0034
		STR 	14H 			//0F90 	1094

		//;CHIPINIT.C: 603: URDLH =0;
		CLRF 	15H 			//0F91 	1195

		//;CHIPINIT.C: 604: TCF=1;
		BSR 	1CH, 0H 			//0F92 	241C
		RET 					//0F93 	1008
		CLRF 	51H 			//0F94 	11D1
		BTSS 	4DH, 7H 		//0F95 	2FCD
		LJUMP 	79EH 			//0F96 	3F9E
		COMR 	4CH, 1H 		//0F97 	19CC
		COMR 	4DH, 1H 		//0F98 	19CD
		INCR 	4CH, 1H 		//0F99 	1ACC
		BTSC 	3H, 2H 			//0F9A 	2903
		INCR 	4DH, 1H 		//0F9B 	1ACD
		CLRF 	51H 			//0F9C 	11D1
		INCR 	51H, 1H 		//0F9D 	1AD1
		BTSS 	4FH, 7H 		//0F9E 	2FCF
		LJUMP 	7A7H 			//0F9F 	3FA7
		COMR 	4EH, 1H 		//0FA0 	19CE
		COMR 	4FH, 1H 		//0FA1 	19CF
		INCR 	4EH, 1H 		//0FA2 	1ACE
		BTSC 	3H, 2H 			//0FA3 	2903
		INCR 	4FH, 1H 		//0FA4 	1ACF
		LDWI 	1H 			//0FA5 	0001
		XORWR 	51H, 1H 		//0FA6 	16D1
		CLRF 	52H 			//0FA7 	11D2
		CLRF 	53H 			//0FA8 	11D3
		LDR 	4CH, 0H 			//0FA9 	184C
		IORWR 	4DH, 0H 		//0FAA 	144D
		BTSC 	3H, 2H 			//0FAB 	2903
		LJUMP 	7C7H 			//0FAC 	3FC7
		CLRF 	50H 			//0FAD 	11D0
		INCR 	50H, 1H 		//0FAE 	1AD0
		BTSC 	4DH, 7H 		//0FAF 	2BCD
		LJUMP 	7B4H 			//0FB0 	3FB4
		LSLF 	4CH, 1H 		//0FB1 	05CC
		RLR 	4DH, 1H 			//0FB2 	1DCD
		LJUMP 	7AEH 			//0FB3 	3FAE
		LSLF 	52H, 1H 		//0FB4 	05D2
		RLR 	53H, 1H 			//0FB5 	1DD3
		LDR 	4DH, 0H 			//0FB6 	184D
		SUBWR 	4FH, 0H 		//0FB7 	124F
		BTSS 	3H, 2H 			//0FB8 	2D03
		LJUMP 	7BCH 			//0FB9 	3FBC
		LDR 	4CH, 0H 			//0FBA 	184C
		SUBWR 	4EH, 0H 		//0FBB 	124E
		BTSS 	3H, 0H 			//0FBC 	2C03
		LJUMP 	7C3H 			//0FBD 	3FC3
		LDR 	4CH, 0H 			//0FBE 	184C
		SUBWR 	4EH, 1H 		//0FBF 	12CE
		LDR 	4DH, 0H 			//0FC0 	184D
		SUBWFB 	4FH, 1H 		//0FC1 	0BCF
		BSR 	52H, 0H 			//0FC2 	2452
		LSRF 	4DH, 1H 		//0FC3 	06CD
		RRR 	4CH, 1H 			//0FC4 	1CCC
		DECRSZ 	50H, 1H 		//0FC5 	1BD0
		LJUMP 	7B4H 			//0FC6 	3FB4
		LDR 	51H, 0H 			//0FC7 	1851
		BTSC 	3H, 2H 			//0FC8 	2903
		LJUMP 	7CFH 			//0FC9 	3FCF
		COMR 	52H, 1H 		//0FCA 	19D2
		COMR 	53H, 1H 		//0FCB 	19D3
		INCR 	52H, 1H 		//0FCC 	1AD2
		BTSC 	3H, 2H 			//0FCD 	2903
		INCR 	53H, 1H 		//0FCE 	1AD3
		LDR 	53H, 0H 			//0FCF 	1853
		STR 	4DH 			//0FD0 	10CD
		LDR 	52H, 0H 			//0FD1 	1852
		STR 	4CH 			//0FD2 	10CC
		RET 					//0FD3 	1008

		//;rx.c: 173: if(RFs.RecBufFull==0)return;
		MOVLB 	1H 			//0FD4 	1021
		LDR 	2BH, 0H 			//0FD5 	182B
		BTSC 	3H, 2H 			//0FD6 	2903
		RET 					//0FD7 	1008

		//;rx.c: 174: RFs.RecBufFull=0;
		CLRF 	2BH 			//0FD8 	11AB

		//;rx.c: 176: if(buf_rec[0]!=buf_rec[1])return;
		LDR 	2DH, 0H 			//0FD9 	182D
		XORWR 	2CH, 0H 		//0FDA 	162C
		BTSS 	3H, 2H 			//0FDB 	2D03
		RET 					//0FDC 	1008

		//;rx.c: 178: if(!CheckRfCmd(buf_rec[2]))return;
		LDR 	2EH, 0H 			//0FDD 	182E
		LCALL 	7E9H 			//0FDE 	37E9
		MOVLP 	8H 			//0FDF 	0188
		XORWI 	0H 			//0FE0 	0A00
		BTSC 	3H, 2H 			//0FE1 	2903
		RET 					//0FE2 	1008

		//;rx.c: 180: RxData=buf_rec[2];
		MOVLB 	1H 			//0FE3 	1021
		LDR 	2EH, 0H 			//0FE4 	182E
		STR 	5AH 			//0FE5 	10DA

		//;rx.c: 182: RemOutTime=100;
		LDWI 	64H 			//0FE6 	0064
		STR 	59H 			//0FE7 	10D9
		RET 					//0FE8 	1008
		MOVLB 	0H 			//0FE9 	1020
		STR 	4CH 			//0FEA 	10CC

		//;rx.c: 29: switch( rfcmd)
		LJUMP 	7EEH 			//0FEB 	3FEE

		//;rx.c: 32: case 0X08:
		//;rx.c: 33: case 0X0E:
		//;rx.c: 34: case 0X0C:
		//;rx.c: 35: case 0X0A:
		//;rx.c: 36: return 1;
		RETW 	1H 			//0FEC 	0401

		//;rx.c: 38: return 0;
		RETW 	0H 			//0FED 	0400
		LDR 	4CH, 0H 			//0FEE 	184C
		XORWI 	1H 			//0FEF 	0A01
		BTSC 	3H, 2H 			//0FF0 	2903
		LJUMP 	7ECH 			//0FF1 	3FEC
		XORWI 	9H 			//0FF2 	0A09
		BTSC 	3H, 2H 			//0FF3 	2903
		LJUMP 	7ECH 			//0FF4 	3FEC
		XORWI 	2H 			//0FF5 	0A02
		BTSC 	3H, 2H 			//0FF6 	2903
		LJUMP 	7ECH 			//0FF7 	3FEC
		XORWI 	6H 			//0FF8 	0A06
		BTSC 	3H, 2H 			//0FF9 	2903
		LJUMP 	7ECH 			//0FFA 	3FEC
		XORWI 	2H 			//0FFB 	0A02
		BTSC 	3H, 2H 			//0FFC 	2903
		LJUMP 	7ECH 			//0FFD 	3FEC
		ORG		0FFEH
		LJUMP 	7EDH 			//0FFE 	3FED
		RET 					//0FFF 	1008
		ORG		1531H

		//;sub.c: 265: static unsigned char LedMenuIndex=0;
		//;sub.c: 266: static unsigned char DspBuf,DspCnt;
		//;sub.c: 268: if(++DspCnt <=16 )return;
		LDWI 	11H 			//1531 	0011
		MOVLB 	1H 			//1532 	1021
		INCR 	4FH, 1H 		//1533 	1ACF
		SUBWR 	4FH, 0H 		//1534 	124F
		BTSS 	3H, 0H 			//1535 	2C03
		RET 					//1536 	1008

		//;sub.c: 269: DspCnt = 0;
		CLRF 	4FH 			//1537 	11CF

		//;sub.c: 271: {LATB1=0;LATD1=0;LATC1=0;LATC0=0; LATA0=0;LATB0=0;LATB3=0; LATA4=0;LATD2=0;
		//+                          LATA1=0;LATD3=0;};
		LCALL 	7EDH 			//1538 	37ED
		MOVLP 	15H 			//1539 	0195

		//;sub.c: 275: LedMenuIndex++;
		MOVLB 	1H 			//153A 	1021
		LDWI 	4H 			//153B 	0004
		INCR 	51H, 1H 		//153C 	1AD1

		//;sub.c: 276: if(LedMenuIndex>3)
		SUBWR 	51H, 0H 		//153D 	1251
		BTSS 	3H, 0H 			//153E 	2C03
		LJUMP 	5ADH 			//153F 	3DAD

		//;sub.c: 277: LedMenuIndex=0;
		CLRF 	51H 			//1540 	11D1
		LJUMP 	5ADH 			//1541 	3DAD

		//;sub.c: 282: if(PwrType)
		LDR 	7BH, 0H 			//1542 	187B
		BTSC 	3H, 2H 			//1543 	2903
		LJUMP 	566H 			//1544 	3D66

		//;sub.c: 283: {
		//;sub.c: 284: if(PwrType)LATD2=1;
		LDR 	7BH, 0H 			//1545 	187B
		BTSC 	3H, 2H 			//1546 	2903
		LJUMP 	54AH 			//1547 	3D4A
		MOVLB 	2H 			//1548 	1022
		BSR 	FH, 2H 			//1549 	250F

		//;sub.c: 286: if(HeatType==1)LATA4=1;
		MOVLB 	1H 			//154A 	1021
		DECRSZ 	64H, 0H 		//154B 	1B64
		LJUMP 	54FH 			//154C 	3D4F
		MOVLB 	2H 			//154D 	1022
		BSR 	CH, 4H 			//154E 	260C

		//;sub.c: 288: if(ManualType||ModeType)
		LDR 	78H, 0H 			//154F 	1878
		BTSC 	3H, 2H 			//1550 	2903
		LDR 	7AH, 0H 			//1551 	187A
		BTSC 	3H, 2H 			//1552 	2903
		LJUMP 	566H 			//1553 	3D66

		//;sub.c: 289: {
		//;sub.c: 302: if(StrongType==1){
		DECRSZ 	79H, 0H 		//1554 	1B79
		LJUMP 	559H 			//1555 	3D59

		//;sub.c: 303: LATB3=1;
		MOVLB 	2H 			//1556 	1022
		BSR 	DH, 3H 			//1557 	258D

		//;sub.c: 304: }
		LJUMP 	566H 			//1558 	3D66

		//;sub.c: 305: else if(StrongType==2){
		LDWI 	2H 			//1559 	0002
		XORWR 	79H, 0H 		//155A 	1679
		BTSS 	3H, 2H 			//155B 	2D03
		LJUMP 	560H 			//155C 	3D60

		//;sub.c: 306: LATB0=1;
		MOVLB 	2H 			//155D 	1022
		BSR 	DH, 0H 			//155E 	240D

		//;sub.c: 307: }
		LJUMP 	566H 			//155F 	3D66

		//;sub.c: 308: else if(StrongType==3){
		LDWI 	3H 			//1560 	0003
		XORWR 	79H, 0H 		//1561 	1679
		BTSS 	3H, 2H 			//1562 	2D03
		LJUMP 	566H 			//1563 	3D66

		//;sub.c: 309: LATA0=1;
		MOVLB 	2H 			//1564 	1022
		BSR 	CH, 0H 			//1565 	240C

		//;sub.c: 310: }
		//;sub.c: 311: }
		//;sub.c: 312: }
		//;sub.c: 313: {LATB1=1;LATD1=0;LATC1=0;LATC0=0;};
		MOVLB 	2H 			//1566 	1022
		BSR 	DH, 1H 			//1567 	248D
		BCR 	FH, 1H 			//1568 	208F
		BCR 	EH, 1H 			//1569 	208E
		BCR 	EH, 0H 			//156A 	200E

		//;sub.c: 314: break;
		RET 					//156B 	1008

		//;sub.c: 316: if(PwrType)
		LDR 	7BH, 0H 			//156C 	187B
		BTSC 	3H, 2H 			//156D 	2903
		LJUMP 	593H 			//156E 	3D93

		//;sub.c: 317: {
		//;sub.c: 318: if(ModeType==1){ LATB3=1; }
		DECRSZ 	7AH, 0H 		//156F 	1B7A
		LJUMP 	574H 			//1570 	3D74
		MOVLB 	2H 			//1571 	1022
		BSR 	DH, 3H 			//1572 	258D
		LJUMP 	581H 			//1573 	3D81

		//;sub.c: 319: else if(ModeType==2){ LATB0=1; }
		LDWI 	2H 			//1574 	0002
		XORWR 	7AH, 0H 		//1575 	167A
		BTSS 	3H, 2H 			//1576 	2D03
		LJUMP 	57BH 			//1577 	3D7B
		MOVLB 	2H 			//1578 	1022
		BSR 	DH, 0H 			//1579 	240D
		LJUMP 	581H 			//157A 	3D81

		//;sub.c: 320: else if(ModeType==3){ LATA0=1; }
		LDWI 	3H 			//157B 	0003
		XORWR 	7AH, 0H 		//157C 	167A
		BTSS 	3H, 2H 			//157D 	2D03
		LJUMP 	581H 			//157E 	3D81
		MOVLB 	2H 			//157F 	1022
		BSR 	CH, 0H 			//1580 	240C

		//;sub.c: 322: if(ManualType==1)LATA4=1;
		DECRSZ 	78H, 0H 		//1581 	1B78
		LJUMP 	586H 			//1582 	3D86
		MOVLB 	2H 			//1583 	1022
		BSR 	CH, 4H 			//1584 	260C
		LJUMP 	593H 			//1585 	3D93

		//;sub.c: 323: else if(ManualType==2){LATA4=1;LATD2=1;}
		LDWI 	2H 			//1586 	0002
		XORWR 	78H, 0H 		//1587 	1678
		BTSS 	3H, 2H 			//1588 	2D03
		LJUMP 	58DH 			//1589 	3D8D
		MOVLB 	2H 			//158A 	1022
		BSR 	CH, 4H 			//158B 	260C
		LJUMP 	592H 			//158C 	3D92

		//;sub.c: 324: else if(ManualType==3)LATD2=1;
		LDWI 	3H 			//158D 	0003
		XORWR 	78H, 0H 		//158E 	1678
		BTSS 	3H, 2H 			//158F 	2D03
		LJUMP 	593H 			//1590 	3D93
		MOVLB 	2H 			//1591 	1022
		BSR 	FH, 2H 			//1592 	250F

		//;sub.c: 325: }
		//;sub.c: 326: {LATD1=1;LATB1=0;LATC1=0;LATC0=0;};
		MOVLB 	2H 			//1593 	1022
		BSR 	FH, 1H 			//1594 	248F
		BCR 	DH, 1H 			//1595 	208D
		BCR 	EH, 1H 			//1596 	208E
		BCR 	EH, 0H 			//1597 	200E

		//;sub.c: 327: break;
		RET 					//1598 	1008

		//;sub.c: 330: DspBuf=SpecialDisplay1();
		LCALL 	781H 			//1599 	3781
		MOVLP 	15H 			//159A 	0195
		STR 	50H 			//159B 	10D0

		//;sub.c: 331: ShowLedData(DspBuf);
		LCALL 	5BCH 			//159C 	35BC

		//;sub.c: 332: {LATC1=1;LATB1=0;LATD1=0;LATC0=0;};
		MOVLB 	2H 			//159D 	1022
		BSR 	EH, 1H 			//159E 	248E
		BCR 	DH, 1H 			//159F 	208D
		BCR 	FH, 1H 			//15A0 	208F
		BCR 	EH, 0H 			//15A1 	200E

		//;sub.c: 333: break;
		RET 					//15A2 	1008

		//;sub.c: 336: DspBuf=SpecialDisplay2();
		LCALL 	7B5H 			//15A3 	37B5
		MOVLP 	15H 			//15A4 	0195
		STR 	50H 			//15A5 	10D0

		//;sub.c: 337: ShowLedData(DspBuf);
		LCALL 	5BCH 			//15A6 	35BC

		//;sub.c: 338: {LATC0=1;LATB1=0;LATD1=0;LATC1=0;};
		MOVLB 	2H 			//15A7 	1022
		BSR 	EH, 0H 			//15A8 	240E
		BCR 	DH, 1H 			//15A9 	208D
		BCR 	FH, 1H 			//15AA 	208F
		BCR 	EH, 1H 			//15AB 	208E

		//;sub.c: 339: break;
		RET 					//15AC 	1008
		LDR 	51H, 0H 			//15AD 	1851
		XORWI 	0H 			//15AE 	0A00
		BTSC 	3H, 2H 			//15AF 	2903
		LJUMP 	542H 			//15B0 	3D42
		XORWI 	1H 			//15B1 	0A01
		BTSC 	3H, 2H 			//15B2 	2903
		LJUMP 	56CH 			//15B3 	3D6C
		XORWI 	3H 			//15B4 	0A03
		BTSC 	3H, 2H 			//15B5 	2903
		LJUMP 	599H 			//15B6 	3D99
		XORWI 	1H 			//15B7 	0A01
		BTSC 	3H, 2H 			//15B8 	2903
		LJUMP 	5A3H 			//15B9 	3DA3
		LJUMP 	5BBH 			//15BA 	3DBB
		RET 					//15BB 	1008
		STR 	70H 			//15BC 	10F0

		//;sub.c: 206: switch (DspBuf)
		LJUMP 	776H 			//15BD 	3F76

		//;sub.c: 207: {
		//;sub.c: 208: case 1: LATB0=LATB3=1;
		MOVLB 	2H 			//15BE 	1022
		BSR 	DH, 3H 			//15BF 	258D
		BTSS 	DH, 3H 			//15C0 	2D8D
		LJUMP 	5C5H 			//15C1 	3DC5
		MOVLB 	2H 			//15C2 	1022
		BSR 	DH, 0H 			//15C3 	240D
		RET 					//15C4 	1008
		MOVLB 	2H 			//15C5 	1022
		BCR 	DH, 0H 			//15C6 	200D

		//;sub.c: 209: break; case 2: LATA0=LATB0=LATA4=LATD2=LATD3=1;
		RET 					//15C7 	1008
		MOVLB 	2H 			//15C8 	1022
		BSR 	FH, 3H 			//15C9 	258F
		BTSS 	FH, 3H 			//15CA 	2D8F
		LJUMP 	5CFH 			//15CB 	3DCF
		MOVLB 	2H 			//15CC 	1022
		BSR 	FH, 2H 			//15CD 	250F
		LJUMP 	5D1H 			//15CE 	3DD1
		MOVLB 	2H 			//15CF 	1022
		BCR 	FH, 2H 			//15D0 	210F
		BTSS 	FH, 2H 			//15D1 	2D0F
		LJUMP 	5D6H 			//15D2 	3DD6
		MOVLB 	2H 			//15D3 	1022
		BSR 	CH, 4H 			//15D4 	260C
		LJUMP 	5D8H 			//15D5 	3DD8
		MOVLB 	2H 			//15D6 	1022
		BCR 	CH, 4H 			//15D7 	220C
		BTSS 	CH, 4H 			//15D8 	2E0C
		LJUMP 	5DDH 			//15D9 	3DDD
		MOVLB 	2H 			//15DA 	1022
		BSR 	DH, 0H 			//15DB 	240D
		LJUMP 	5DFH 			//15DC 	3DDF
		MOVLB 	2H 			//15DD 	1022
		BCR 	DH, 0H 			//15DE 	200D
		BTSS 	DH, 0H 			//15DF 	2C0D
		LJUMP 	5E4H 			//15E0 	3DE4
		MOVLB 	2H 			//15E1 	1022
		BSR 	CH, 0H 			//15E2 	240C
		RET 					//15E3 	1008
		MOVLB 	2H 			//15E4 	1022
		BCR 	CH, 0H 			//15E5 	200C

		//;sub.c: 210: break; case 3: LATA0=LATB0=LATB3=LATA4=LATD3=1;
		RET 					//15E6 	1008
		MOVLB 	2H 			//15E7 	1022
		BSR 	FH, 3H 			//15E8 	258F
		BTSS 	FH, 3H 			//15E9 	2D8F
		LJUMP 	5EEH 			//15EA 	3DEE
		MOVLB 	2H 			//15EB 	1022
		BSR 	CH, 4H 			//15EC 	260C
		LJUMP 	5F0H 			//15ED 	3DF0
		MOVLB 	2H 			//15EE 	1022
		BCR 	CH, 4H 			//15EF 	220C
		BTSS 	CH, 4H 			//15F0 	2E0C
		LJUMP 	5F5H 			//15F1 	3DF5
		MOVLB 	2H 			//15F2 	1022
		BSR 	DH, 3H 			//15F3 	258D
		LJUMP 	5F7H 			//15F4 	3DF7
		MOVLB 	2H 			//15F5 	1022
		BCR 	DH, 3H 			//15F6 	218D
		BTSS 	DH, 3H 			//15F7 	2D8D
		LJUMP 	5FCH 			//15F8 	3DFC
		MOVLB 	2H 			//15F9 	1022
		BSR 	DH, 0H 			//15FA 	240D
		LJUMP 	5FEH 			//15FB 	3DFE
		MOVLB 	2H 			//15FC 	1022
		BCR 	DH, 0H 			//15FD 	200D
		BTSS 	DH, 0H 			//15FE 	2C0D
		LJUMP 	603H 			//15FF 	3E03
		MOVLB 	2H 			//1600 	1022
		BSR 	CH, 0H 			//1601 	240C
		RET 					//1602 	1008
		MOVLB 	2H 			//1603 	1022
		BCR 	CH, 0H 			//1604 	200C

		//;sub.c: 211: break; case 4: LATB0=LATB3=LATA1=LATD3=1;
		RET 					//1605 	1008
		MOVLB 	2H 			//1606 	1022
		BSR 	FH, 3H 			//1607 	258F
		BTSS 	FH, 3H 			//1608 	2D8F
		LJUMP 	60DH 			//1609 	3E0D
		MOVLB 	2H 			//160A 	1022
		BSR 	CH, 1H 			//160B 	248C
		LJUMP 	60FH 			//160C 	3E0F
		MOVLB 	2H 			//160D 	1022
		BCR 	CH, 1H 			//160E 	208C
		BTSS 	CH, 1H 			//160F 	2C8C
		LJUMP 	614H 			//1610 	3E14
		MOVLB 	2H 			//1611 	1022
		BSR 	DH, 3H 			//1612 	258D
		LJUMP 	616H 			//1613 	3E16
		MOVLB 	2H 			//1614 	1022
		BCR 	DH, 3H 			//1615 	218D
		BTSS 	DH, 3H 			//1616 	2D8D
		LJUMP 	61BH 			//1617 	3E1B
		MOVLB 	2H 			//1618 	1022
		BSR 	DH, 0H 			//1619 	240D
		RET 					//161A 	1008
		MOVLB 	2H 			//161B 	1022
		BCR 	DH, 0H 			//161C 	200D

		//;sub.c: 212: break; case 5: LATA0=LATB3=LATA4=LATA1=LATD3=1;
		RET 					//161D 	1008
		MOVLB 	2H 			//161E 	1022
		BSR 	FH, 3H 			//161F 	258F
		BTSS 	FH, 3H 			//1620 	2D8F
		LJUMP 	625H 			//1621 	3E25
		MOVLB 	2H 			//1622 	1022
		BSR 	CH, 1H 			//1623 	248C
		LJUMP 	627H 			//1624 	3E27
		MOVLB 	2H 			//1625 	1022
		BCR 	CH, 1H 			//1626 	208C
		BTSS 	CH, 1H 			//1627 	2C8C
		LJUMP 	62CH 			//1628 	3E2C
		MOVLB 	2H 			//1629 	1022
		BSR 	CH, 4H 			//162A 	260C
		LJUMP 	62EH 			//162B 	3E2E
		MOVLB 	2H 			//162C 	1022
		BCR 	CH, 4H 			//162D 	220C
		BTSS 	CH, 4H 			//162E 	2E0C
		LJUMP 	633H 			//162F 	3E33
		MOVLB 	2H 			//1630 	1022
		BSR 	DH, 3H 			//1631 	258D
		LJUMP 	635H 			//1632 	3E35
		MOVLB 	2H 			//1633 	1022
		BCR 	DH, 3H 			//1634 	218D
		BTSS 	DH, 3H 			//1635 	2D8D
		LJUMP 	63AH 			//1636 	3E3A
		MOVLB 	2H 			//1637 	1022
		BSR 	CH, 0H 			//1638 	240C
		RET 					//1639 	1008
		MOVLB 	2H 			//163A 	1022
		BCR 	CH, 0H 			//163B 	200C

		//;sub.c: 213: break; case 6: LATA0=LATD3=LATB3=LATA4=LATD2=LATA1=1;
		RET 					//163C 	1008
		MOVLB 	2H 			//163D 	1022
		BSR 	CH, 1H 			//163E 	248C
		BTSS 	CH, 1H 			//163F 	2C8C
		LJUMP 	644H 			//1640 	3E44
		MOVLB 	2H 			//1641 	1022
		BSR 	FH, 2H 			//1642 	250F
		LJUMP 	646H 			//1643 	3E46
		MOVLB 	2H 			//1644 	1022
		BCR 	FH, 2H 			//1645 	210F
		BTSS 	FH, 2H 			//1646 	2D0F
		LJUMP 	64BH 			//1647 	3E4B
		MOVLB 	2H 			//1648 	1022
		BSR 	CH, 4H 			//1649 	260C
		LJUMP 	64DH 			//164A 	3E4D
		MOVLB 	2H 			//164B 	1022
		BCR 	CH, 4H 			//164C 	220C
		BTSS 	CH, 4H 			//164D 	2E0C
		LJUMP 	652H 			//164E 	3E52
		MOVLB 	2H 			//164F 	1022
		BSR 	DH, 3H 			//1650 	258D
		LJUMP 	654H 			//1651 	3E54
		MOVLB 	2H 			//1652 	1022
		BCR 	DH, 3H 			//1653 	218D
		BTSS 	DH, 3H 			//1654 	2D8D
		LJUMP 	659H 			//1655 	3E59
		MOVLB 	2H 			//1656 	1022
		BSR 	FH, 3H 			//1657 	258F
		LJUMP 	65BH 			//1658 	3E5B
		MOVLB 	2H 			//1659 	1022
		BCR 	FH, 3H 			//165A 	218F
		BTSS 	FH, 3H 			//165B 	2D8F
		LJUMP 	660H 			//165C 	3E60
		MOVLB 	2H 			//165D 	1022
		BSR 	CH, 0H 			//165E 	240C
		RET 					//165F 	1008
		MOVLB 	2H 			//1660 	1022
		BCR 	CH, 0H 			//1661 	200C

		//;sub.c: 214: break; case 7: LATA0=LATB0=LATB3=1;
		RET 					//1662 	1008
		MOVLB 	2H 			//1663 	1022
		BSR 	DH, 3H 			//1664 	258D
		BTSS 	DH, 3H 			//1665 	2D8D
		LJUMP 	66AH 			//1666 	3E6A
		MOVLB 	2H 			//1667 	1022
		BSR 	DH, 0H 			//1668 	240D
		LJUMP 	66CH 			//1669 	3E6C
		MOVLB 	2H 			//166A 	1022
		BCR 	DH, 0H 			//166B 	200D
		BTSS 	DH, 0H 			//166C 	2C0D
		LJUMP 	671H 			//166D 	3E71
		MOVLB 	2H 			//166E 	1022
		BSR 	CH, 0H 			//166F 	240C
		RET 					//1670 	1008
		MOVLB 	2H 			//1671 	1022
		BCR 	CH, 0H 			//1672 	200C

		//;sub.c: 215: break; case 8: LATA0=LATB0=LATB3=LATA4=LATD2=LATA1=LATD3=1;
		RET 					//1673 	1008
		MOVLB 	2H 			//1674 	1022
		BSR 	FH, 3H 			//1675 	258F
		BTSS 	FH, 3H 			//1676 	2D8F
		LJUMP 	67BH 			//1677 	3E7B
		MOVLB 	2H 			//1678 	1022
		BSR 	CH, 1H 			//1679 	248C
		LJUMP 	67DH 			//167A 	3E7D
		MOVLB 	2H 			//167B 	1022
		BCR 	CH, 1H 			//167C 	208C
		BTSS 	CH, 1H 			//167D 	2C8C
		LJUMP 	682H 			//167E 	3E82
		MOVLB 	2H 			//167F 	1022
		BSR 	FH, 2H 			//1680 	250F
		LJUMP 	684H 			//1681 	3E84
		MOVLB 	2H 			//1682 	1022
		BCR 	FH, 2H 			//1683 	210F
		BTSS 	FH, 2H 			//1684 	2D0F
		LJUMP 	689H 			//1685 	3E89
		MOVLB 	2H 			//1686 	1022
		BSR 	CH, 4H 			//1687 	260C
		LJUMP 	68BH 			//1688 	3E8B
		MOVLB 	2H 			//1689 	1022
		BCR 	CH, 4H 			//168A 	220C
		BTSS 	CH, 4H 			//168B 	2E0C
		LJUMP 	690H 			//168C 	3E90
		MOVLB 	2H 			//168D 	1022
		BSR 	DH, 3H 			//168E 	258D
		LJUMP 	692H 			//168F 	3E92
		MOVLB 	2H 			//1690 	1022
		BCR 	DH, 3H 			//1691 	218D
		BTSS 	DH, 3H 			//1692 	2D8D
		LJUMP 	697H 			//1693 	3E97
		MOVLB 	2H 			//1694 	1022
		BSR 	DH, 0H 			//1695 	240D
		LJUMP 	699H 			//1696 	3E99
		MOVLB 	2H 			//1697 	1022
		BCR 	DH, 0H 			//1698 	200D
		BTSS 	DH, 0H 			//1699 	2C0D
		LJUMP 	69EH 			//169A 	3E9E
		MOVLB 	2H 			//169B 	1022
		BSR 	CH, 0H 			//169C 	240C
		RET 					//169D 	1008
		MOVLB 	2H 			//169E 	1022
		BCR 	CH, 0H 			//169F 	200C

		//;sub.c: 216: break; case 9: LATA0=LATB0=LATB3=LATA4=LATA1=LATD3=1;
		RET 					//16A0 	1008
		MOVLB 	2H 			//16A1 	1022
		BSR 	FH, 3H 			//16A2 	258F
		BTSS 	FH, 3H 			//16A3 	2D8F
		LJUMP 	6A8H 			//16A4 	3EA8
		MOVLB 	2H 			//16A5 	1022
		BSR 	CH, 1H 			//16A6 	248C
		LJUMP 	6AAH 			//16A7 	3EAA
		MOVLB 	2H 			//16A8 	1022
		BCR 	CH, 1H 			//16A9 	208C
		BTSS 	CH, 1H 			//16AA 	2C8C
		LJUMP 	6AFH 			//16AB 	3EAF
		MOVLB 	2H 			//16AC 	1022
		BSR 	CH, 4H 			//16AD 	260C
		LJUMP 	6B1H 			//16AE 	3EB1
		MOVLB 	2H 			//16AF 	1022
		BCR 	CH, 4H 			//16B0 	220C
		BTSS 	CH, 4H 			//16B1 	2E0C
		LJUMP 	6B6H 			//16B2 	3EB6
		MOVLB 	2H 			//16B3 	1022
		BSR 	DH, 3H 			//16B4 	258D
		LJUMP 	6B8H 			//16B5 	3EB8
		MOVLB 	2H 			//16B6 	1022
		BCR 	DH, 3H 			//16B7 	218D
		BTSS 	DH, 3H 			//16B8 	2D8D
		LJUMP 	6BDH 			//16B9 	3EBD
		MOVLB 	2H 			//16BA 	1022
		BSR 	DH, 0H 			//16BB 	240D
		LJUMP 	6BFH 			//16BC 	3EBF
		MOVLB 	2H 			//16BD 	1022
		BCR 	DH, 0H 			//16BE 	200D
		BTSS 	DH, 0H 			//16BF 	2C0D
		LJUMP 	6C4H 			//16C0 	3EC4
		MOVLB 	2H 			//16C1 	1022
		BSR 	CH, 0H 			//16C2 	240C
		RET 					//16C3 	1008
		MOVLB 	2H 			//16C4 	1022
		BCR 	CH, 0H 			//16C5 	200C

		//;sub.c: 217: break; case 0: LATA0=LATB0=LATB3=LATA4=LATD2=LATA1=1;
		RET 					//16C6 	1008
		MOVLB 	2H 			//16C7 	1022
		BSR 	CH, 1H 			//16C8 	248C
		BTSS 	CH, 1H 			//16C9 	2C8C
		LJUMP 	6CEH 			//16CA 	3ECE
		MOVLB 	2H 			//16CB 	1022
		BSR 	FH, 2H 			//16CC 	250F
		LJUMP 	6D0H 			//16CD 	3ED0
		MOVLB 	2H 			//16CE 	1022
		BCR 	FH, 2H 			//16CF 	210F
		BTSS 	FH, 2H 			//16D0 	2D0F
		LJUMP 	6D5H 			//16D1 	3ED5
		MOVLB 	2H 			//16D2 	1022
		BSR 	CH, 4H 			//16D3 	260C
		LJUMP 	6D7H 			//16D4 	3ED7
		MOVLB 	2H 			//16D5 	1022
		BCR 	CH, 4H 			//16D6 	220C
		BTSS 	CH, 4H 			//16D7 	2E0C
		LJUMP 	6DCH 			//16D8 	3EDC
		MOVLB 	2H 			//16D9 	1022
		BSR 	DH, 3H 			//16DA 	258D
		LJUMP 	6DEH 			//16DB 	3EDE
		MOVLB 	2H 			//16DC 	1022
		BCR 	DH, 3H 			//16DD 	218D
		BTSS 	DH, 3H 			//16DE 	2D8D
		LJUMP 	6E3H 			//16DF 	3EE3
		MOVLB 	2H 			//16E0 	1022
		BSR 	DH, 0H 			//16E1 	240D
		LJUMP 	6E5H 			//16E2 	3EE5
		MOVLB 	2H 			//16E3 	1022
		BCR 	DH, 0H 			//16E4 	200D
		BTSS 	DH, 0H 			//16E5 	2C0D
		LJUMP 	6EAH 			//16E6 	3EEA
		MOVLB 	2H 			//16E7 	1022
		BSR 	CH, 0H 			//16E8 	240C
		RET 					//16E9 	1008
		MOVLB 	2H 			//16EA 	1022
		BCR 	CH, 0H 			//16EB 	200C

		//;sub.c: 218: break; case 10: LATD3=0;
		RET 					//16EC 	1008
		MOVLB 	2H 			//16ED 	1022
		BCR 	FH, 3H 			//16EE 	218F

		//;sub.c: 220: break; case 11: LATA0=LATB0=LATD2=LATA1=LATD3=1;
		RET 					//16EF 	1008
		MOVLB 	2H 			//16F0 	1022
		BSR 	FH, 3H 			//16F1 	258F
		BTSS 	FH, 3H 			//16F2 	2D8F
		LJUMP 	6F7H 			//16F3 	3EF7
		MOVLB 	2H 			//16F4 	1022
		BSR 	CH, 1H 			//16F5 	248C
		LJUMP 	6F9H 			//16F6 	3EF9
		MOVLB 	2H 			//16F7 	1022
		BCR 	CH, 1H 			//16F8 	208C
		BTSS 	CH, 1H 			//16F9 	2C8C
		LJUMP 	6FEH 			//16FA 	3EFE
		MOVLB 	2H 			//16FB 	1022
		BSR 	FH, 2H 			//16FC 	250F
		LJUMP 	700H 			//16FD 	3F00
		MOVLB 	2H 			//16FE 	1022
		BCR 	FH, 2H 			//16FF 	210F
		BTSS 	FH, 2H 			//1700 	2D0F
		LJUMP 	705H 			//1701 	3F05
		MOVLB 	2H 			//1702 	1022
		BSR 	DH, 0H 			//1703 	240D
		LJUMP 	707H 			//1704 	3F07
		MOVLB 	2H 			//1705 	1022
		BCR 	DH, 0H 			//1706 	200D
		BTSS 	DH, 0H 			//1707 	2C0D
		LJUMP 	70CH 			//1708 	3F0C
		MOVLB 	2H 			//1709 	1022
		BSR 	CH, 0H 			//170A 	240C
		RET 					//170B 	1008
		MOVLB 	2H 			//170C 	1022
		BCR 	CH, 0H 			//170D 	200C

		//;sub.c: 221: break; case 12: LATA0=LATA4=LATD2=LATA1=LATD3=1;
		RET 					//170E 	1008
		MOVLB 	2H 			//170F 	1022
		BSR 	FH, 3H 			//1710 	258F
		BTSS 	FH, 3H 			//1711 	2D8F
		LJUMP 	716H 			//1712 	3F16
		MOVLB 	2H 			//1713 	1022
		BSR 	CH, 1H 			//1714 	248C
		LJUMP 	718H 			//1715 	3F18
		MOVLB 	2H 			//1716 	1022
		BCR 	CH, 1H 			//1717 	208C
		BTSS 	CH, 1H 			//1718 	2C8C
		LJUMP 	71DH 			//1719 	3F1D
		MOVLB 	2H 			//171A 	1022
		BSR 	FH, 2H 			//171B 	250F
		LJUMP 	71FH 			//171C 	3F1F
		MOVLB 	2H 			//171D 	1022
		BCR 	FH, 2H 			//171E 	210F
		BTSS 	FH, 2H 			//171F 	2D0F
		LJUMP 	724H 			//1720 	3F24
		MOVLB 	2H 			//1721 	1022
		BSR 	CH, 4H 			//1722 	260C
		LJUMP 	726H 			//1723 	3F26
		MOVLB 	2H 			//1724 	1022
		BCR 	CH, 4H 			//1725 	220C
		BTSS 	CH, 4H 			//1726 	2E0C
		LJUMP 	72BH 			//1727 	3F2B
		MOVLB 	2H 			//1728 	1022
		BSR 	CH, 0H 			//1729 	240C
		RET 					//172A 	1008
		MOVLB 	2H 			//172B 	1022
		BCR 	CH, 0H 			//172C 	200C

		//;sub.c: 222: break; case 13: LATB0=LATB3=LATD2=LATA1=LATD3=1;
		RET 					//172D 	1008
		MOVLB 	2H 			//172E 	1022
		BSR 	FH, 3H 			//172F 	258F
		BTSS 	FH, 3H 			//1730 	2D8F
		LJUMP 	735H 			//1731 	3F35
		MOVLB 	2H 			//1732 	1022
		BSR 	CH, 1H 			//1733 	248C
		LJUMP 	737H 			//1734 	3F37
		MOVLB 	2H 			//1735 	1022
		BCR 	CH, 1H 			//1736 	208C
		BTSS 	CH, 1H 			//1737 	2C8C
		LJUMP 	73CH 			//1738 	3F3C
		MOVLB 	2H 			//1739 	1022
		BSR 	FH, 2H 			//173A 	250F
		LJUMP 	73EH 			//173B 	3F3E
		MOVLB 	2H 			//173C 	1022
		BCR 	FH, 2H 			//173D 	210F
		BTSS 	FH, 2H 			//173E 	2D0F
		LJUMP 	743H 			//173F 	3F43
		MOVLB 	2H 			//1740 	1022
		BSR 	DH, 3H 			//1741 	258D
		LJUMP 	745H 			//1742 	3F45
		MOVLB 	2H 			//1743 	1022
		BCR 	DH, 3H 			//1744 	218D
		BTSS 	DH, 3H 			//1745 	2D8D
		LJUMP 	74AH 			//1746 	3F4A
		MOVLB 	2H 			//1747 	1022
		BSR 	DH, 0H 			//1748 	240D
		RET 					//1749 	1008
		MOVLB 	2H 			//174A 	1022
		BCR 	DH, 0H 			//174B 	200D

		//;sub.c: 223: break; case 14: LATA4=LATD2=LATA1=1;
		RET 					//174C 	1008
		MOVLB 	2H 			//174D 	1022
		BSR 	CH, 1H 			//174E 	248C
		BTSS 	CH, 1H 			//174F 	2C8C
		LJUMP 	754H 			//1750 	3F54
		MOVLB 	2H 			//1751 	1022
		BSR 	FH, 2H 			//1752 	250F
		LJUMP 	756H 			//1753 	3F56
		MOVLB 	2H 			//1754 	1022
		BCR 	FH, 2H 			//1755 	210F
		BTSS 	FH, 2H 			//1756 	2D0F
		LJUMP 	75BH 			//1757 	3F5B
		MOVLB 	2H 			//1758 	1022
		BSR 	CH, 4H 			//1759 	260C
		RET 					//175A 	1008
		MOVLB 	2H 			//175B 	1022
		BCR 	CH, 4H 			//175C 	220C

		//;sub.c: 224: break; case 15: LATA0=LATD2=LATA1=LATD3=1;
		RET 					//175D 	1008
		MOVLB 	2H 			//175E 	1022
		BSR 	FH, 3H 			//175F 	258F
		BTSS 	FH, 3H 			//1760 	2D8F
		LJUMP 	765H 			//1761 	3F65
		MOVLB 	2H 			//1762 	1022
		BSR 	CH, 1H 			//1763 	248C
		LJUMP 	767H 			//1764 	3F67
		MOVLB 	2H 			//1765 	1022
		BCR 	CH, 1H 			//1766 	208C
		BTSS 	CH, 1H 			//1767 	2C8C
		LJUMP 	76CH 			//1768 	3F6C
		MOVLB 	2H 			//1769 	1022
		BSR 	FH, 2H 			//176A 	250F
		LJUMP 	76EH 			//176B 	3F6E
		MOVLB 	2H 			//176C 	1022
		BCR 	FH, 2H 			//176D 	210F
		BTSS 	FH, 2H 			//176E 	2D0F
		LJUMP 	773H 			//176F 	3F73
		MOVLB 	2H 			//1770 	1022
		BSR 	CH, 0H 			//1771 	240C
		RET 					//1772 	1008
		MOVLB 	2H 			//1773 	1022
		BCR 	CH, 0H 			//1774 	200C

		//;sub.c: 225: }
		RET 					//1775 	1008
		LDR 	70H, 0H 			//1776 	1870
		STR 	4H 			//1777 	1084
		LDWI 	10H 			//1778 	0010
		SUBWR 	4H, 0H 		//1779 	1204
		BTSC 	3H, 0H 			//177A 	2803
		RET 					//177B 	1008
		MOVLP 	6H 			//177C 	0186
		LSLF 	4H, 0H 			//177D 	0504
		ADDWI 	72H 			//177E 	0E72
		STR 	2H 			//177F 	1082
		RET 					//1780 	1008

		//;sub.c: 230: unsigned char DspBuf;
		//;sub.c: 231: switch(DspIndex)
		LJUMP 	793H 			//1781 	3F93
		CLRF 	74H 			//1782 	11F4

		//;sub.c: 234: break; case DsMode: DspBuf=11;
		LJUMP 	79BH 			//1783 	3F9B
		LDWI 	BH 			//1784 	000B
		LJUMP 	791H 			//1785 	3F91

		//;sub.c: 235: break; case DsStrong: DspBuf=12;
		LDWI 	CH 			//1786 	000C
		LJUMP 	791H 			//1787 	3F91

		//;sub.c: 236: break; case DsHeat: DspBuf=13;
		LDWI 	DH 			//1788 	000D
		LJUMP 	791H 			//1789 	3F91

		//;sub.c: 237: break; case DsManual: DspBuf=14;
		LDWI 	EH 			//178A 	000E
		LJUMP 	791H 			//178B 	3F91

		//;sub.c: 238: break; case DsTime: DspBuf=(TimeMin/10);
		LDWI 	AH 			//178C 	000A
		STR 	70H 			//178D 	10F0
		LDR 	77H, 0H 			//178E 	1877
		LCALL 	79DH 			//178F 	379D
		MOVLP 	15H 			//1790 	0195
		STR 	74H 			//1791 	10F4

		//;sub.c: 239: break;
		LJUMP 	79BH 			//1792 	3F9B
		LCALL 	7FAH 			//1793 	37FA
		MOVLP 	15H 			//1794 	0195
		BTSC 	3H, 0H 			//1795 	2803
		LJUMP 	79BH 			//1796 	3F9B
		MOVLP 	6H 			//1797 	0186
		LSLF 	4H, 0H 			//1798 	0504
		ADDWI 	C8H 			//1799 	0EC8
		STR 	2H 			//179A 	1082

		//;sub.c: 241: return DspBuf;
		LDR 	74H, 0H 			//179B 	1874
		RET 					//179C 	1008
		STR 	71H 			//179D 	10F1
		CLRF 	73H 			//179E 	11F3
		LDR 	70H, 0H 			//179F 	1870
		BTSC 	3H, 2H 			//17A0 	2903
		LJUMP 	7B3H 			//17A1 	3FB3
		CLRF 	72H 			//17A2 	11F2
		INCR 	72H, 1H 		//17A3 	1AF2
		BTSC 	70H, 7H 		//17A4 	2BF0
		LJUMP 	7A8H 			//17A5 	3FA8
		LSLF 	70H, 1H 		//17A6 	05F0
		LJUMP 	7A3H 			//17A7 	3FA3
		LSLF 	73H, 1H 		//17A8 	05F3
		LDR 	70H, 0H 			//17A9 	1870
		SUBWR 	71H, 0H 		//17AA 	1271
		BTSS 	3H, 0H 			//17AB 	2C03
		LJUMP 	7B0H 			//17AC 	3FB0
		LDR 	70H, 0H 			//17AD 	1870
		SUBWR 	71H, 1H 		//17AE 	12F1
		BSR 	73H, 0H 			//17AF 	2473
		LSRF 	70H, 1H 		//17B0 	06F0
		DECRSZ 	72H, 1H 		//17B1 	1BF2
		LJUMP 	7A8H 			//17B2 	3FA8
		LDR 	73H, 0H 			//17B3 	1873
		RET 					//17B4 	1008

		//;sub.c: 246: unsigned char DspBuf;
		//;sub.c: 247: switch(DspIndex)
		LJUMP 	7C3H 			//17B5 	3FC3
		LDWI 	FH 			//17B6 	000F
		LJUMP 	7C1H 			//17B7 	3FC1

		//;sub.c: 250: break; case DsMode: DspBuf=ModeType;
		LDR 	7AH, 0H 			//17B8 	187A
		LJUMP 	7C1H 			//17B9 	3FC1

		//;sub.c: 251: break; case DsStrong: DspBuf=StrongType;
		LDR 	79H, 0H 			//17BA 	1879
		LJUMP 	7C1H 			//17BB 	3FC1

		//;sub.c: 252: break; case DsHeat: DspBuf=HeatType;
		LDR 	64H, 0H 			//17BC 	1864
		LJUMP 	7C1H 			//17BD 	3FC1

		//;sub.c: 253: break; case DsManual: DspBuf=ManualType;
		LDR 	78H, 0H 			//17BE 	1878
		LJUMP 	7C1H 			//17BF 	3FC1

		//;sub.c: 254: break; case DsTime: DspBuf=TimeMin;
		LDR 	77H, 0H 			//17C0 	1877
		STR 	75H 			//17C1 	10F5

		//;sub.c: 255: break;
		LJUMP 	7CBH 			//17C2 	3FCB
		LCALL 	7FAH 			//17C3 	37FA
		MOVLP 	15H 			//17C4 	0195
		BTSC 	3H, 0H 			//17C5 	2803
		LJUMP 	7CBH 			//17C6 	3FCB
		MOVLP 	6H 			//17C7 	0186
		LSLF 	4H, 0H 			//17C8 	0504
		ADDWI 	BCH 			//17C9 	0EBC
		STR 	2H 			//17CA 	1082

		//;sub.c: 257: if(DspIndex!=DsSty)
		LDR 	60H, 0H 			//17CB 	1860
		BTSC 	3H, 2H 			//17CC 	2903
		LJUMP 	7D3H 			//17CD 	3FD3

		//;sub.c: 258: DspBuf%=10;
		LDWI 	AH 			//17CE 	000A
		STR 	70H 			//17CF 	10F0
		LDR 	75H, 0H 			//17D0 	1875
		LCALL 	7D5H 			//17D1 	37D5
		STR 	75H 			//17D2 	10F5

		//;sub.c: 259: return DspBuf;
		LDR 	75H, 0H 			//17D3 	1875
		RET 					//17D4 	1008
		STR 	72H 			//17D5 	10F2
		LDWI 	8H 			//17D6 	0008
		STR 	73H 			//17D7 	10F3
		CLRF 	74H 			//17D8 	11F4
		LDR 	72H, 0H 			//17D9 	1872
		STR 	71H 			//17DA 	10F1
		LDWI 	7H 			//17DB 	0007
		LSRF 	71H, 1H 		//17DC 	06F1
		DECRSZ 	9H, 1H 		//17DD 	1B89
		LJUMP 	7DCH 			//17DE 	3FDC
		LSLF 	74H, 0H 		//17DF 	0574
		IORWR 	71H, 0H 		//17E0 	1471
		STR 	74H 			//17E1 	10F4
		LSLF 	72H, 1H 		//17E2 	05F2
		LDR 	70H, 0H 			//17E3 	1870
		SUBWR 	74H, 0H 		//17E4 	1274
		BTSS 	3H, 0H 			//17E5 	2C03
		LJUMP 	7E9H 			//17E6 	3FE9
		LDR 	70H, 0H 			//17E7 	1870
		SUBWR 	74H, 1H 		//17E8 	12F4
		DECRSZ 	73H, 1H 		//17E9 	1BF3
		LJUMP 	7D9H 			//17EA 	3FD9
		LDR 	74H, 0H 			//17EB 	1874
		RET 					//17EC 	1008
		MOVLB 	2H 			//17ED 	1022
		BCR 	DH, 1H 			//17EE 	208D
		BCR 	FH, 1H 			//17EF 	208F
		BCR 	EH, 1H 			//17F0 	208E
		BCR 	EH, 0H 			//17F1 	200E
		BCR 	CH, 0H 			//17F2 	200C
		BCR 	DH, 0H 			//17F3 	200D
		BCR 	DH, 3H 			//17F4 	218D
		BCR 	CH, 4H 			//17F5 	220C
		BCR 	FH, 2H 			//17F6 	210F
		BCR 	CH, 1H 			//17F7 	208C
		BCR 	FH, 3H 			//17F8 	218F
		RET 					//17F9 	1008
		MOVLB 	1H 			//17FA 	1021
		LDR 	60H, 0H 			//17FB 	1860
		STR 	4H 			//17FC 	1084
		LDWI 	6H 			//17FD 	0006
		SUBWR 	4H, 0H 		//17FE 	1204
		RET 					//17FF 	1008
		RETW 	0H 			//1800 	0400
		RETW 	0H 			//1801 	0400
		RETW 	0H 			//1802 	0400
		RETW 	0H 			//1803 	0400
		RETW 	0H 			//1804 	0400
		RETW 	0H 			//1805 	0400
		RETW 	0H 			//1806 	0400
		RETW 	0H 			//1807 	0400
		RETW 	0H 			//1808 	0400
		RETW 	0H 			//1809 	0400
		RETW 	0H 			//180A 	0400
		RETW 	0H 			//180B 	0400
		RETW 	0H 			//180C 	0400
		RETW 	0H 			//180D 	0400
		RETW 	0H 			//180E 	0400
		RETW 	0H 			//180F 	0400
		RETW 	0H 			//1810 	0400
		RETW 	0H 			//1811 	0400
		RETW 	0H 			//1812 	0400
		RETW 	0H 			//1813 	0400
		RETW 	0H 			//1814 	0400
		RETW 	0H 			//1815 	0400
		RETW 	0H 			//1816 	0400
		RETW 	0H 			//1817 	0400
		RETW 	0H 			//1818 	0400
		RETW 	0H 			//1819 	0400
		RETW 	0H 			//181A 	0400
		RETW 	0H 			//181B 	0400
		RETW 	0H 			//181C 	0400
		RETW 	0H 			//181D 	0400
		RETW 	0H 			//181E 	0400
		RETW 	0H 			//181F 	0400
		RETW 	0H 			//1820 	0400
		RETW 	0H 			//1821 	0400
		RETW 	0H 			//1822 	0400
		RETW 	0H 			//1823 	0400
		RETW 	0H 			//1824 	0400
		RETW 	0H 			//1825 	0400
		RETW 	0H 			//1826 	0400
		RETW 	0H 			//1827 	0400
		RETW 	0H 			//1828 	0400
		RETW 	0H 			//1829 	0400
		RETW 	16H 			//182A 	0416
		RETW 	13H 			//182B 	0413
		RETW 	12H 			//182C 	0412
		RETW 	FH 			//182D 	040F
		RETW 	0H 			//182E 	0400
		RETW 	0H 			//182F 	0400
		RETW 	0H 			//1830 	0400
		RETW 	0H 			//1831 	0400
		RETW 	0H 			//1832 	0400
		RETW 	0H 			//1833 	0400
		RETW 	0H 			//1834 	0400
		RETW 	0H 			//1835 	0400
		RETW 	0H 			//1836 	0400
		RETW 	0H 			//1837 	0400
		RETW 	0H 			//1838 	0400
		RETW 	0H 			//1839 	0400
		RETW 	0H 			//183A 	0400
		RETW 	0H 			//183B 	0400
		RETW 	0H 			//183C 	0400
		RETW 	0H 			//183D 	0400
		RETW 	0H 			//183E 	0400
		RETW 	0H 			//183F 	0400
		RETW 	0H 			//1840 	0400
		RETW 	0H 			//1841 	0400
		RETW 	16H 			//1842 	0416
		RETW 	14H 			//1843 	0414
		RETW 	13H 			//1844 	0413
		RETW 	11H 			//1845 	0411
		RETW 	0H 			//1846 	0400
		RETW 	0H 			//1847 	0400
		RETW 	11H 			//1848 	0411
		RETW 	FH 			//1849 	040F
		RETW 	FH 			//184A 	040F
		RETW 	CH 			//184B 	040C
		RETW 	0H 			//184C 	0400
		RETW 	0H 			//184D 	0400
		RETW 	EH 			//184E 	040E
		RETW 	CH 			//184F 	040C
		RETW 	CH 			//1850 	040C
		RETW 	9H 			//1851 	0409
		RETW 	0H 			//1852 	0400
		RETW 	0H 			//1853 	0400
		RETW 	EH 			//1854 	040E
		RETW 	CH 			//1855 	040C
		RETW 	CH 			//1856 	040C
		RETW 	9H 			//1857 	0409
		RETW 	0H 			//1858 	0400
		RETW 	0H 			//1859 	0400
		RETW 	0H 			//185A 	0400
		RETW 	0H 			//185B 	0400
		RETW 	0H 			//185C 	0400
		RETW 	0H 			//185D 	0400
		RETW 	0H 			//185E 	0400
		RETW 	0H 			//185F 	0400
		RETW 	0H 			//1860 	0400
		RETW 	0H 			//1861 	0400
		RETW 	0H 			//1862 	0400
		RETW 	0H 			//1863 	0400
		RETW 	0H 			//1864 	0400
		RETW 	0H 			//1865 	0400
		RETW 	0H 			//1866 	0400
		RETW 	0H 			//1867 	0400
		RETW 	0H 			//1868 	0400
		RETW 	0H 			//1869 	0400
		RETW 	0H 			//186A 	0400
		RETW 	0H 			//186B 	0400
		RETW 	0H 			//186C 	0400
		RETW 	0H 			//186D 	0400
		RETW 	0H 			//186E 	0400
		RETW 	0H 			//186F 	0400
		RETW 	0H 			//1870 	0400
		RETW 	0H 			//1871 	0400
		RETW 	0H 			//1872 	0400
		RETW 	0H 			//1873 	0400
		RETW 	0H 			//1874 	0400
		RETW 	0H 			//1875 	0400
		RETW 	0H 			//1876 	0400
		RETW 	0H 			//1877 	0400
		RETW 	0H 			//1878 	0400
		RETW 	0H 			//1879 	0400
		RETW 	0H 			//187A 	0400
		RETW 	0H 			//187B 	0400
		RETW 	0H 			//187C 	0400
		RETW 	0H 			//187D 	0400
		RETW 	0H 			//187E 	0400
		RETW 	0H 			//187F 	0400
		RETW 	0H 			//1880 	0400
		RETW 	0H 			//1881 	0400
		RETW 	0H 			//1882 	0400
		RETW 	0H 			//1883 	0400
		RETW 	23H 			//1884 	0423
		RETW 	1FH 			//1885 	041F
		RETW 	1EH 			//1886 	041E
		RETW 	18H 			//1887 	0418
		RETW 	0H 			//1888 	0400
		RETW 	0H 			//1889 	0400
		RETW 	0H 			//188A 	0400
		RETW 	0H 			//188B 	0400
		RETW 	0H 			//188C 	0400
		RETW 	0H 			//188D 	0400
		RETW 	0H 			//188E 	0400
		RETW 	0H 			//188F 	0400
		RETW 	0H 			//1890 	0400
		RETW 	0H 			//1891 	0400
		RETW 	0H 			//1892 	0400
		RETW 	0H 			//1893 	0400
		RETW 	0H 			//1894 	0400
		RETW 	0H 			//1895 	0400
		RETW 	0H 			//1896 	0400
		RETW 	0H 			//1897 	0400
		RETW 	0H 			//1898 	0400
		RETW 	0H 			//1899 	0400
		RETW 	0H 			//189A 	0400
		RETW 	0H 			//189B 	0400
		RETW 	24H 			//189C 	0424
		RETW 	20H 			//189D 	0420
		RETW 	1FH 			//189E 	041F
		RETW 	1BH 			//189F 	041B
		RETW 	0H 			//18A0 	0400
		RETW 	0H 			//18A1 	0400
		RETW 	1BH 			//18A2 	041B
		RETW 	19H 			//18A3 	0419
		RETW 	18H 			//18A4 	0418
		RETW 	13H 			//18A5 	0413
		RETW 	0H 			//18A6 	0400
		RETW 	0H 			//18A7 	0400
		RETW 	17H 			//18A8 	0417
		RETW 	14H 			//18A9 	0414
		RETW 	14H 			//18AA 	0414
		RETW 	FH 			//18AB 	040F
		RETW 	0H 			//18AC 	0400
		RETW 	0H 			//18AD 	0400
		RETW 	18H 			//18AE 	0418
		RETW 	13H 			//18AF 	0413
		RETW 	14H 			//18B0 	0414
		RETW 	FH 			//18B1 	040F
		RETW 	0H 			//18B2 	0400
		RETW 	0H 			//18B3 	0400
		RETW 	0H 			//18B4 	0400
		RETW 	0H 			//18B5 	0400
		RETW 	0H 			//18B6 	0400
		RETW 	0H 			//18B7 	0400
		RETW 	0H 			//18B8 	0400
		RETW 	0H 			//18B9 	0400
		RETW 	0H 			//18BA 	0400
		RETW 	0H 			//18BB 	0400
		RETW 	80H 			//18BC 	0480
		RETW 	78H 			//18BD 	0478
		RETW 	CH 			//18BE 	040C
		RETW 	0H 			//18BF 	0400
		RETW 	4H 			//18C0 	0404
			END
