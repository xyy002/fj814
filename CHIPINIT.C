...
void Sys_init()
{
	...
	// OSCCON = 0B01110001;   //WDT 32KHZ IRCF=111=16MHZ/2=8MHZ,62.5NS/T
	 ...
	 PORTB = 0B00000000;	 
	 TRISB = 0B00010000;	 //PB输入输出 0-输出 1-输入 						 
	 PORTC = 0B00000000;	 
...
void Time1Initial(void)
{
    ...
    CKOCON=0B00100000;
    TCKSRC=0B00000001;    //TIM1时钟为HIRC的1倍频 16k
    ...
    TIM1CCMR1 =0B00000000;//CC1通道被配置为输出
		//BIT7：输出比较1清零使能
    ...
    TIM1CCER1 =0B00000000; //比较1互补输出使能，低电平有效；比较器1输出使能，低电平有效
...
void Px_Level_Change_INITIAL(void)
{
    EPS0=0B00000000; 	//
    //Px0~Px3中断管脚选择
    EPS1=0B00000001;   //选择PB7管脚中断
    //Px4~Px7中断管脚选择 
    ITYPE0 = 0B00000000;//双沿中断
    ITYPE1 = 0B11000000;  
    EPIE0  = 0B00010000;//使能中断7  
    INTCON = 0B01000000;  //外设中断使能
}
...
void UART_INITIAL(void)
{
    ...
    URIER =0B00000000;  //使能发接收中断
    URLCR =0B00000001;  //8位数据，停止位1，无奇偶校验
    URMCR =0B00011000;
    
    //URDLL =104;         //9600波特率 = Fosc/16*[URDLH:URDLL]   16M
	URDLL =52;         //9600波特率 = Fosc/16*[URDLH:URDLL]   8M
    URDLH =0;
    TCF=1;
	//INTCON=0B11000000;
    //TCF: 发送完成标志
